...
// LsimulatorCharacter, LsimulatorDropTable, LsimulatorObject, ItemInstance,
// LsimulatorWorld
//

public class PcInstance extends LsimulatorCharacter {
		...
		if (getMapId() <= 10000) {
			for (LsimulatorObject visible : LsimulatorWorld.getInstance().getVisibleObjects(this, Config.PC_RECOGNIZE_RANGE)) {
				if (!knownsObject(visible)) {
					...
					if (visible instanceof NpcInstance) {
						NpcInstance npc = (NpcInstance) visible;
	 ...
	 *            通知するプレイヤーの配列
	 */
	private void notifyPlayersLogout(List<PcInstance> playersArray) {
		for (PcInstance player : playersArray) {
	...
	@Override
	public void onAction(PcInstance attacker) {
		onAction(attacker, 0);
	}
	...
	public boolean checkNonPvP(PcInstance pc, LsimulatorCharacter target) {
		PcInstance targetpc = null;
		if (target instanceof PcInstance) {
			targetpc = (PcInstance) target;
		}
		else if (target instanceof PetInstance) {
			targetpc = (PcInstance) ((PetInstance) target).getMaster();
		}
		else if (target instanceof SummonInstance) {
			targetpc = (PcInstance) ((SummonInstance) target).getMaster();
		}
		if (targetpc == null) {
	...
	private boolean isInWarAreaAndWarTime(PcInstance pc, PcInstance target) {
	...
	public void setPetTarget(LsimulatorCharacter target) {
		...
		for (Object pet : petList) {
			if (pet instanceof PetInstance) {
				PetInstance pets = (PetInstance) pet;
				pets.setMasterTarget(target);
			}
			else if (pet instanceof SummonInstance) {
				SummonInstance summon = (SummonInstance) pet;
	...
	public void receiveManaDamage(LsimulatorCharacter attacker, int mpDamage) { // 攻撃でＭＰを減らすときはここを使用
		if ((mpDamage > 0) && !isDead()) {
			...
			if ((attacker instanceof PcInstance) && ((PcInstance) attacker).isPinkName()) {
				...
				for (LsimulatorObject object : LsimulatorWorld.getInstance().getVisibleObjects(attacker)) {
					if (object instanceof GuardInstance) {
						GuardInstance guard = (GuardInstance) object;
						guard.setTarget(((PcInstance) attacker));
					}
				}
			}
	...
	public void receiveDamage(LsimulatorCharacter attacker, double damage, boolean isMagicDamage) { // 攻撃でＨＰを減らすときはここを使用
		if ((getCurrentHp() > 0) && !isDead()) {
			if (attacker != this) {
				if (!(attacker instanceof EffectInstance) && !knownsObject(attacker) && (attacker.getMapId() == getMapId())) {
			...
			if (hasSkillEffect(MORTAL_BODY) && (getId() != attacker.getId())) {
				...
				if ((damage > 0) && (rnd <= 10)) {
					if (attacker instanceof PcInstance) {
						PcInstance attackPc = (PcInstance) attacker;
						...
						NpcInstance attackNpc = (NpcInstance) attacker;
	...
	private class Death implements Runnable {
		...
		public void run() {
			...
			l1skilluse.handleCommands(PcInstance.this, CANCELLATION, getId(), getX(), getY(), null, 0, LsimulatorSkillUse.TYPE_LOGIN);
			...
			if (lastAttacker != PcInstance.this) {
				// セーフティーゾーン、コンバットゾーンで最後に殺したキャラが
				// プレイヤーorペットだったら、ペナルティなし
				if (getZoneType() != 0) {
					PcInstance player = null;
					if (lastAttacker instanceof PcInstance) {
						player = (PcInstance) lastAttacker;
					}
					else if (lastAttacker instanceof PetInstance) {
						player = (PcInstance) ((PetInstance) lastAttacker).getMaster();
					}
					else if (lastAttacker instanceof SummonInstance) {
						player = (PcInstance) ((SummonInstance) lastAttacker).getMaster();
					}
					if (player != null) {
						// 戦争中に戦争エリアに居る場合は例外
						if (!isInWarAreaAndWarTime(PcInstance.this, player)) {
							return;
						}
					}
				}
			...
			// 決闘中ならペナルティなし
			PcInstance fightPc = null;
			if (lastAttacker instanceof PcInstance) {
				fightPc = (PcInstance) lastAttacker;
			}
			...
			if (lastAttacker instanceof GuardInstance) {
				if (get_PKcount() > 0) {
					set_PKcount(get_PKcount() - 1);
				}
				setLastPk(null);
			}
			if (lastAttacker instanceof GuardianInstance) {
			...
			_pcDeleteTimer = new LsimulatorPcDeleteTimer(PcInstance.this);
	...
	private void caoPenaltyResult(int count) {
		for (int i = 0; i < count; i++) {
			ItemInstance item = getInventory().CaoPenalty();
	...
	public boolean simWarResult(LsimulatorCharacter lastAttacker) {
		...
		PcInstance attacker = null;
		String enemyClanName = null;
		boolean sameWar = false;

		if (lastAttacker instanceof PcInstance) {
			attacker = (PcInstance) lastAttacker;
		}
		else if (lastAttacker instanceof PetInstance) {
			attacker = (PcInstance) ((PetInstance) lastAttacker).getMaster();
		}
		else if (lastAttacker instanceof SummonInstance) {
			attacker = (PcInstance) ((SummonInstance) lastAttacker).getMaster();
		}
		else {
			return false;
		}
	...
	private static Logger _log = Logger.getLogger(PcInstance.class.getName());
