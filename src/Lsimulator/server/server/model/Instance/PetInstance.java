...
public class PetInstance extends NpcInstance {
	...
	/** 領出寵物 */
	public PetInstance(LsimulatorNpc template, PcInstance master, LsimulatorPet l1pet) {
		super(template);
	...
	/** 馴養寵物 */
	public PetInstance(NpcInstance target, PcInstance master, int itemid) {
		super(null);
	...
	public void evolvePet(int new_itemobjid) {
		...
		if ((obj != null) && (obj instanceof NpcInstance)) {
			PetInstance new_pet = (PetInstance) obj;
			LsimulatorInventory new_petInventory = new_pet.getInventory();
			List<ItemInstance> itemList = getInventory().getItems();
	...
	public void onFinalAction(PcInstance player, String action) {
		...
		if (status == 6) {
			...
			for (Object petObject : petList) {
				if (petObject instanceof PetInstance) { // 寵物
					PetInstance pet = (PetInstance) petObject;
					if ((_petMaster != null)
				...
				} else if (petObject instanceof SummonInstance) { // 召喚獸
					SummonInstance summon = (SummonInstance) petObject;
	...
	// 使用寵物裝備
	public void usePetWeapon(PetInstance pet, ItemInstance weapon) {
	...
	public void usePetArmor(PetInstance pet, ItemInstance armor) {
	...
	private void setPetWeapon(PetInstance pet, ItemInstance weapon) {
	...
	private void removePetWeapon(PetInstance pet, ItemInstance weapon) {
		int itemId = weapon.getItem().getItemId();
		LsimulatorPetItem petItem = PetItemTable.getInstance().getTemplate(itemId);
		if (petItem == null) {
			return;
	...
	private void setPetArmor(PetInstance pet, ItemInstance armor) {
	...
	private void removePetArmor(PetInstance pet, ItemInstance armor) {
		int itemId = armor.getItem().getItemId();
		LsimulatorPetItem petItem = PetItemTable.getInstance().getTemplate(itemId);
		if (petItem == null) {
			return;
