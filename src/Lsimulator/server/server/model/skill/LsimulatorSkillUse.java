...
import Lsimulator.server.server.serverpackets.S_RangeSkill;
...
import Lsimulator.server.server.serverpackets.S_SkillIconShield;
import Lsimulator.server.server.serverpackets.S_SkillIconWaterLife;
...
import Lsimulator.server.server.serverpackets.S_Strup;
import Lsimulator.server.server.serverpackets.S_TrueTarget;
...
public class LsimulatorSkillUse {
	...
	private LsimulatorSkills _skill;
	...
	private LsimulatorCharacter _user = null;
	...
	private static Logger _log = Logger.getLogger(LsimulatorSkillUse.class.getName());
	...
	private static class TargetStatus {
		...
		public TargetStatus(LsimulatorCharacter _cha) {
			_target = _cha;
		}

		public TargetStatus(LsimulatorCharacter _cha, boolean _flg) {
			_isCalc = _flg;
		}

		public LsimulatorCharacter getTarget() {
			return _target;
		}
	...
	public boolean checkUseSkill(LsimulatorPcInstance player, int skillid, int target_id, int x, int y, String message, int time, int type,
		...
		// 事前チェック
		if (!(l1object instanceof LsimulatorCharacter)) { // ターゲットがキャラクター以外の場合何もしない。
			checkedResult = false;
		}
	...
	private boolean isNormalSkillUsable() {
		...
		if (_user instanceof LsimulatorPcInstance) {
			...
			LsimulatorPolyMorph poly = PolyTable.getInstance().getTemplate(polyId);
			// 魔法が使えない変身
	...
	private boolean isTarget(LsimulatorCharacter cha) throws Exception {
		...
		// 攻擊型魔法無法攻擊自己
		if ((_skill.getType() == LsimulatorSkills.TYPE_ATTACK) && (cha.getId() == _user.getId())) {
			return false;
		}
		...
		if ((((_skill.getTargetTo() & LsimulatorSkills.TARGET_TO_PC) == LsimulatorSkills.TARGET_TO_PC)
				|| ((_skill.getTargetTo() & LsimulatorSkills.TARGET_TO_CLAN) == LsimulatorSkills.TARGET_TO_CLAN) || ((_skill.getTargetTo() & LsimulatorSkills.TARGET_TO_PARTY) == LsimulatorSkills.TARGET_TO_PARTY))
				...
				for (LsimulatorWar war : LsimulatorWorld.getInstance().getWarList()) {
					if (war.CheckClanInWar(_player.getClanname())) { // 自クランが戦争に参加中
						if (war.CheckClanInSameWar( // 同じ戦争に参加中
							...
							if (LsimulatorCastleLocation.checkInAllWarArea(enemy.getX(), enemy.getY(), enemy.getMapId())) {
								return true;
							}
						}
					}
...
*/
	...
	private void makeTargetList() {
		try {
			...
			if ((_skill.getTargetTo() == LsimulatorSkills.TARGET_TO_ME) && ((_skill.getType() & LsimulatorSkills.TYPE_ATTACK) != LsimulatorSkills.TYPE_ATTACK)) {
			...
			if ((_skillId == LIGHTNING) || (_skillId == FREEZING_BREATH)) { // ライトニング、フリージングブレス直線的に範囲を決める
				List<LsimulatorObject> al1object = LsimulatorWorld.getInstance().getVisibleLineObjects(_user, _target);

				for (LsimulatorObject tgobj : al1object) {
					if (tgobj == null) {
						continue;
					}
					if (!(tgobj instanceof LsimulatorCharacter)) { // ターゲットがキャラクター以外の場合何もしない。
						continue;
					}
			...
			if (getSkillArea() == 0) { // 単体の場合
				if (!_user.glanceCheck(_target.getX(), _target.getY())) { // 直線上に障害物があるか
					if (((_skill.getType() & LsimulatorSkills.TYPE_ATTACK) == LsimulatorSkills.TYPE_ATTACK) && (_skillId != 10026) && (_skillId != 10027)
				...
				if ((_skillId != 49) && !(_skill.getTarget().equals("attack") || (_skill.getType() == LsimulatorSkills.TYPE_ATTACK))) {
					// 攻撃系以外のスキルとH-A以外はターゲット自身を含める
					_targetList.add(new TargetStatus(_user));
				}

				List<LsimulatorObject> objects;
				if (getSkillArea() == -1) {
					objects = LsimulatorWorld.getInstance().getVisibleObjects(_user);
				}
				else {
					objects = LsimulatorWorld.getInstance().getVisibleObjects(_target, getSkillArea());
				}
				for (LsimulatorObject tgobj : objects) {
					if (tgobj == null) {
						continue;
					}
					if (!(tgobj instanceof LsimulatorCharacter)) { // ターゲットがキャラクター以外の場合何もしない。
			...
			_log.log(Level.FINEST, "exception in LsimulatorSkilluse makeTargetList{0}", e);
		}
	}
	...
	private void sendFailMessageHandle() {
		...
		if ((_skill.getType() != LsimulatorSkills.TYPE_ATTACK) && !_skill.getTarget().equals("none") && _targetList.isEmpty()) {
			sendFailMessage();
		}
	}
	...
	// マジックリストに追加する。
	private void addMagicList(LsimulatorCharacter cha, boolean repetition) {
		if (_skillTime == 0) {
		...
		if (_skillId == CURSE_POISON) {  // 毒咒持續時間移至 LsimulatorPoison 處理。
			return;
		}
		if ((_skillId == CURSE_PARALYZE) || (_skillId == CURSE_PARALYZE2)) { // 木乃伊的咀咒、石化持續時間移至 LsimulatorCurseParalysis 處理。
			return;
		}
		if (_skillId == SHAPE_CHANGE) { // 變形術持續時間移至 LsimulatorPolyMorph 處理。
			return;
		}
		if ((_skillId == BLESSED_ARMOR) || (_skillId == HOLY_WEAPON // 武器・防具に効果がある処理はLsimulatorItemInstanceに移譲。
		...
		if ((_skillId == AWAKEN_ANTHARAS) || (_skillId == AWAKEN_FAFURION) || (_skillId == AWAKEN_VALAKAS)) { // 覚醒の効果処理はLsimulatorAwakeに移譲。
			return;
		}
		// 骷髏毀壞持續時間另外處理
		...
		if (_user instanceof LsimulatorPcInstance) {
			...
			if (_skill.getTarget().equals("attack") && (_skillId != 18)) {
				...
				if (getSkillArea() == 0) { // 單體攻擊魔法
					...
					LsimulatorCharacter[] cha = new LsimulatorCharacter[_targetList.size()];
					int i = 0;
			...
			else if (_skill.getTarget().equals("none") && (_skill.getType() == LsimulatorSkills.TYPE_ATTACK)) { // 無方向範囲攻撃魔法
				LsimulatorCharacter[] cha = new LsimulatorCharacter[_targetList.size()];
				int i = 0;
				for (TargetStatus ts : _targetList) {
					cha[i] = ts.getTarget();
	...
	/** 刪除重複的魔法狀態 */
	private void deleteRepeatedSkills(LsimulatorCharacter cha) {
	...
	// 重複しているスキルを一旦すべて削除
	private void stopSkillList(LsimulatorCharacter cha, int[] repeat_skill) {
	...
	private void setDelay() {
		if (_skill.getReuseDelay() > 0) {
			LsimulatorSkillDelay.onSkillUse(_user, _skill.getReuseDelay());
		}
	}
	...
	private void runSkill() {
		...
		switch(_skillId) {
			case LIFE_STREAM:
				LsimulatorEffectSpawn.getInstance().spawnEffect(81169, _skill.getBuffDuration() * 1000, _targetX, _targetY, _user.getMapId());
				return;
			case CUBE_IGNITION:
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				// 確率系スキルで失敗が確定している場合
				if (((_skill.getType() == LsimulatorSkills.TYPE_CURSE) || (_skill.getType() == LsimulatorSkills.TYPE_PROBABILITY)) && isTargetFailure(cha)) {
					iter.remove();
					continue;
				}
				...
				if ((_skill.getType() == LsimulatorSkills.TYPE_ATTACK) && (_user.getId() != cha.getId())) { // 攻撃系スキル＆ターゲットが使用者以外であること。
				...
				else if ((_skill.getType() == LsimulatorSkills.TYPE_CURSE) || (_skill.getType() == LsimulatorSkills.TYPE_PROBABILITY)) { // 確率系スキル
				...
				// 治癒性魔法
				else if (_skill.getType() == LsimulatorSkills.TYPE_HEAL) {
					// 回復量
				...
				switch(_skillId) {
					...
					case 20011: // 毒霧-前方 3X3
						...
						for (int i = 0; i < 3; i++) {
							for (int j = 0; j < 3; j++) {
								...
								LsimulatorEffectSpawn.getInstance().spawnEffect(93002, 10000, _user.getX() - locX, _user.getY() - locY, _user.getMapId());
							}
						}
						break;
					// 衝擊之暈
						...
						if (cha instanceof LsimulatorPcInstance) {
							...
							if ((item != null) && (item.getItem().getType2() == 1)) {
								...
								if (safe_enchant < 0) { // 強化不可
									...
									pc.getInventory().updateItem(item, LsimulatorPcInventory.COL_ENCHANTLVL);
								}
								else {
									pc.sendPackets( // \f1何も起きませんでした。
									new S_ServerMessage(79));
					...
					case BLESSED_ARMOR:
						...
						break;
					default:
						LsimulatorBuffUtil.skillEffect(_user, cha, _target, _skillId, _getBuffIconDuration, dmg);
						break;
				}
				...
				// ■■■■ 個別処理ここまで ■■■■
				 ...
				 // 治癒性魔法攻擊不死係的怪物。
				if ((_skill.getType() == LsimulatorSkills.TYPE_HEAL) && (_calcType == PC_NPC) && (undeadType == 1)) {
					dmg *= -1;
				}
				// 治癒性魔法無法對此不死係起作用
				if ((_skill.getType() == LsimulatorSkills.TYPE_HEAL) && (_calcType == PC_NPC) && (undeadType == 3)) {
					dmg = 0;
				}
				// 無法對城門、守護塔補血
				...
				// 補血判斷
				if ((_skill.getType() == LsimulatorSkills.TYPE_HEAL) && (dmg < 0)) {
	...
	private void detection(LsimulatorPcInstance pc) {
		...
		LsimulatorWorldTraps.getInstance().onDetection(pc);
	}
	...
	// 対象がPC、サモン、ペットかを返す
	private boolean isPcSummonPet(LsimulatorCharacter cha) {
	...
	// ターゲットに対して必ず失敗になるか返す
	private boolean isTargetFailure(LsimulatorCharacter cha) {
		boolean isTU = false;
	...
	// 魔法屏障發動判斷
	private boolean isUseCounterMagic(LsimulatorCharacter cha) {
