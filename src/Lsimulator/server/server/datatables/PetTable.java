...
public class PetTable {
	...
	private final Map<Integer, LsimulatorPet> _pets = Maps.newMap();
	...
	private void load() {
		...
		try {
			...
			while (rs.next()) {
				LsimulatorPet pet = new LsimulatorPet();
	...
	public void storePet(LsimulatorPet pet) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = LsimulatorDatabaseFactory.getInstance().getConnection();
	...
	/** 更新寵物飽食度 */
	public void storePetFood(LsimulatorPet pet) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = LsimulatorDatabaseFactory.getInstance().getConnection();
	...
	public void buyNewPet(int petNpcId, int objid, int itemobjid, int upLv,
			int lvExp) {
		LsimulatorPetType petType = PetTypeTable.getInstance().get(petNpcId);
		LsimulatorPet l1pet = new LsimulatorPet();
		l1pet.set_itemobjid(itemobjid);
		l1pet.set_objid(objid);
	...
	public LsimulatorPet getTemplate(int itemobjid) {
		return _pets.get(new Integer(itemobjid));
	}

	public LsimulatorPet[] getPetTableList() {
		return _pets.values().toArray(new LsimulatorPet[_pets.size()]);
	}
}
