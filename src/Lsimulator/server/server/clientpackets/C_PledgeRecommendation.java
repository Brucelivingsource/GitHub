...
import Lsimulator.server.server.serverpackets.S_PledgeRecommendation;
	...
	public C_PledgeRecommendation(byte[] decrypt, ClientThread client) throws Exception {
		...
		if(data == 0){ // 登陸推薦血盟
            ...
            if(type == 1){ // 接受玩家加入
				...
				// 聯盟
            	if(pc.getClanRank() < 7){ 
            		joinPc.setClanRank(LsimulatorClan.CLAN_RANK_LEAGUE_PUBLIC);
            		joinPc.sendPackets(new S_PacketBox(S_PacketBox.MSG_RANK_CHANGED, LsimulatorClan.CLAN_RANK_LEAGUE_PUBLIC, joinPc.getName())); // 你的階級變更為
            	} else { // 一般血盟
