...
public class LsimulatorBoardTopic {
	private static Logger _log = Logger.getLogger(LsimulatorBoardTopic.class.getName());
	...
	private LsimulatorBoardTopic(int id, String name, String title, String content) {
		_id = id;
		_name = name;
		_date = today();
		_title = title;
	...
	private LsimulatorBoardTopic(ResultSet rs) throws SQLException {
		_id = rs.getInt("id");
	...
	public synchronized static LsimulatorBoardTopic create(String name, String title,
			String content) {
		Connection con = null;
		...
		try {
			...
			LsimulatorBoardTopic topic = new LsimulatorBoardTopic(id, name, title, content);
	...
	public static LsimulatorBoardTopic findById(int id) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			...
			if (rs.next()) {
				return new LsimulatorBoardTopic(rs);
			}
		} catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		} finally {
	...
	public static List<LsimulatorBoardTopic> index(int limit) {
		return index(0, limit);
	}

	public static List<LsimulatorBoardTopic> index(int id, int limit) {
		List<LsimulatorBoardTopic> result = new ArrayList<LsimulatorBoardTopic>();
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			...
			rs = pstm.executeQuery();
			while (rs.next()) {
				result.add(new LsimulatorBoardTopic(rs));
			}
			return result;
		} catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
