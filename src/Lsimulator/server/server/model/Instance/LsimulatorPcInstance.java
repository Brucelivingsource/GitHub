...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.ENTANGLE;
...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.MASS_SLOW;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.MORTAL_BODY;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.SHAPE_CHANGE;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.SLOW;
...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STRIKER_GALE;
...
import Lsimulator.server.server.PacketOutput;
import Lsimulator.server.server.WarTimeController;
import Lsimulator.server.server.command.executor.LsimulatorHpBar;
...
import Lsimulator.server.server.model.HpRegeneration;
import Lsimulator.server.server.model.HpRegenerationByDoll;
import Lsimulator.server.server.model.ItemMakeByDoll;
...
import Lsimulator.server.server.model.LsimulatorDwarfForElfInventory;
import Lsimulator.server.server.model.LsimulatorDwarfInventory;
import Lsimulator.server.server.model.LsimulatorEquipmentSlot;
import Lsimulator.server.server.model.LsimulatorExcludingList;
import Lsimulator.server.server.model.LsimulatorInventory;
import Lsimulator.server.server.model.LsimulatorKarma;
import Lsimulator.server.server.model.LsimulatorMagic;
import Lsimulator.server.server.model.LsimulatorObject;
import Lsimulator.server.server.model.LsimulatorParty;
import Lsimulator.server.server.model.LsimulatorPartyRefresh;
import Lsimulator.server.server.model.LsimulatorPcDeleteTimer;
import Lsimulator.server.server.model.LsimulatorPcInventory;
import Lsimulator.server.server.model.LsimulatorPinkName;
...
import Lsimulator.server.server.model.MpReductionByAwake;
import Lsimulator.server.server.model.MpRegeneration;
import Lsimulator.server.server.model.MpRegenerationByDoll;
import Lsimulator.server.server.model.classes.LsimulatorClassFeature;
import Lsimulator.server.server.model.gametime.LsimulatorGameTimeCarrier;
import Lsimulator.server.server.model.monitor.LsimulatorPcAutoUpdate;
import Lsimulator.server.server.model.monitor.LsimulatorPcExpMonitor;
import Lsimulator.server.server.model.monitor.LsimulatorPcGhostMonitor;
import Lsimulator.server.server.model.monitor.LsimulatorPcHellMonitor;
import Lsimulator.server.server.model.monitor.LsimulatorPcInvisDelay;
...
import Lsimulator.server.server.serverpackets.S_RedMessage;
...
import Lsimulator.server.server.serverpackets.S_EquipmentSlot;
import Lsimulator.server.server.serverpackets.S_Fight;
...
import Lsimulator.server.server.utils.CalcStat;
...
// LsimulatorCharacter, LsimulatorDropTable, LsimulatorObject, LsimulatorItemInstance,
// LsimulatorWorld
//

public class LsimulatorPcInstance extends LsimulatorCharacter {
	...
	private LsimulatorPartyRefresh _rp;
	...
	public void startItemMakeByDoll() {
		...
		if (LsimulatorMagicDoll.isItemMake(this)) {
	...
	public void startHpRegenerationByDoll() {
		...
		if (LsimulatorMagicDoll.isHpRegeneration(this)) {
			isExistHprDoll = true;
		}
	...
	public void startMpRegenerationByDoll() {
		...
		if (LsimulatorMagicDoll.isMpRegeneration(this)) {
			isExistMprDoll = true;
		}
	...
	public void startObjectAutoUpdate() {
		...
		_autoUpdateFuture = GeneralThreadPool.getInstance().pcScheduleAtFixedRate(new LsimulatorPcAutoUpdate(getId()), 0L, INTERVAL_AUTO_UPDATE);
	}
	...
	public void onPerceive(LsimulatorPcInstance perceivedFrom) {
		...
		if (isCrown()) { // 君主
			LsimulatorClan clan = LsimulatorWorld.getInstance().getClan(getClanname());
			if (clan != null) {
	...
	private void removeOutOfRangeObjects() {
		for (LsimulatorObject known : getKnownObjects()) {
			if (known == null) {
				continue;
			}
	...
	public void updateObject() {
		...
		if (getMapId() <= 10000) {
			for (LsimulatorObject visible : LsimulatorWorld.getInstance().getVisibleObjects(this, Config.PC_RECOGNIZE_RANGE)) {
				if (!knownsObject(visible)) {
					visible.onPerceive(this);
				}
				else {
					if (visible instanceof LsimulatorNpcInstance) {
						LsimulatorNpcInstance npc = (LsimulatorNpcInstance) visible;
						if (getLocation().isInScreen(npc.getLocation()) && (npc.getHiddenStatus() != 0)) {
							npc.approachPlayer(this);
						}
					}
				}
				if (hasSkillEffect(GMSTATUS_HPBAR) && LsimulatorHpBar.isHpBarTarget(visible)) {
					sendPackets(new S_HPMeter((LsimulatorCharacter) visible));
				}
			}
		} else { // 旅館內判斷
			for (LsimulatorObject visible : LsimulatorWorld.getInstance().getVisiblePlayer(this)) {
				if (!knownsObject(visible)) {
					visible.onPerceive(this);
				}
				if (hasSkillEffect(GMSTATUS_HPBAR) && LsimulatorHpBar.isHpBarTarget(visible)) {
					if (getInnKeyId() == ((LsimulatorCharacter) visible).getInnKeyId()) {
						sendPackets(new S_HPMeter((LsimulatorCharacter) visible));
					}
				}
			}
	...
	public void sendVisualEffectAtLogin() {
		for (LsimulatorCastle ca : CastleTable.getInstance().getCastleTableList()) {
	...
	public LsimulatorPcInstance() {
		_accessLevel = 0;
		_currentWeapon = 0;
		_inventory = new LsimulatorPcInventory(this);
		_dwarf = new LsimulatorDwarfInventory(this);
		_dwarfForElf = new LsimulatorDwarfForElfInventory(this);
		_tradewindow = new LsimulatorInventory();
		_bookmarks = Lists.newList();
		_quest = new LsimulatorQuest(this);
		_equipSlot = new LsimulatorEquipmentSlot(this); // コンストラクタでthisポインタを渡すのは安全だろうか・・・
	}
	...
	@Override
	public LsimulatorPcInventory getInventory() {
		return _inventory;
	}

	public LsimulatorDwarfInventory getDwarfInventory() {
		return _dwarf;
	}

	public LsimulatorDwarfForElfInventory getDwarfForElfInventory() {
		return _dwarfForElf;
	}

	public LsimulatorInventory getTradeWindowInventory() {
		return _tradewindow;
	}
	...
	public void setClassId(int i) {
		_classId = i;
		_classFeature = LsimulatorClassFeature.newClassFeature(i);
	}

	private LsimulatorClassFeature _classFeature = null;

	public LsimulatorClassFeature getClassFeature() {
	...
	public LsimulatorClan getClan() {
		return LsimulatorWorld.getInstance().getClan(getClanname());
	}
	...
	private LsimulatorPcInstance getStat() {
		return null;
	}

	public void reduceCurrentHp(double d, LsimulatorCharacter l1character) {
	 ...
	 *            通知するプレイヤーの配列
	 */
	private void notifyPlayersLogout(List<LsimulatorPcInstance> playersArray) {
		for (LsimulatorPcInstance player : playersArray) {
	...
	public void logout() {
		...
		if (getClanid() != 0) // クラン所属
		{
			LsimulatorClan clan = world.getClan(getClanname());
			if (clan != null) {
	...
	public LsimulatorParty getParty() {
		return _party;
	}

	public void setParty(LsimulatorParty p) {
		_party = p;
	}
	...
	public LsimulatorChatParty getChatParty() {
		return _chatParty;
	}

	public void setChatParty(LsimulatorChatParty cp) {
		_chatParty = cp;
	}
	...
	private List<LsimulatorPrivateShopSellList> _sellList = Lists.newList();

	public List<LsimulatorPrivateShopSellList> getSellList() {
		return _sellList;
	}

	private List<LsimulatorPrivateShopBuyList> _buyList = Lists.newList();

	public List<LsimulatorPrivateShopBuyList> getBuyList() {
		return _buyList;
	}
	...
	@Override
	public void onAction(LsimulatorPcInstance attacker) {
		onAction(attacker, 0);
	}

	@Override
	public void onAction(LsimulatorPcInstance attacker, int skillId) {
		// XXX:NullPointerException回避。onActionの引数の型はLsimulatorCharacterのほうが良い？
		if (attacker == null) {
			return;
		}
		// テレポート処理中
		...
		if ((getCurrentHp() > 0) && !isDead()) {
			...
			LsimulatorAttack attack = new LsimulatorAttack(attacker, this, skillId);
			if (attack.calcHit()) {
				if (hasSkillEffect(COUNTER_BARRIER)) {
					LsimulatorMagic magic = new LsimulatorMagic(this, attacker);
	...
	public boolean checkNonPvP(LsimulatorPcInstance pc, LsimulatorCharacter target) {
		LsimulatorPcInstance targetpc = null;
		if (target instanceof LsimulatorPcInstance) {
			targetpc = (LsimulatorPcInstance) target;
		}
		else if (target instanceof LsimulatorPetInstance) {
			targetpc = (LsimulatorPcInstance) ((LsimulatorPetInstance) target).getMaster();
		}
		else if (target instanceof LsimulatorSummonInstance) {
			targetpc = (LsimulatorPcInstance) ((LsimulatorSummonInstance) target).getMaster();
		}
		if (targetpc == null) {
		...
		if (!Config.ALT_NONPVP) { // Non-PvP設定
			...
			if (target instanceof LsimulatorPcInstance) {
				LsimulatorPcInstance targetPc = (LsimulatorPcInstance) target;
	...
	private boolean isInWarAreaAndWarTime(LsimulatorPcInstance pc, LsimulatorPcInstance target) {
		// pcとtargetが戦争中に戦争エリアに居るか
		int castleId = LsimulatorCastleLocation.getCastleIdByArea(pc);
		int targetCastleId = LsimulatorCastleLocation.getCastleIdByArea(target);
	...
	public void setPetTarget(LsimulatorCharacter target) {
		Object[] petList = getPetList().values().toArray();
		for (Object pet : petList) {
			if (pet instanceof LsimulatorPetInstance) {
				LsimulatorPetInstance pets = (LsimulatorPetInstance) pet;
				pets.setMasterTarget(target);
			}
			else if (pet instanceof LsimulatorSummonInstance) {
				LsimulatorSummonInstance summon = (LsimulatorSummonInstance) pet;
	...
	public void receiveDamage(LsimulatorCharacter attacker, int damage, int attr) {
		int player_mr = getMr();
	...
	public void receiveManaDamage(LsimulatorCharacter attacker, int mpDamage) { // 攻撃でＭＰを減らすときはここを使用
		if ((mpDamage > 0) && !isDead()) {
			...
			if (attacker instanceof LsimulatorPcInstance) {
				LsimulatorPinkName.onAction(this, attacker);
			}
			if ((attacker instanceof LsimulatorPcInstance) && ((LsimulatorPcInstance) attacker).isPinkName()) {
				// ガードが画面内にいれば、攻撃者をガードのターゲットに設定する
				for (LsimulatorObject object : LsimulatorWorld.getInstance().getVisibleObjects(attacker)) {
					if (object instanceof LsimulatorGuardInstance) {
						LsimulatorGuardInstance guard = (LsimulatorGuardInstance) object;
						guard.setTarget(((LsimulatorPcInstance) attacker));
					}
				}
			}
	...
	public void receiveDamage(LsimulatorCharacter attacker, double damage, boolean isMagicDamage) { // 攻撃でＨＰを減らすときはここを使用
		if ((getCurrentHp() > 0) && !isDead()) {
			if (attacker != this) {
				if (!(attacker instanceof LsimulatorEffectInstance) && !knownsObject(attacker) && (attacker.getMapId() == getMapId())) {
			...
			if (hasSkillEffect(MORTAL_BODY) && (getId() != attacker.getId())) {
				...
				if ((damage > 0) && (rnd <= 10)) {
					if (attacker instanceof LsimulatorPcInstance) {
						LsimulatorPcInstance attackPc = (LsimulatorPcInstance) attacker;
					...
					else if (attacker instanceof LsimulatorNpcInstance) {
						LsimulatorNpcInstance attackNpc = (LsimulatorNpcInstance) attacker;
	...
	public void death(LsimulatorCharacter lastAttacker) {
		synchronized (this) {
			if (isDead()) {
				return;
			}
	...
	private class Death implements Runnable {
		...
		public void run() {
			...
			l1skilluse.handleCommands(LsimulatorPcInstance.this, CANCELLATION, getId(), getX(), getY(), null, 0, LsimulatorSkillUse.TYPE_LOGIN);
			...
			if (lastAttacker != LsimulatorPcInstance.this) {
				...
				if (getZoneType() != 0) {
					...
					if (lastAttacker instanceof LsimulatorPcInstance) {
						...
						player = (LsimulatorPcInstance) ((LsimulatorPetInstance) lastAttacker).getMaster();
					}
					else if (lastAttacker instanceof LsimulatorSummonInstance) {
						player = (LsimulatorPcInstance) ((LsimulatorSummonInstance) lastAttacker).getMaster();
					}
					if (player != null) {
						// 戦争中に戦争エリアに居る場合は例外
						if (!isInWarAreaAndWarTime(LsimulatorPcInstance.this, player)) {
							return;
						}
					}
				}
			...
			// 決闘中ならペナルティなし
			LsimulatorPcInstance fightPc = null;
			if (lastAttacker instanceof LsimulatorPcInstance) {
				fightPc = (LsimulatorPcInstance) lastAttacker;
			}

			// 判斷是否屬於新手保護階段, 並且是被其他玩家所殺死
			boolean isNovice = false;
			if (hasSkillEffect(LsimulatorSkillId.STATUS_NOVICE) && (fightPc != null)) {
			...
			if (lastAttacker instanceof LsimulatorGuardInstance) {
				if (get_PKcount() > 0) {
					set_PKcount(get_PKcount() - 1);
				}
				setLastPk(null);
			}
			if (lastAttacker instanceof LsimulatorGuardianInstance) {
			...
			_pcDeleteTimer = new LsimulatorPcDeleteTimer(LsimulatorPcInstance.this);
	...
	private void caoPenaltyResult(int count) {
		for (int i = 0; i < count; i++) {
			LsimulatorItemInstance item = getInventory().CaoPenalty();

			if (item != null) {
				getInventory().tradeItem(item, item.isStackable() ? item.getCount() : 1,
						LsimulatorWorld.getInstance().getInventory(getX(), getY(), getMapId()));
	...
	public boolean castleWarResult() {
		...
		castleId = LsimulatorCastleLocation.getCastleIdByArea(this);
		if (castleId != 0) { // 旗内に居る
	...
	public boolean simWarResult(LsimulatorCharacter lastAttacker) {
		...
		LsimulatorPcInstance attacker = null;
		String enemyClanName = null;
		boolean sameWar = false;

		if (lastAttacker instanceof LsimulatorPcInstance) {
			attacker = (LsimulatorPcInstance) lastAttacker;
		}
		else if (lastAttacker instanceof LsimulatorPetInstance) {
			attacker = (LsimulatorPcInstance) ((LsimulatorPetInstance) lastAttacker).getMaster();
		}
		else if (lastAttacker instanceof LsimulatorSummonInstance) {
			attacker = (LsimulatorPcInstance) ((LsimulatorSummonInstance) lastAttacker).getMaster();
		}
		else {
			return false;
		}
	...
	public LsimulatorBookMark getBookMark(String name) {
		for (int i = 0; i < _bookmarks.size(); i++) {
			LsimulatorBookMark element = _bookmarks.get(i);
	...
	public LsimulatorBookMark getBookMark(int id) {
		for (int i = 0; i < _bookmarks.size(); i++) {
			LsimulatorBookMark element = _bookmarks.get(i);
			if (element.getId() == id) {
				return element;
	...
	public void addBookMark(LsimulatorBookMark book) {
		_bookmarks.add(book);
	}

	public void removeBookMark(LsimulatorBookMark book) {
	...
	public LsimulatorQuest getQuest() {
		return _quest;
	}
	...
	private static Logger _log = Logger.getLogger(LsimulatorPcInstance.class.getName());
	private ClientThread _netConnection;
	private int _classId;
	private int _type;
	private int _exp;
	private final LsimulatorKarma _karma = new LsimulatorKarma();
	private boolean _gm;
	private boolean _monitor;
	private boolean _gmInvis;
	private short _accessLevel;
	private int _currentWeapon;
	private final LsimulatorPcInventory _inventory;
	private final LsimulatorDwarfInventory _dwarf;
	private final LsimulatorDwarfForElfInventory _dwarfForElf;
	private final LsimulatorInventory _tradewindow;
	private LsimulatorItemInstance _weapon;
	private LsimulatorParty _party;
	private LsimulatorChatParty _chatParty;
	...
	private final List<LsimulatorBookMark> _bookmarks;
	private LsimulatorQuest _quest;
	...
	private LsimulatorEquipmentSlot _equipSlot;
	private LsimulatorPcDeleteTimer _pcDeleteTimer;
	...
	public LsimulatorEquipmentSlot getEquipSlot() {
		return _equipSlot;
	}
	...
	public static LsimulatorPcInstance load(String charName) {
		LsimulatorPcInstance result = null;
		try {
	...
	public double getMaxWeight() {
		...
		weightReductionByDoll += LsimulatorMagicDoll.getWeightReductionByDoll(this);
	...
	public void beginInvisTimer() {
		...
		GeneralThreadPool.getInstance().pcSchedule(new LsimulatorPcInvisDelay(getId()), DELAY_INVIS);
	}
	...
	public void beginExpMonitor() {
		_expMonitorFuture = GeneralThreadPool.getInstance().pcScheduleAtFixedRate(new LsimulatorPcExpMonitor(getId()), 0L, INTERVAL_EXP_MONITOR);
	}
	...
	private void levelUp(int gap) {
		...
		if ((getLevel() == 99) && Config.ALT_REVIVAL_POTION) {
			try {
				LsimulatorItem l1item = ItemTable.getInstance().getTemplate(43000);
				if (l1item != null) {
		...
		if ((getMapId() == 2005 || getMapId() == 86)) { // 新手村
			if (getLevel() >= 13) { // 等級大於13
				if (getQuest().get_step(LsimulatorQuest.QUEST_TUTOR) != 255) {
					getQuest().set_step(LsimulatorQuest.QUEST_TUTOR, 255);
				}
				LsimulatorTeleport.teleport(this, 33084, 33391, (short) 4, 5, true);// 銀騎士村
			}
		} else if (getLevel() >= 52) { // 指定レベル
			if (getMapId() == 777) { // 見捨てられた者たちの地(影の神殿)
				LsimulatorTeleport.teleport(this, 34043, 32184, (short) 4, 5, true); // 象牙の塔前
			} else if ((getMapId() == 778) || (getMapId() == 779)) { // 見捨てられた者たちの地(欲望の洞窟)
				LsimulatorTeleport.teleport(this, 32608, 33178, (short) 4, 5, true); // WB
			}
		}
	...
	public void beginGameTimeCarrier() {
		new LsimulatorGameTimeCarrier(this).start();
	}
	...
	public void beginGhost(int locx, int locy, short mapid, boolean canTalk, int sec) {
		...
		LsimulatorTeleport.teleport(this, locx, locy, mapid, 5, true);
		if (sec > 0) {
			_ghostFuture = GeneralThreadPool.getInstance().pcSchedule(new LsimulatorPcGhostMonitor(getId()), sec * 1000);
		}
	}

	public void makeReadyEndGhost() {
		setReserveGhost(true);
		LsimulatorTeleport.teleport(this, _ghostSaveLocX, _ghostSaveLocY, _ghostSaveMapId, _ghostSaveHeading, true);
	}
	...
	public void beginHell(boolean isFirst) {
		...
		if (getMapId() != 666) {
			...
			int locy = 32777;
			short mapid = 666;
			LsimulatorTeleport.teleport(this, locx, locy, mapid, 5, false);
		}
		...
		if (_hellFuture == null) {
			_hellFuture = GeneralThreadPool.getInstance().pcScheduleAtFixedRate(new LsimulatorPcHellMonitor(getId()), 0L, 1000L);
		}
	}
	...
	public void endHell() {
		...
		// 地獄から脱出したら火田村へ帰還させる。
		int[] loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_ORCISH_FOREST);
		LsimulatorTeleport.teleport(this, loc[0], loc[1], (short) loc[2], 5, true);
		try {
			save();
		}
	...
	public void startRefreshParty() {// 組隊更新 3.3C
		...
		if (!_rpActive) {
			...
			_rp = new LsimulatorPartyRefresh(this);
	...
	private final LsimulatorExcludingList _excludingList = new LsimulatorExcludingList();

	public LsimulatorExcludingList getExcludingList() {
	...
	public void changeFightType(int oldType, int newType) {
		...
		switch (oldType) {
			case 1:
				addAc(2);
				addMr(-3);
				sendPackets(new S_Fight(S_Fight.TYPE_JUSTICE_LEVELsimulator, S_Fight.FLAG_OFF));
				break;
			...
			case -1:
				...
				addBowDmgup(-1);
				addSp(-1);
				sendPackets(new S_Fight(S_Fight.TYPE_EVIL_LEVELsimulator, S_Fight.FLAG_OFF));
				break;
		...
		switch (newType) {
			case 1:
				addAc(-2);
				addMr(3);
				sendPackets(new S_Fight(S_Fight.TYPE_JUSTICE_LEVELsimulator, S_Fight.FLAG_ON));
				break;
			...
			case -1:
				...
				addBowDmgup(1);
				addSp(1);
				sendPackets(new S_Fight(S_Fight.TYPE_EVIL_LEVELsimulator, S_Fight.FLAG_ON));
				break;
	...
	public void checkNoviceType() {
		...
		if (getLevel() > Config.NOVICE_MAX_LEVEL) {
			// 判斷之前是否具有新手保護狀態
			if (hasSkillEffect(LsimulatorSkillId.STATUS_NOVICE)) {
				// 移除新手保護狀態
				removeSkillEffect(LsimulatorSkillId.STATUS_NOVICE);
			...
			if (!hasSkillEffect(LsimulatorSkillId.STATUS_NOVICE)) {
				// 增加新手保護狀態
				setSkillEffect(LsimulatorSkillId.STATUS_NOVICE, 0);
	...
	public void setEquipments(){
		...
		for (LsimulatorItemInstance item : getInventory().getItems()) {
			...
			} else if (item.isEquipped() && item.getItem().getType2() == 2) { // LsimulatorArmor
