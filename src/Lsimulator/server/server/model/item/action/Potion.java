...
import Lsimulator.server.server.serverpackets.S_SkillIconBlessOfEva;
import Lsimulator.server.server.serverpackets.S_SkillIconGFX;
import Lsimulator.server.server.serverpackets.S_SkillIconWisdomPotion;
...
public class Potion {
	...
	public static void Brave(LsimulatorPcInstance pc, LsimulatorItemInstance item, int item_id) {
		...
		// 判斷持續時間 && 使用類型
		/* 勇敢藥水類 */
		if (item_id == LsimulatorItemId.POTION_OF_EMOTION_BRAVERY
				|| item_id == LsimulatorItemId.B_POTION_OF_EMOTION_BRAVERY
				|| item_id == LsimulatorItemId.POTION_OF_REINFORCED_CASE
				|| item_id == LsimulatorItemId.W_POTION_OF_EMOTION_BRAVERY
				|| item_id == LsimulatorItemId.DEVILS_BLOOD
				|| item_id == LsimulatorItemId.COIN_OF_REPUTATION) {
			if (item_id == LsimulatorItemId.POTION_OF_EMOTION_BRAVERY) { // 勇敢藥水
				time = 300;
			} else if (item_id == LsimulatorItemId.B_POTION_OF_EMOTION_BRAVERY) { // 受祝福的勇敢藥水
				time = 350;
			} else if (item_id == LsimulatorItemId.POTION_OF_REINFORCED_CASE) { // 強化勇氣的藥水
				time = 1800;
			} else if (item_id == LsimulatorItemId.DEVILS_BLOOD) { // 惡魔之血
				time = 600;
			} else if (item_id == LsimulatorItemId.COIN_OF_REPUTATION) { // 名譽貨幣
				time = 600;
			} else if (item_id == LsimulatorItemId.W_POTION_OF_EMOTION_BRAVERY) { // 福利勇敢藥水
				time = 1200;
			}
			buff_brave(pc, STATUS_BRAVE, (byte) 1, time); // 給予勇敢藥水效果
			pc.getInventory().removeItem(item, 1);
			/* 精靈餅乾 & 祝福的精靈餅乾 */
		} else if (item_id == LsimulatorItemId.ELVEN_WAFER
				|| item_id == LsimulatorItemId.B_ELVEN_WAFER
				|| item_id == LsimulatorItemId.W_POTION_OF_FOREST) {
			if (item_id == LsimulatorItemId.ELVEN_WAFER) { // 精靈餅乾
				time = 480;
			} else if (item_id == LsimulatorItemId.B_ELVEN_WAFER) { // 祝福的精靈餅乾
				time = 700;
			} else if (item_id == LsimulatorItemId.W_POTION_OF_FOREST) { // 福利森林藥水
				time = 1920;
			}
			buff_brave(pc, STATUS_ELFBRAVE, (byte) 3, time); // 給予精靈餅乾效果
			pc.getInventory().removeItem(item, 1);
			/* 生命之樹果實 */
		} else if (item_id == LsimulatorItemId.FORBIDDEN_FRUIT) { // 生命之樹果實
			time = 480;
	...
	public static void useGreenPotion(LsimulatorPcInstance pc, LsimulatorItemInstance item, int itemId) {
		...
		int time = 0;
		if (itemId == LsimulatorItemId.POTION_OF_HASTE_SELF || itemId == 40030) { // 自我加速藥水、象牙塔加速藥水
			time = 300;
		} else if (itemId == LsimulatorItemId.B_POTION_OF_HASTE_SELF) { // 受祝福的 自我加速藥水
			time = 350;
	...
	public static void useWisdomPotion(LsimulatorPcInstance pc, LsimulatorItemInstance item, int item_id) {
		...
		int time = 0;
		if (item_id == LsimulatorItemId.POTION_OF_EMOTION_WISDOM) { // 慎重藥水
			time = 300;
		} else if (item_id == LsimulatorItemId.B_POTION_OF_EMOTION_WISDOM) { // 受祝福的 慎重藥水
			time = 360;
