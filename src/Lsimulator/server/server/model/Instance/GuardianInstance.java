...
public class GuardianInstance extends NpcInstance {
	...
	private static Logger _log = Logger.getLogger(GuardianInstance.class
			.getName());

	private GuardianInstance _npc = this;
	 ...
	 * @param template
	 */
	public GuardianInstance(LsimulatorNpc template) {
		super(template);
		if (!isDropitems()) {
			doGDropItem(0);
		}
	...
	class Death implements Runnable {
		...
		public void run() {
			...
			if (lastAttacker instanceof PcInstance) {
				...
				player = (PcInstance) ((PetInstance) lastAttacker)
						.getMaster();
			} else if (lastAttacker instanceof SummonInstance) {
				player = (PcInstance) ((SummonInstance) lastAttacker)
						.getMaster();
			}
			if (player != null) {
