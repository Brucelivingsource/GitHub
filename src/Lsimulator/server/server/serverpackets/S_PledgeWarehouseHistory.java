...
import static Lsimulator.server.server.Opcodes.S_OPCODE_PACKETBOX;
...
public class S_PledgeWarehouseHistory extends ServerBasePacket {
	...
	public S_PledgeWarehouseHistory(int clanId){
		...
		try{
            ...
            while (rs.next()) {
            	LsimulatorItem item = ItemTable.getInstance().getTemplate(ItemTable.getInstance().findItemIdByName(rs.getString("item_name")));
