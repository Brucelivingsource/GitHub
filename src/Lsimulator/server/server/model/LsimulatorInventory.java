...
public class LsimulatorInventory extends LsimulatorObject {
	...
	public synchronized ItemInstance storeItem(int id, int count) {
		...
		LsimulatorItem temp = ItemTable.getInstance().getTemplate(id);
		if (temp == null) {
			return null;
		}
	...
	/**
	 * インベントリから指定されたアイテムIDのアイテムを削除する。LsimulatorItemInstanceへの参照
	...
	public ItemInstance receiveDamage(ItemInstance item, int count) {
		...
		updateItem(item, LsimulatorPcInventory.COL_DURABILITY);
		return item;
	}
