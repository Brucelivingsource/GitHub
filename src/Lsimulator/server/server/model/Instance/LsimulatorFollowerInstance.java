...
public class LsimulatorFollowerInstance extends LsimulatorNpcInstance {
	...
	public LsimulatorFollowerInstance(LsimulatorNpc template, LsimulatorNpcInstance target, LsimulatorCharacter master) {
		super(template);
	...
	private void createNewItem(LsimulatorPcInstance pc, int item_id, int count) {
	...
	public void spawn(int npcId, int X, int Y, int H, short Map) {
		...
		if (l1npc != null) {
			...
			try {
				...
				LsimulatorQuestInstance newnpc = (LsimulatorQuestInstance) object;
				newnpc.onNpcAI();
