...
public class SpawnTable {
	...
	private void fillSpawnTable() {
		...
		try {
			...
			LsimulatorSpawn spawnDat;
			LsimulatorNpc template1;
			while (rs.next()) {
				...
				if (template1 == null) {
					...
					spawnDat = new LsimulatorSpawn(template1);
	...
	public LsimulatorSpawn getTemplate(int Id) {
		return _spawntable.get(new Integer(Id));
	}

	public void addNewSpawn(LsimulatorSpawn spawn) {
		_highestId++;
	...
	private static int calcCount(LsimulatorNpc npc, int count, double rate) {
		if (rate == 0) {
			return 0;
		}
