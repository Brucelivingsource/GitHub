...
public class BuddyTable {
	...
	private final Map<Integer, LsimulatorBuddy> _buddys = Maps.newMap();
		...
		try {
			...
			while (charIdRS.next()) {
				...
				try {
					...
					LsimulatorBuddy buddy = new LsimulatorBuddy(charId);
	...
	public LsimulatorBuddy getBuddyTable(int charId) {
		LsimulatorBuddy buddy = _buddys.get(charId);
		if (buddy == null) {
			buddy = new LsimulatorBuddy(charId);
	...
	public void removeBuddy(int charId, String buddyName) {
		Connection con = null;
		PreparedStatement pstm = null;
		LsimulatorBuddy buddy = getBuddyTable(charId);
