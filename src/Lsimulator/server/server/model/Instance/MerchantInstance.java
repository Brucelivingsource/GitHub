...
public class MerchantInstance extends NpcInstance {
	 ...
	 * @param template
	 */
	public MerchantInstance(LsimulatorNpc template) {
		super(template);
	}
		...
		if (talking != null) {
			if (npcid == 70841) { // ルーディエル
				...
				if (playerLv < 2 && quest_step == 0) {
					...
					ItemInstance item = player.getInventory().storeItem(
	...
	private static String talkToTownadviser(PcInstance pc, int town_id) {
		String htmlid;
	...
	private static String talkToTownmaster(PcInstance pc, int town_id) {
		String htmlid;
		if (pc.getHomeTownId() == town_id) {
			htmlid = "hometown";
		} else {
	...
	public void doFinalAction(PcInstance player) {
	}

	private boolean checkHasCastle(PcInstance player, int castle_id) {
	...
	private boolean checkClanLeader(PcInstance player) {
		if (player.isCrown()) { // 君主
			LsimulatorClan clan = LsimulatorWorld.getInstance().getClan(player.getClanname());
			if (clan != null) {
	...
	private int getNecessarySealCount(PcInstance pc) {
		int rulerCount = 0;
	...
	private void createRuler(PcInstance pc, int attr, int sealCount) {
		...
		ItemInstance item = pc.getInventory().storeItem(rulerId, 1);
		if (item != null) {
	...
	private String talkToDoromond(PcInstance pc) {
		String htmlid = "";
	...
	private String talkToAlex(PcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 3) {
			htmlid = "jpe0021";
	...
	private String talkToAlexInTrainingRoom(PcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 3) {
			htmlid = "jpe0031";
		} else {
	...
	private String cancellation(PcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 13) {
			htmlid = "jpe0161";
		} else {
	...
	private String talkToRuba(PcInstance pc) {
		String htmlid = "";
	...
	private String talkToSIGuide(PcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 3) {
			htmlid = "en0301";
	...
	private String talkToPopirea(PcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 25) {
			htmlid = "jpe0041";
	...
	private String talkToSecondtbox(PcInstance pc) {
		String htmlid = "";
	...
	private String talkToThirdtbox(PcInstance pc) {
		String htmlid = "";
	...
	private void newUserHelp(PcInstance pc, int helpNo) {
		switch (helpNo) {
