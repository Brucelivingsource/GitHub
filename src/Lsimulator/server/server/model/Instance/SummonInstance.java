...
public class SummonInstance extends NpcInstance {
	...
	// サモンモンスター用
	public SummonInstance(LsimulatorNpc template, LsimulatorCharacter master) {
		super(template);
	...
	// 造屍術處理
	public SummonInstance(NpcInstance target, LsimulatorCharacter master, boolean isCreateZombie) {
		super(null);
		setId(IdFactory.getInstance().nextId());

		if (isCreateZombie) { // クリエイトゾンビ
			int npcId = 45065;
			PcInstance pc = (PcInstance) master;
			...
			LsimulatorNpc template = NpcTable.getInstance().getTemplate(npcId).clone();
		...
		if ((target instanceof MonsterInstance) && !((MonsterInstance) target).is_storeDroped()) {
		...
		// ペットが攻撃中だった場合止めさせる
		for (NpcInstance each : master.getPetList().values()) {
	...
	public synchronized void Death(LsimulatorCharacter lastAttacker) {
		if (!isDead()) {
			...
			// 死亡時物品給予主人或掉落地面
			LsimulatorInventory targetInventory = _master.getInventory();
			List<ItemInstance> items = _inventory.getItems();
			for (ItemInstance item : items) {
				if (_master.getInventory().checkAddItem( // 容量重量確認及びメッセージ送信
					...
					// \f1%0が%1をくれました。
					((PcInstance) _master).sendPackets(new S_ServerMessage(
							143, getName(), item.getLogName()));
				} else { // 持てないので足元に落とす
					targetInventory = LsimulatorWorld.getInstance().getInventory(
							getX(), getY(), getMapId());
	...
	public synchronized void returnToNature() {
		...
		if (!_tamed) {
			...
			for (ItemInstance item : items) {
				if (_master.getInventory().checkAddItem( // 容量重量確認及びメッセージ送信
						item, item.getCount()) == LsimulatorInventory.OK) {_inventory.tradeItem(item, item.getCount(), targetInventory);
					// \f1%0が%1をくれました。
					((PcInstance) _master).sendPackets(new S_ServerMessage(
							143, getName(), item.getLogName()));
				} else { // 持てないので足元に落とす
	...
	public void liberate() {
		MonsterInstance monster = new MonsterInstance(getNpcTemplate());
	...
	public void onAction(PcInstance attacker, int skillId) {
		...
		PcInstance master = (PcInstance) cha;
	...
	public void onFinalAction(PcInstance player, String action) {
		...
		if (status == 6) {
			PcInstance petMaster = (PcInstance) _master;
			if (_tamed) {
				// テイミングモンスター、クリエイトゾンビの解放
				liberate();
			} else {
			...
			for (Object petObject : petList) {
				if (petObject instanceof SummonInstance) {
					SummonInstance summon = (SummonInstance) petObject;
					petMaster.sendPackets(new S_SummonPack(summon, petMaster));
					return;
				} else if (petObject instanceof PetInstance) {
			...
			for (Object petObject : petList) {
				...
				} else if (petObject instanceof PetInstance) { // ペット
					PetInstance pet = (PetInstance) petObject;
					if ((player != null)
	...
	public void setCurrentHp(int i) {
		...
		if (_master instanceof PcInstance) {
			...
			PcInstance Master = (PcInstance) _master;
	...
	public boolean isExsistMaster() {
		...
		if (getMaster() != null) {
			...
			if (LsimulatorWorld.getInstance().getPlayer(masterName) == null) {
