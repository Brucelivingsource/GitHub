...
import Lsimulator.server.server.serverpackets.S_SkillIconBloodstain;
...
public interface LsimulatorSkillTimer {
...
class LsimulatorSkillStop {
	public static void stopSkill(LsimulatorCharacter cha, int skillId) {
		...
		// ****** 状態変化が解けた場合
			...
			LsimulatorSkills _skill = SkillsTable.getInstance().getTemplate(THUNDER_GRAB); // 奪命之雷
			int _fetterDuration = _skill.getBuffDuration() * 1000;
			cha.setSkillEffect(STATUS_FREEZE, _fetterDuration);
			LsimulatorEffectSpawn.getInstance().spawnEffect(81182, _fetterDuration, cha.getX(), cha.getY(), cha.getMapId());
		}
...
class LsimulatorSkillTimerThreadImpl extends Thread implements LsimulatorSkillTimer {
	public LsimulatorSkillTimerThreadImpl(LsimulatorCharacter cha, int skillId, int timeMillis) {
		_cha = cha;
		_skillId = skillId;
	...
	public void end() {
		super.interrupt();
		LsimulatorSkillStop.stopSkill(_cha, _skillId);
	}
...
class LsimulatorSkillTimerTimerImpl implements LsimulatorSkillTimer, Runnable {
	private static Logger _log = Logger.getLogger(LsimulatorSkillTimerTimerImpl.class.getName());

	private ScheduledFuture<?> _future = null;

	public LsimulatorSkillTimerTimerImpl(LsimulatorCharacter cha, int skillId, int timeMillis) {
		_cha = cha;
		_skillId = skillId;
		_timeMillis = timeMillis;
