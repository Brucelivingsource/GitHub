...
/** 安塔瑞斯、法利昂副本 */
public class LsimulatorDragonSlayer {
	private static Logger _log = Logger.getLogger(LsimulatorDragonSlayer.class.getName());

	private static LsimulatorDragonSlayer _instance;
	...
	public static LsimulatorDragonSlayer getInstance() {
		if (_instance == null) {
			_instance = new LsimulatorDragonSlayer();
		}
		return _instance;
	}
	...
	public void resetDragonSlayer(int portalNumber) {
		...
		for (Object obj : LsimulatorWorld.getInstance().getVisibleObjects(mapId).values()) {
			...
			if (obj instanceof LsimulatorPcInstance) {
				...
				if (pc != null) {
					if (pc.isDead()) {
						...
						LsimulatorTeleport.teleport(pc, 33710, 32521, (short) 4, pc.getHeading(), true);
					}
				}
			}
			// 門關閉
			...
			// 刪除副本內的物品
			else if (obj instanceof LsimulatorInventory) {
