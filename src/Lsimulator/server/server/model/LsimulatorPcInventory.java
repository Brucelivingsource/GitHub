...
import Lsimulator.server.server.serverpackets.S_DeleteInventoryItem;
import Lsimulator.server.server.serverpackets.S_EquipmentSlot;
import Lsimulator.server.server.serverpackets.S_ItemColor;
...
import Lsimulator.server.server.serverpackets.S_ItemAmount;
...
public class LsimulatorPcInventory extends LsimulatorInventory {
		...
		try {
			...
			for (LsimulatorItemInstance item : storage.loadItems(_owner.getId())) {
				...
				if (item.getItemId() == 40309) {
					LsimulatorRaceTicket ticket = RaceTicketTable.getInstance()
							.getTemplate(item.getId());
					if (ticket != null) {
						LsimulatorItem temp = (LsimulatorItem) item.getItem().clone();
	...
	private void takeoffWeapon(int polyid) {
		...
		// 装備出来ない武器を装備してるか？
		takeoff = !LsimulatorPolyMorph.isEquipableWeapon(polyid, weapon_type);
	...
	private void takeoffArmor(int polyid) {
		...
		for (int type = 0; type <= 13; type++) {
			...
			if (getTypeEquipped(2, type) != 0
					&& !LsimulatorPolyMorph.isEquipableArmor(polyid, type)) {
