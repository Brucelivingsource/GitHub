...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.HOLY_WEAPON;
...
public class LsimulatorMerchantInstance extends LsimulatorNpcInstance {
	/**
	 * 
	 */
	 ...
	 * @param template
	 */
	public LsimulatorMerchantInstance(LsimulatorNpc template) {
		super(template);
	}
			...
			if (npcid == 70841) { // ルーディエル
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 15) {
						int lv15_step = quest.get_step(LsimulatorQuest.QUEST_LEVELsimulator5);
						if ((lv15_step == 2)
								|| (lv15_step == LsimulatorQuest.QUEST_END)) { // クリア済み
							htmlid = "gunterp11";
						} else {
							htmlid = "gunterp9";
						}
					...
					int lv30_step = quest.get_step(LsimulatorQuest.QUEST_LEVEL30);
					if (lv30_step == 0) { // 未開始
						htmlid = "gunterk9";
					} else if (lv30_step == 1) {
						htmlid = "gunterkE1";
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 45) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVEL30)) { // lv30クリア済み
							int lv45_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL45);
							if (lv45_step == LsimulatorQuest.QUEST_END) { // クリア済み
								htmlid = "masha4";
					...
					if (player.getLevel() >= 45) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVEL30)) { // Lv30クエスト終了済み
							int lv45_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL45);
							if (lv45_step == LsimulatorQuest.QUEST_END) { // クリア済み
								htmlid = "mashak3";
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 15) {
						...
						if (lv15_step == 1) { // ゼロクリア済み
							htmlid = "zero5";
						} else if (lv15_step == LsimulatorQuest.QUEST_END) { // ゼロ、グンタークリア済み
							htmlid = "zero1";// 6
						} else {
							htmlid = "zero1";
						}
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 30) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVELsimulator5)) { // lv15試練クリア済み
							int lv30_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL30);
							if (lv30_step == LsimulatorQuest.QUEST_END) { // クリア済み
								htmlid = "aria3";
				...
				if (player.isCrown()) { // 君主
					int lv45_step = quest.get_step(LsimulatorQuest.QUEST_LEVEL45);
					if ((lv45_step >= 1) && (lv45_step != LsimulatorQuest.QUEST_END)) { // 開始かつ未終了
				...
				// if (player.isCrown()) { // 君主
				// int lv45_step = quest.get_step(LsimulatorQuest.QUEST_LEVEL45);
				// if (lv45_step >= 6 && lv45_step == LsimulatorQuest.QUEST_END ) {
				// //メグクリア済みor終了
				// htmlid = "pieta9";
				...
				if (player.isKnight()) { // ナイト
					if (player.getLevel() >= 15) {
						int lv15_step = quest.get_step(LsimulatorQuest.QUEST_LEVELsimulator5);
						if (lv15_step == LsimulatorQuest.QUEST_END) { // アノンクリア済み
							htmlid = "aanon7";
				...
				if (player.isKnight()) { // ナイト
					if (player.getLevel() >= 30) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVELsimulator5)) { // LV15クエスト終了済み
							int lv30_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL30);
							if (lv30_step == 0) { // 未開始
								htmlid = "mark1";
				...
				if (player.isCrown()) { // 君主
					...
					int lv30_step = quest.get_step(LsimulatorQuest.QUEST_LEVEL30);
					if (lv30_step == LsimulatorQuest.QUEST_END) { // ゲラド終了済み
						htmlid = "gerardkEcg";
				...
				if (player.getTempCharGfx() == 2374) { // スケルトン変身
					if (player.isKnight()) { // ナイト
						if (quest.get_step(LsimulatorQuest.QUEST_LEVEL30) == 6) { // 復活のポーション同意済み
							htmlid = "jim2";
						} else {
							htmlid = "jim4";
						}
				...
				if (player.isElf()) { // エルフ
					if (player.getLevel() >= 15) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVELsimulator5)) {
							htmlid = "oth5";
						} else {
							htmlid = "oth1";
						}
				...
				if (player.isElf()) { // エルフ
					if (player.getLevel() >= 30) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVELsimulator5)) { // Lv15終了済み
							int lv30_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL30);
							if (lv30_step == LsimulatorQuest.QUEST_END) { // 終了済み
								htmlid = "motherEE3";
							} else if (lv30_step >= 1) { // 同意済み
								htmlid = "motherEE2";
				...
				if (player.isWizard()) { // ウィザード
					if (player.getLevel() >= 15) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVELsimulator5)) { // 終了済み
							htmlid = "jem6";
						} else {
							htmlid = "jem1";
						}
				...
				if (player.isWizard()) { // ウィザード
					int lv30_step = quest.get_step(LsimulatorQuest.QUEST_LEVEL30);
					if (lv30_step == LsimulatorQuest.QUEST_END) {
						if (player.getLevel() >= 45) {
							int lv45_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL45);
							if ((lv45_step >= 1)
									&& (lv45_step != LsimulatorQuest.QUEST_END)) { // 同意済み
								htmlid = "talassmq2";
				...
				if ((player.getLevel() >= 50) && (lv45_step == LsimulatorQuest.QUEST_END)) {
					if (player.isCrown()) {
						if (lv50_step == 0) {
							htmlid = "dicardingp1";
						} else if (lv50_step == LsimulatorQuest.QUEST_END) {
							htmlid = "dicardingp15";
						} else if (lv50_step >= 5) {
				...
				if (player.isDarkelf()) { // ダークエルフ
					if (player.getLevel() >= 15) {
						int lv15_step = quest.get_step(LsimulatorQuest.QUEST_LEVELsimulator5);
						if (lv15_step == LsimulatorQuest.QUEST_END) { // 終了済み
							htmlid = "kanguard3";
				...
				if (player.isDarkelf()) { // 黑暗妖精
					if (player.getLevel() >= 45) {
						if (quest.isEnd(LsimulatorQuest.QUEST_LEVEL30)) { // 30級任務結束
							int lv45_step = quest
									.get_step(LsimulatorQuest.QUEST_LEVEL45);
							if (lv45_step == LsimulatorQuest.QUEST_END) { // 45級任務結束
								if (player.getLevel() < 50) { // 未達到 Lv50
									htmlid = "bluedikaq3";
								} else {
									int lv50_step = quest
											.get_step(LsimulatorQuest.QUEST_LEVEL50);
									if (lv50_step == LsimulatorQuest.QUEST_END) { // 50級任務結束
										htmlid = "bluedikaq8";
				...
				if (player.isDarkelf()) {
					if (quest.get_step(LsimulatorQuest.QUEST_LEVEL45) == 1) { // ブルディカ同意済み
						htmlid = "koup12";
					}
				}
				...
				if (player.isDarkelf()) { // 黑暗妖精
					if (player.getLevel() >= 50) {
						int lv50_step = quest
						.get_step(LsimulatorQuest.QUEST_LEVEL50);
						if ((lv50_step == LsimulatorQuest.QUEST_END) || (lv50_step >= 4)) { // 50級任務結束
							htmlid = "kimaq4";
						} else if (lv50_step == 3) {
							htmlid = "kimaq3";
						} else {
							if (lv50_step >= 1) { // 需先找布魯迪卡，才能進行後續任務
								htmlid = "kimaq1";
								quest.set_step(LsimulatorQuest.QUEST_LEVEL50, 2);
							} else {
								htmlid = "kima1";
							}
						}
				...
				if (quest.get_step(LsimulatorQuest.QUEST_LYRA) >= 1) { // 契約済み
					htmlid = "lyraEv3";
				} else { // 未契約
					htmlid = "lyraEv1";
				}
				...
				if (!quest.isEnd(LsimulatorQuest.QUEST_OILSKINMANT)) {
				...
				int time = LsimulatorGameTimeClock.getInstance().currentTime()
						.getSeconds() % 86400;
				...
				htmlid = talkToTownmaster(player,
						LsimulatorTownLocation.TOWNID_TALKING_ISLAND);
			} else if (npcid == 70546) { // ケント村 タウンマスター
				htmlid = talkToTownmaster(player, LsimulatorTownLocation.TOWNID_KENT);
			} else if (npcid == 70567) { // グルーディン村 タウンマスター
				htmlid = talkToTownmaster(player, LsimulatorTownLocation.TOWNID_GLUDIO);
			} else if (npcid == 70815) { // 火田村 タウンマスター
				htmlid = talkToTownmaster(player,
						LsimulatorTownLocation.TOWNID_ORCISH_FOREST);
			} else if (npcid == 70774) { // ウッドベック村 タウンマスター
				htmlid = talkToTownmaster(player,
						LsimulatorTownLocation.TOWNID_WINDAWOOD);
			} else if (npcid == 70799) { // シルバーナイトタウン タウンマスター
				htmlid = talkToTownmaster(player,
						LsimulatorTownLocation.TOWNID_SILVER_KNIGHT_TOWN);
			} else if (npcid == 70594) { // ギラン都市 タウンマスター
				htmlid = talkToTownmaster(player, LsimulatorTownLocation.TOWNID_GIRAN);
			} else if (npcid == 70860) { // ハイネ都市 タウンマスター
				htmlid = talkToTownmaster(player, LsimulatorTownLocation.TOWNID_HEINE);
			} else if (npcid == 70654) { // ウェルダン村 タウンマスター
				htmlid = talkToTownmaster(player, LsimulatorTownLocation.TOWNID_WERLDAN);
			} else if (npcid == 70748) { // 象牙の塔の村 タウンマスター
				htmlid = talkToTownmaster(player, LsimulatorTownLocation.TOWNID_OREN);
			} else if (npcid == 70534) { // 話せる島の村 タウンアドバイザー
				htmlid = talkToTownadviser(player,
						LsimulatorTownLocation.TOWNID_TALKING_ISLAND);
			} else if (npcid == 70556) { // ケント村 タウンアドバイザー
				htmlid = talkToTownadviser(player, LsimulatorTownLocation.TOWNID_KENT);
			} else if (npcid == 70572) { // グルーディン村 タウンアドバイザー
				htmlid = talkToTownadviser(player, LsimulatorTownLocation.TOWNID_GLUDIO);
				...
				htmlid = talkToTownadviser(player, LsimulatorTownLocation.TOWNID_GIRAN);
			} else if (npcid == 70876) { // ハイネ都市 タウンアドバイザー
				htmlid = talkToTownadviser(player, LsimulatorTownLocation.TOWNID_HEINE);
			} else if (npcid == 70663) { // ウェルダン村 タウンアドバイザー
				htmlid = talkToTownadviser(player,
						LsimulatorTownLocation.TOWNID_WERLDAN);
			} else if (npcid == 70761) { // 象牙の塔の村 タウンアドバイザー
				htmlid = talkToTownadviser(player, LsimulatorTownLocation.TOWNID_OREN);
				...
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == 3) {
					htmlid = "lukein13";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == LsimulatorQuest.QUEST_END)
						&& (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == 2)
						&& player.getInventory().checkItem(40631)) {
					htmlid = "lukein10";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == LsimulatorQuest.QUEST_END) {
					htmlid = "lukein0";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 11) {
					if (player.getInventory().checkItem(40716)) {
						htmlid = "lukein9";
					}
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) >= 1)
						&& (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) <= 10)) {
					htmlid = "lukein8";
				}
			} else if (npcid == 71063) { // 小さな箱-１番目（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_TBOX1) == LsimulatorQuest.QUEST_END) {
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 1) {
					htmlid = "maptbox";
				}
			} else if (npcid == 71064) { // 小さな箱-2番目-ｂ地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 2) {
					htmlid = talkToSecondtbox(player);
				}
			} else if (npcid == 71065) { // 小さな箱-2番目-c地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 3) {
					htmlid = talkToSecondtbox(player);
				}
			} else if (npcid == 71066) { // 小さな箱-2番目-d地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 4) {
					htmlid = talkToSecondtbox(player);
				}
			} else if (npcid == 71067) { // 小さな箱-3番目-e地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 5) {
					htmlid = talkToThirdtbox(player);
				}
			} else if (npcid == 71068) { // 小さな箱-3番目-f地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 6) {
					htmlid = talkToThirdtbox(player);
				}
			} else if (npcid == 71069) { // 小さな箱-3番目-g地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 7) {
					htmlid = talkToThirdtbox(player);
				}
			} else if (npcid == 71070) { // 小さな箱-3番目-h地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 8) {
					htmlid = talkToThirdtbox(player);
				}
			} else if (npcid == 71071) { // 小さな箱-3番目-i地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 9) {
					htmlid = talkToThirdtbox(player);
				}
			} else if (npcid == 71072) { // 小さな箱-3番目-j地点（海賊島の秘密）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 10) {
					htmlid = talkToThirdtbox(player);
				}
			} else if (npcid == 71056) { // シミズ（消えた息子）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == 4) {
					if (player.getInventory().checkItem(40631)) {
						htmlid = "SIMIZZ11";
					} else {
						htmlid = "SIMIZZ0";
					}
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_SIMIZZ) == 2) {
					htmlid = "SIMIZZ0";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_SIMIZZ) == LsimulatorQuest.QUEST_END) {
					htmlid = "SIMIZZ15";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_SIMIZZ) == 1) {
					htmlid = "SIMIZZ6";
				}
			} else if (npcid == 71057) { // ドイル（宝の地図1）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_DOIL) == LsimulatorQuest.QUEST_END) {
					htmlid = "doil4b";
				}
			} else if (npcid == 71059) { // ルディアン（宝の地図2）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_RUDIAN) == LsimulatorQuest.QUEST_END) {
					htmlid = "rudian1c";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_RUDIAN) == 1) {
					htmlid = "rudian7";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_DOIL) == LsimulatorQuest.QUEST_END) {
					htmlid = "rudian1b";
				} else {
					htmlid = "rudian1a";
				}
			} else if (npcid == 71060) { // レスタ（宝の地図3）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == LsimulatorQuest.QUEST_END) {
					htmlid = "resta1e";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_SIMIZZ) == LsimulatorQuest.QUEST_END) {
					htmlid = "resta14";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == 4) {
					htmlid = "resta13";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == 3) {
					htmlid = "resta11";
					player.getQuest().set_step(LsimulatorQuest.QUEST_RESTA, 4);
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_RESTA) == 2) {
					htmlid = "resta16";
				} else if (((player.getQuest().get_step(LsimulatorQuest.QUEST_SIMIZZ) == 2) && (player
						.getQuest().get_step(LsimulatorQuest.QUEST_CADMUS) == 1))
						|| player.getInventory().checkItem(40647)) {
					htmlid = "resta1a";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_CADMUS) == 1)
						|| player.getInventory().checkItem(40647)) {
					htmlid = "resta1c";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_SIMIZZ) == 2) {
					htmlid = "resta1b";
				...
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_CADMUS) == LsimulatorQuest.QUEST_END) {
					htmlid = "cadmus1c";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_CADMUS) == 3) {
					htmlid = "cadmus8";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_CADMUS) == 2) {
					htmlid = "cadmus1a";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_DOIL) == LsimulatorQuest.QUEST_END) {
					htmlid = "cadmus1b";
				}
			} else if (npcid == 71036) { // カミーラ（ドレイクの真実）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == LsimulatorQuest.QUEST_END) {
					htmlid = "kamyla26";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 4)
						&& player.getInventory().checkItem(40717)) {
					htmlid = "kamyla15";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 4) {
					htmlid = "kamyla14";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 3)
						&& player.getInventory().checkItem(40630)) {
					htmlid = "kamyla12";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 3) {
					htmlid = "kamyla11";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 2)
						&& player.getInventory().checkItem(40644)) {
					htmlid = "kamyla9";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 1) {
					htmlid = "kamyla8";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_CADMUS) == LsimulatorQuest.QUEST_END)
						&& player.getInventory().checkItem(40621)) {
					htmlid = "kamyla1";
				}
			} else if (npcid == 71089) { // フランコ（ドレイクの真実）
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 2) {
					htmlid = "francu12";
				}
			} else if (npcid == 71090) { // 試練のクリスタル2（ドレイクの真実）
				if ((player.getQuest().get_step(LsimulatorQuest.QUEST_CRYSTAL) == 1)
						&& player.getInventory().checkItem(40620)) {
					htmlid = "jcrystal2";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_CRYSTAL) == 1) {
					htmlid = "jcrystal3";
				}
			} else if (npcid == 71091) { // 試練のクリスタル3（ドレイクの真実）
				if ((player.getQuest().get_step(LsimulatorQuest.QUEST_CRYSTAL) == 2)
						&& player.getInventory().checkItem(40654)) {
					htmlid = "jcrystall2";
				}
			} else if (npcid == 71074) { // リザードマンの長老
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == LsimulatorQuest.QUEST_END) {
					htmlid = "lelder0";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == 3)
						&& player.getInventory().checkItem(40634)) {
					htmlid = "lelder12";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == 3) {
					htmlid = "lelder11";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == 2)
						&& player.getInventory().checkItem(40633)) {
					htmlid = "lelder7";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == 2) {
					htmlid = "lelder7b";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == 1) {
					htmlid = "lelder7b";
				...
				if ((player.getQuest().get_step(LsimulatorQuest.QUEST_KEPLISHA) == LsimulatorQuest.QUEST_END)
				...
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_DESIRE) == LsimulatorQuest.QUEST_END) {
					htmlid = "minicod10";
				} else if (player.getKarmaLevel() >= 1) {
					htmlid = "minicod07";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_DESIRE) == 1)
						&& (player.getTempCharGfx() == 6034)) { // コラププリースト変身
					htmlid = "minicod03";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_DESIRE) == 1)
						&& (player.getTempCharGfx() != 6034)) {
					htmlid = "minicod05";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_SHADOWS) == LsimulatorQuest.QUEST_END // 影の神殿側クエスト終了
				)
				...
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_SHADOWS) == LsimulatorQuest.QUEST_END) {
					htmlid = "minitos10";
				} else if (player.getKarmaLevel() <= -1) {
					htmlid = "minitos07";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_SHADOWS) == 1)
						&& (player.getTempCharGfx() == 6035)) { // レッサーデーモン変身
					htmlid = "minitos03";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_SHADOWS) == 1)
						&& (player.getTempCharGfx() != 6035)) {
					htmlid = "minitos05";
				} else if ((player.getQuest().get_step(LsimulatorQuest.QUEST_DESIRE) == LsimulatorQuest.QUEST_END // 欲望の洞窟側クエスト終了
				)
				...
				if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == LsimulatorQuest.QUEST_END) {
					htmlid = "orena14";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 1) {
					htmlid = "orena0";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 2) {
					htmlid = "orena2";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 3) {
					htmlid = "orena3";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 4) {
					htmlid = "orena4";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 5) {
					htmlid = "orena5";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 6) {
					htmlid = "orena6";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 7) {
					htmlid = "orena7";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 8) {
					htmlid = "orena8";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 9) {
					htmlid = "orena9";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 10) {
					htmlid = "orena10";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 11) {
					htmlid = "orena11";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 12) {
					htmlid = "orena12";
				} else if (player.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 13) {
					htmlid = "orena13";
				}
				...
				if (!player.isElf()) {
					htmlid = "robinhood2";
						...
						LsimulatorQuest.QUEST_MOONOFLONGBOW) == 255) {
					htmlid = "robinhood12";
				} else if (player.getQuest().get_step(
						LsimulatorQuest.QUEST_MOONOFLONGBOW) == 8) {
					if (player.getInventory().checkItem(40491, 30)
						...
						LsimulatorQuest.QUEST_MOONOFLONGBOW) == 7) {
					if (player.getInventory().checkItem(41352, 4)
						...
						LsimulatorQuest.QUEST_MOONOFLONGBOW) == 2)
						|| (player.getQuest().get_step(
								LsimulatorQuest.QUEST_MOONOFLONGBOW) == 3)
						|| (player.getQuest().get_step(
								LsimulatorQuest.QUEST_MOONOFLONGBOW) == 4)
						|| (player.getQuest().get_step(
								LsimulatorQuest.QUEST_MOONOFLONGBOW) == 5)
						|| (player.getQuest().get_step(
								LsimulatorQuest.QUEST_MOONOFLONGBOW) == 6)) {
					htmlid = "robinhood13";
				} else if (player.getQuest().get_step(
						LsimulatorQuest.QUEST_MOONOFLONGBOW) == 1) {
					htmlid = "robinhood8";
				} else {
					htmlid = "robinhood1";
				}
				...
				if (!player.isElf()) {
					...
					htmlid = "zybril3";
						...
						&& ((player.getQuest().get_step(
								LsimulatorQuest.QUEST_MOONOFLONGBOW) == 2) || (player
								.getQuest().get_step(
										LsimulatorQuest.QUEST_MOONOFLONGBOW) == 1))) {
					htmlid = "zybril1";
				} else {
				...
				if (player.isDragonKnight()) { // ドラゴンナイト
					if ((player.getLevel() >= 50)
							&& (lv45_step == LsimulatorQuest.QUEST_END)) {
						if (lv50_step == 0) {
							htmlid = "prokel21";
							...
							&& (lv30_step == LsimulatorQuest.QUEST_END)) {
						if (lv45_step == 0) {
							htmlid = "prokel15";
							...
							&& (lv15_step == LsimulatorQuest.QUEST_END)) {
						if (lv30_step == 0) {
							htmlid = "prokel8";
						...
						if (lv15_step == 0) {
							htmlid = "prokel2";
						} else if (lv15_step == LsimulatorQuest.QUEST_END) { // クリア済み)
							htmlid = "prokel7";
						} else {
							htmlid = "prokel4";
						}
				 ...
				 * quest.get_step(LsimulatorQuest.QUEST_LEVEL45); if
				...
				if (player.isIllusionist()) { // 幻術士
					if (player.getLevel() > 49
						...
						if (lv45_step == 0) {
							htmlid = "silrein18";
						} else if (lv45_step == LsimulatorQuest.QUEST_END) {
							htmlid = "silrein26";
						} else if (lv45_step > 4) {
						...
						if (lv30_step == 0) {
							htmlid = "silrein11";
						} else if (lv30_step == LsimulatorQuest.QUEST_END) {
							htmlid = "silrein15";
						} else if (lv30_step > 0) { // ?
							htmlid = "silrein14";
						} else {
						...
						if (lv15_step == 0) {
							htmlid = "silrein2";
						} else if (lv15_step == LsimulatorQuest.QUEST_END) {
							htmlid = "silrein5";
						} else {
							htmlid = "silrein4";
						}
				...
				// STATUS_END = 3;
				if (LsimulatorBugBearRace.getInstance().getGameStatus() == 0) {
					htmlid = "maeno5";
				} else if (LsimulatorBugBearRace.getInstance().getGameStatus() == 1) {
					htmlid = "maeno1";
				} else if (LsimulatorBugBearRace.getInstance().getGameStatus() == 2) {
					htmlid = "maeno3";
				} else if (LsimulatorBugBearRace.getInstance().getGameStatus() == 3) {
					htmlid = "maeno5";
				}
			} else if (npcid == 81255) { // 新手導師
				int quest_step = quest.get_step(LsimulatorQuest.QUEST_TUTOR);// 任務編號階段
				...
				if (playerLv < 2 && quest_step == 0) {
					...
					LsimulatorItemInstance item = player.getInventory().storeItem(
							42099, 5); // 指定傳送卷軸(隱藏之谷) * 5
					player.sendPackets(new S_ServerMessage(143, item.getItem()
							.getName()));
					quest.set_step(LsimulatorQuest.QUEST_TUTOR, 1); // 設定任務
					htmlid = "";
				...
				int quest_step = quest.get_step(LsimulatorQuest.QUEST_TUTOR2);// 任務編號階段
				int playerLv = player.getLevel();// 角色等級
				if (playerLv > 4 && playerLv < 13 && quest_step > 2) {
					...
					newUserHelp(player, 2);// HASTE
					quest.set_step(LsimulatorQuest.QUEST_TUTOR2, 2); // 設定任務
					htmlid = "admin3";// 獲得裝備
					...
					quest.set_step(LsimulatorQuest.QUEST_TUTOR2, 1); // 設定任務
					htmlid = "admin2";
				} else if (playerLv < 5) {
					newUserHelp(player, 2);// HASTE
					newUserHelp(player, 3);// 神聖
	...
	private static String talkToTownadviser(LsimulatorPcInstance pc, int town_id) {
		String htmlid;
	...
	private static String talkToTownmaster(LsimulatorPcInstance pc, int town_id) {
		String htmlid;
		if (pc.getHomeTownId() == town_id) {
			htmlid = "hometown";
		} else {
	...
	private boolean checkHasCastle(LsimulatorPcInstance player, int castle_id) {
	...
	private boolean checkClanLeader(LsimulatorPcInstance player) {
		if (player.isCrown()) { // 君主
			LsimulatorClan clan = LsimulatorWorld.getInstance().getClan(player.getClanname());
			if (clan != null) {
	...
	private int getNecessarySealCount(LsimulatorPcInstance pc) {
		int rulerCount = 0;
	...
	private void createRuler(LsimulatorPcInstance pc, int attr, int sealCount) {
		...
		LsimulatorItemInstance item = pc.getInventory().storeItem(rulerId, 1);
		if (item != null) {
	...
	private String talkToDoromond(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getQuest().get_step(LsimulatorQuest.QUEST_DOROMOND) == 0) {
			htmlid = "jpe0011";
		} else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_DOROMOND) == 1) {
			htmlid = "jpe0015";
		}
	...
	private String talkToAlex(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 3) {
			htmlid = "jpe0021";
		} else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_DOROMOND) < 2) {
			htmlid = "jpe0022";
		} else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_AREX) == LsimulatorQuest.QUEST_END) {
			htmlid = "jpe0023";
	...
	private String talkToAlexInTrainingRoom(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 3) {
			htmlid = "jpe0031";
		} else {
			if (pc.getQuest().get_step(LsimulatorQuest.QUEST_DOROMOND) < 2) {
				htmlid = "jpe0035";
			} else {
				htmlid = "jpe0036";
			}
	...
	private String cancellation(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 13) {
			htmlid = "jpe0161";
		} else {
	...
	private String talkToRuba(LsimulatorPcInstance pc) {
		String htmlid = "";
	...
	private String talkToSIGuide(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 3) {
			htmlid = "en0301";
	...
	private String talkToPopirea(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getLevel() < 25) {
			htmlid = "jpe0041";
	...
	private String talkToSecondtbox(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TBOX1) == LsimulatorQuest.QUEST_END) {
	...
	private String talkToThirdtbox(LsimulatorPcInstance pc) {
		String htmlid = "";
		if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TBOX2) == LsimulatorQuest.QUEST_END) {
			if (pc.getInventory().checkItem(40701)) {
				htmlid = "maptboxd";
			} else {
				htmlid = "maptbox0";
	...
	private void newUserHelp(LsimulatorPcInstance pc, int helpNo) {
		switch (helpNo) {
