...
public class MobGroupTable {
	...
	private final Map<Integer, LsimulatorMobGroup> _mobGroupIndex = Maps.newMap();
	...
	private void loadMobGroup() {
		...
		try {
			...
			while (rs.next()) {
				...
				List<LsimulatorNpcCount> minions = Lists.newList();
				for (int i = 1; i <= 7; i++) {
					int id = rs.getInt("minion" + i + "_id");
					int count = rs.getInt("minion" + i + "_count");
					minions.add(new LsimulatorNpcCount(id, count));
				}
				LsimulatorMobGroup mobGroup = new LsimulatorMobGroup(mobGroupId, leaderId, minions, isRemoveGroup);
	...
	public LsimulatorMobGroup getTemplate(int mobGroupId) {
