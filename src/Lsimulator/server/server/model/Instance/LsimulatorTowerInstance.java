...
public class LsimulatorTowerInstance extends LsimulatorNpcInstance {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	public LsimulatorTowerInstance(LsimulatorNpc template) {
		super(template);
	}
	...
	public void receiveDamage(LsimulatorCharacter attacker, int damage) { // 攻撃でＨＰを減らすときはここを使用
		...
		if ((_castle_id > 0) && WarTimeController.getInstance().isNowWar(_castle_id)) { // 戦争時間内
			...
			if (attacker instanceof LsimulatorPcInstance) {
				pc = (LsimulatorPcInstance) attacker;
			}
			else if (attacker instanceof LsimulatorPetInstance) {
				pc = (LsimulatorPcInstance) ((LsimulatorPetInstance) attacker).getMaster();
			}
			else if (attacker instanceof LsimulatorSummonInstance) {
				pc = (LsimulatorPcInstance) ((LsimulatorSummonInstance) attacker).getMaster();
			}
			if (pc == null) {
				return;
			}
	...
	class Death implements Runnable {
		...
		LsimulatorTowerInstance npc = (LsimulatorTowerInstance) object;
