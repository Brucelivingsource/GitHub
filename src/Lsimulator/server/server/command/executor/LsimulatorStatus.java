...
public class LsimulatorStatus implements LsimulatorCommandExecutor {
	private LsimulatorStatus() {
	}

	public static LsimulatorCommandExecutor getInstance() {
		return new LsimulatorStatus();
	}
	...
	public void execute(LsimulatorPcInstance pc, String cmdName, String arg) {
		try {
			...
			LsimulatorPcInstance target = null;
			if (char_name.equalsIgnoreCase("me")) {
				target = pc;
			}
			else {
				target = LsimulatorWorld.getInstance().getPlayer(char_name);
			}
			...
			pc.sendPackets(new S_SystemMessage(target.getName() + " 的" + param + "值" + value + "被變更了。"));
		}
		catch (Exception e) {
