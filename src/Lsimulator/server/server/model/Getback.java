	...
	public static int[] GetBack_Location(LsimulatorPcInstance pc, boolean bScroll_Escape) {
		...
		if (getbackList != null) {
			...
			if (pc.isElf() && (getback._getbackTownIdForElf > 0)) {
				loc = LsimulatorTownLocation.getGetBackLoc(getback._getbackTownIdForElf);
			}
			else if (pc.isDarkelf() && (getback._getbackTownIdForDarkelf > 0)) {
				loc = LsimulatorTownLocation.getGetBackLoc(getback._getbackTownIdForDarkelf);
			}
			else if (getback._getbackTownId > 0) {
				loc = LsimulatorTownLocation.getGetBackLoc(getback._getbackTownId);
			}
		}
