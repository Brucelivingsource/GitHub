...
public class LsimulatorMonsterInstance extends LsimulatorNpcInstance {
	...
	private static Logger _log = Logger.getLogger(LsimulatorMonsterInstance.class
			.getName());
	...
	public void onDoppel(boolean isChangeShape) {
		if (getNpcTemplate().is_doppel()) {
			...
			} else if (!isDoppel && (_target instanceof LsimulatorPcInstance)) { // 未變形
				setSleepTime(300);
	...
	public void searchTarget() {
		// 目標捜索
		LsimulatorPcInstance lastTarget = null;
		LsimulatorPcInstance targetPlayer = null;

		if (_target != null && _target instanceof LsimulatorPcInstance ) {
			lastTarget = (LsimulatorPcInstance) _target;
			tagertClear();
		}
	...
	public LsimulatorMonsterInstance(LsimulatorNpc template) {
		super(template);
		_storeDroped = false;
	}
	...
	public void ReceiveManaDamage(LsimulatorCharacter attacker, int mpDamage) { // 攻撃でＭＰを減らすときはここを使用
		if ((mpDamage > 0) && !isDead()) {
			...
			if (attacker instanceof LsimulatorPcInstance) { // 仲間意識をもつモンスターのターゲットに設定
				serchLink((LsimulatorPcInstance) attacker, getNpcTemplate()
						.get_family());
			}
	...
	public void receiveDamage(LsimulatorCharacter attacker, int damage) { // 攻撃でＨＰを減らすときはここを使用
		if ((getCurrentHp() > 0) && !isDead()) {
			...
			if (damage >= 0) {
				if (!(attacker instanceof LsimulatorEffectInstance)) { // FWはヘイトなし
	...
	private static void openDoorWhenNpcDied(LsimulatorNpcInstance npc) {
	 ...
	 */
	/*
	 * private void recall(LsimulatorPcInstance pc) { if (getMapId() != pc.getMapId()) {
	...
	private void distributeExpDropKarma(LsimulatorCharacter lastAttacker) {
		...
		if (lastAttacker instanceof LsimulatorPcInstance) {
			pc = (LsimulatorPcInstance) lastAttacker;
		} else if (lastAttacker instanceof LsimulatorPetInstance) {
			pc = (LsimulatorPcInstance) ((LsimulatorPetInstance) lastAttacker).getMaster();
		} else if (lastAttacker instanceof LsimulatorSummonInstance) {
			pc = (LsimulatorPcInstance) ((LsimulatorSummonInstance) lastAttacker).getMaster();
		}
		...
		} else if (lastAttacker instanceof LsimulatorEffectInstance) { // FWが倒した場合
			ArrayList<LsimulatorCharacter> targetList = _hateList.toTargetArrayList();
			ArrayList<Integer> hateList = _hateList.toHateArrayList();
			// ヘイトリストにキャラクターが存在する
			if (!hateList.isEmpty()) {
				...
				if (lastAttacker instanceof LsimulatorPcInstance) {
					...
					pc = (LsimulatorPcInstance) ((LsimulatorPetInstance) lastAttacker)
							.getMaster();
				} else if (lastAttacker instanceof LsimulatorSummonInstance) {
					pc = (LsimulatorPcInstance) ((LsimulatorSummonInstance) lastAttacker)
							.getMaster();
				}
				if (pc != null) {
					int exp = getExp();
	...
	private void distributeDrop() {
		...
		try {
			...
			if ((npcId != 45640)
				...
				DropTable.getInstance().dropShare(LsimulatorMonsterInstance.this,
	...
	private void giveKarma(LsimulatorPcInstance pc) {
		int karma = getKarma();
		if (karma != 0) {
	...
	private void giveUbSeal() {
		if (getUbSealCount() != 0) { // UBの勇者の証
			...
			if (ub != null) {
				for (LsimulatorPcInstance pc : ub.getMembersArray()) {
	...
	public void initHide() {
		...
		if ((npcid == 45061 // カーズドスパルトイ
			...
			setHiddenStatus(LsimulatorNpcInstance.HIDDEN_STATUS_ICE);
			setStatus(4);
		}
	}

	public void initHideForMinion(LsimulatorNpcInstance leader) {
	...
	private void doNextDragonStep(LsimulatorCharacter attacker, int npcid) {
		if (!isNextDragonStepRunning()) {
			...
			if (attacker != null && nextSpawnId > 0) {
				LsimulatorPcInstance _pc = null;
				if (attacker instanceof LsimulatorPcInstance) {
					_pc = (LsimulatorPcInstance) attacker;
				} else if (attacker instanceof LsimulatorPetInstance) {
	...
	class NextDragonStep implements Runnable {
		LsimulatorPcInstance _pc;
		LsimulatorMonsterInstance _mob;
		int _npcid;
		int _transformId;
		int _x;
		int _y;
		...
		public NextDragonStep(LsimulatorPcInstance pc, LsimulatorMonsterInstance mob,
				int transformId) {
			_pc = pc;
			_mob = mob;
		...
		public void run() {
			...
			try {
				Thread.sleep(10500);
				LsimulatorNpcInstance npc = NpcTable.getInstance().newNpcInstance(
						_transformId);
