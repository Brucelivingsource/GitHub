...
import Lsimulator.server.server.serverpackets.S_CloseList;
...
public class LsimulatorPolyMorph {
	...
	public LsimulatorPolyMorph(int id, String name, int polyId, int minLevel,
	...
	// 變身
	public static void doPoly(LsimulatorCharacter cha, int polyId, int timeSecs, int cause) {
		doPoly(cha, polyId, timeSecs, cause, true);
	}

	// 變身
	public static void doPoly(LsimulatorCharacter cha, int polyId, int timeSecs, int cause, boolean cantPolyMessage) {
		...
		if (cha instanceof PcInstance) {
			...
			if (mob.getTempCharGfx() != polyId) {
				...
				int npcStatus = LsimulatorNpcDefaultAction.getInstance().getStatus(polyId);
	 ...
	 * @param polyIndex 1-8
	 */
	public static void doPolyPraivateShop(LsimulatorCharacter cha, int polyIndex){
		if ((cha == null) || cha.isDead()) {
			return;
		}
		if (cha instanceof PcInstance) {
