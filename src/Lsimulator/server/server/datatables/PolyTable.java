...
public class PolyTable {
	...
	private final Map<String, LsimulatorPolyMorph> _polymorphs = Maps.newMap();

	private final Map<Integer, LsimulatorPolyMorph> _polyIdIndex = Maps.newMap();
	...
	private void fillPolyTable(ResultSet rs) throws SQLException {
		while (rs.next()) {
			...
			LsimulatorPolyMorph poly = new LsimulatorPolyMorph(id, name, polyId, minLevel, weaponEquipFlg, armorEquipFlg, canUseSkill, causeFlg);
	...
	public LsimulatorPolyMorph getTemplate(String name) {
		return _polymorphs.get(name);
	}

	public LsimulatorPolyMorph getTemplate(int polyId) {
