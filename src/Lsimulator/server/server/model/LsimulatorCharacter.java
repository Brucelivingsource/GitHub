...
import Lsimulator.server.server.serverpackets.S_Light;
...
public class LsimulatorCharacter extends LsimulatorObject {
	...
	private LsimulatorPoison _poison = null;
	...
	LsimulatorParalysis _paralysis;

	public LsimulatorParalysis getParalysis() {
		return _paralysis;
	}

	public void setParalaysis(LsimulatorParalysis p) {
		_paralysis = p;
	}
	 ...
	 * キャラクターのインベントリを返す。
	 * 
	 * @return キャラクターのインベントリを表す、LsimulatorInventoryオブジェクト。
	 */
	...
	private void addSkillEffect(int skillId, int timeMillis) {
		LsimulatorSkillTimer timer = null;
		if (0 < timeMillis) {
			timer = LsimulatorSkillTimerCreator.create(this, skillId, timeMillis);
			timer.begin();
		}
	...
	public void removeSkillEffect(int skillId) {
		LsimulatorSkillTimer timer = _skillEffect.remove(skillId);
		if (timer != null) {
			timer.end();
		}
	}
	...
	public void clearSkillEffectTimer() {
		for (LsimulatorSkillTimer timer : _skillEffect.values()) {
			if (timer != null) {
				timer.kill();
			}
		}
	...
	public int getSkillEffectTimeSec(int skillId) {
		LsimulatorSkillTimer timer = _skillEffect.get(skillId);
		if (timer == null) {
			return -1;
		}
	 ...
	 * @param timer
	 *            ディレイ時間を表す、LsimulatorItemDelay.ItemDelayTimerオブジェクト。
	 */
	public void addItemDelay(int delayId, LsimulatorItemDelay.ItemDelayTimer timer) {
	...
	/**
	 * キャラクターのアイテムディレイ時間を表す、LsimulatorItemDelay.ItemDelayTimerを返す。
	 * 
	 * @param delayId
	 *            調べるアイテムディレイID。 通常のアイテムであれば0、インビジビリティ クローク、バルログ ブラッディ
	 *            クロークであれば1。
	 * @return アイテムディレイ時間を表す、LsimulatorItemDelay.ItemDelayTimer。
	 */
	public LsimulatorItemDelay.ItemDelayTimer getItemDelayTimer(int delayId) {
	 ...
	 * 
	 * @param poison
	 *            毒を表す、LsimulatorPoisonオブジェクト。
	 */
	public void setPoison(LsimulatorPoison poison) {
		_poison = poison;
	}
	 ...
	 * キャラクターの毒状態を返す。
	 * 
	 * @return キャラクターの毒を表す、LsimulatorPoisonオブジェクト。
	 */
	public LsimulatorPoison getPoison() {
		return _poison;
	}
	 ...
	 */
	public boolean knownsObject(LsimulatorObject obj) {
	 ...
	 * 
	 * @return キャラクターが認識しているオブジェクトを表すLsimulatorObjectが格納されたArrayList。
	 */
	public List<LsimulatorObject> getKnownObjects() {
	...
	public int getAc() {
		return _ac + LsimulatorMagicDoll.getAcByDoll(this); // TODO 魔法娃娃效果 - 防禦增加
	}
	...
	public int getRegistStun() {
		return (_registStun + LsimulatorMagicDoll.getRegistStunByDoll(this));
	}
	...
	public int getRegistStone() {
		return (_registStone + LsimulatorMagicDoll.getRegistStoneByDoll(this));
	}
	...
	public int getRegistSleep() {
		return (_registSleep + LsimulatorMagicDoll.getRegistSleepByDoll(this));
	}
	...
	public int getRegistFreeze() {
		return (_registFreeze
				+ LsimulatorMagicDoll.getRegistFreezeByDoll(this)); // TODO 魔法娃娃效果 - 寒冰耐性增加
	}
	...
	public int getRegistSustain() {
		return (_registSustain + LsimulatorMagicDoll.getRegistSustainByDoll(this));
	}
	...
	public int getRegistBlind() {
		return (_registBlind + LsimulatorMagicDoll.getRegistBlindByDoll(this));
	}
	...
	public int getDmgup() {
		return _dmgup + LsimulatorMagicDoll.getDamageAddByDoll(this); // 魔法娃娃效果 - 近距離的攻擊力增加
	} // 使用するとき
	...
	public int getBowDmgup() {
		return _bowDmgup + LsimulatorMagicDoll.getBowDamageByDoll(this); // 魔法娃娃效果 - 遠距離的攻擊力增加
	}
	...
	public int getHitup() {
		return (_hitup
				+ LsimulatorMagicDoll.getHitAddByDoll(this)); // TODO 魔法娃娃效果 - 近距離的命中力增加
	}
	...
	public int getBowHitup() {
		return (_bowHitup
				+ LsimulatorMagicDoll.getBowHitAddByDoll(this)); // TODO 魔法娃娃效果 - 弓的命中力增加
	}
	...
	// 判斷特定狀態下才可攻擊 NPC
	public boolean isAttackMiss(LsimulatorCharacter cha , int npcId) {
		switch (npcId) {
			case 45912: // 士兵的怨靈
			case 45913: // 士兵的怨靈
			case 45914: // 怨靈
