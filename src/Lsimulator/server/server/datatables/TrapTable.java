...
public class TrapTable {
	...
	private Map<Integer, LsimulatorTrap> _traps = Maps.newMap();

	private TrapTable() {
		initialize();
	}

	private LsimulatorTrap createTrapInstance(String name, TrapStorage storage) throws Exception {
		final String packageName = "Lsimulator.server.server.model.trap.";

		Constructor<?> con = Class.forName(packageName + name).getConstructor(new Class[]
		{ TrapStorage.class });
		return (LsimulatorTrap) con.newInstance(storage);
	}
	...
	private void initialize() {
		...
		try {
			...
			while (rs.next()) {
				...
				LsimulatorTrap trap = createTrapInstance(typeName, new SqlTrapStorage(rs));
	...
	public LsimulatorTrap getTemplate(int id) {
