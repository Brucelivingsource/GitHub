...
// LsimulatorCastleLocation

public class LsimulatorCastleLocation {
	// castle_id
	...
	private static final Map<Integer, LsimulatorLocation> _towers = Maps.newMap();

	static {
		_towers.put(KENT_CASTLE_ID, new LsimulatorLocation(KENT_TOWER_X, KENT_TOWER_Y, KENT_TOWER_MAP));
		_towers.put(OT_CASTLE_ID, new LsimulatorLocation(OT_TOWER_X, OT_TOWER_Y, OT_TOWER_MAP));
		_towers.put(WW_CASTLE_ID, new LsimulatorLocation(WW_TOWER_X, WW_TOWER_Y, WW_TOWER_MAP));
		_towers.put(GIRAN_CASTLE_ID, new LsimulatorLocation(GIRAN_TOWER_X, GIRAN_TOWER_Y, GIRAN_TOWER_MAP));
		_towers.put(HEINE_CASTLE_ID, new LsimulatorLocation(HEINE_TOWER_X, HEINE_TOWER_Y, HEINE_TOWER_MAP));
		_towers.put(DOWA_CASTLE_ID, new LsimulatorLocation(DOWA_TOWER_X, DOWA_TOWER_Y, DOWA_TOWER_MAP));
		_towers.put(ADEN_CASTLE_ID, new LsimulatorLocation(ADEN_TOWER_X, ADEN_TOWER_Y, ADEN_TOWER_MAP));
		_towers.put(DIAD_CASTLE_ID, new LsimulatorLocation(DIAD_TOWER_X, DIAD_TOWER_Y, DIAD_TOWER_MAP));
	}

	private static final Map<Integer, LsimulatorMapArea> _areas = Maps.newMap();

	static {
		_areas.put(KENT_CASTLE_ID, new LsimulatorMapArea(KENT_X1, KENT_Y1, KENT_X2, KENT_Y2, KENT_MAP));
		_areas.put(OT_CASTLE_ID, new LsimulatorMapArea(OT_X1, OT_Y1, OT_X2, OT_Y2, OT_MAP));
		_areas.put(WW_CASTLE_ID, new LsimulatorMapArea(WW_X1, WW_Y1, WW_X2, WW_Y2, WW_MAP));
		_areas.put(GIRAN_CASTLE_ID, new LsimulatorMapArea(GIRAN_X1, GIRAN_Y1, GIRAN_X2, GIRAN_Y2, GIRAN_MAP));
		_areas.put(HEINE_CASTLE_ID, new LsimulatorMapArea(HEINE_X1, HEINE_Y1, HEINE_X2, HEINE_Y2, HEINE_MAP));
		_areas.put(DOWA_CASTLE_ID, new LsimulatorMapArea(DOWA_X1, DOWA_Y1, DOWA_X2, DOWA_Y2, DOWA_MAP));
		_areas.put(ADEN_CASTLE_ID, new LsimulatorMapArea(ADEN_X1, ADEN_Y1, ADEN_X2, ADEN_Y2, ADEN_MAP));
		_areas.put(DIAD_CASTLE_ID, new LsimulatorMapArea(DIAD_X1, DIAD_Y1, DIAD_X2, DIAD_Y2, DIAD_MAP));
	}
	...
	private static final Map<Integer, LsimulatorLocation> _subTowers = Maps.newMap();

	static {
		_subTowers.put(1, new LsimulatorLocation(ADEN_SUB_TOWER1_X, ADEN_SUB_TOWER1_Y, ADEN_TOWER_MAP));
		_subTowers.put(2, new LsimulatorLocation(ADEN_SUB_TOWER2_X, ADEN_SUB_TOWER2_Y, ADEN_TOWER_MAP));
		_subTowers.put(3, new LsimulatorLocation(ADEN_SUB_TOWER3_X, ADEN_SUB_TOWER3_Y, ADEN_TOWER_MAP));
		_subTowers.put(4, new LsimulatorLocation(ADEN_SUB_TOWER4_X, ADEN_SUB_TOWER4_Y, ADEN_TOWER_MAP));
	}

	private LsimulatorCastleLocation() {
	}

	public static int getCastleId(LsimulatorLocation loc) {
		for (Map.Entry<Integer, LsimulatorLocation> entry : _towers.entrySet()) {
	...
	public static int getCastleId(int locx, int locy, short mapid) {
		return getCastleId(new LsimulatorLocation(locx, locy, mapid));
	}

	public static int getCastleIdByArea(LsimulatorLocation loc) {
		for (Map.Entry<Integer, LsimulatorMapArea> entry : _areas.entrySet()) {
	 ...
	 */
	public static int getCastleIdByArea(LsimulatorCharacter cha) {
		return getCastleIdByArea(cha.getLocation());
	}

	public static boolean checkInWarArea(int castleId, LsimulatorLocation loc) {
	 ...
	 */
	public static boolean checkInWarArea(int castleId, LsimulatorCharacter cha) {
		return checkInWarArea(castleId, cha.getLocation());
	}

	public static boolean checkInAllWarArea(LsimulatorLocation loc) {
	...
	public static boolean checkInAllWarArea(int locx, int locy, short mapid) {
		return checkInAllWarArea(new LsimulatorLocation(locx, locy, mapid));
	}
	...
	public static int[] getTowerLoc(int castleId) {
		...
		LsimulatorLocation loc = _towers.get(castleId);
		if (loc != null) {
			result[0] = loc.getX();
			result[1] = loc.getY();
	...
	public static int[] getGetBackLoc(int castle_id) {
		int[] loc;
		if (castle_id == KENT_CASTLE_ID) { // ケント城
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_KENT);
		}
		else if (castle_id == OT_CASTLE_ID) { // オークの森
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_ORCISH_FOREST);
		}
		else if (castle_id == WW_CASTLE_ID) { // ウィンダウッド城
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_WINDAWOOD);
		}
		else if (castle_id == GIRAN_CASTLE_ID) { // ギラン城
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_GIRAN);
		}
		else if (castle_id == HEINE_CASTLE_ID) { // ハイネ城
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_HEINE);
		}
		else if (castle_id == DOWA_CASTLE_ID) { // ドワーフ城
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_WERLDAN);
		}
		else if (castle_id == ADEN_CASTLE_ID) { // アデン城
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_ADEN);
		}
		else if (castle_id == DIAD_CASTLE_ID) { // ディアド要塞
			// ディアド要塞の帰還先は未調査
			...
			loc = LsimulatorTownLocation.getGetBackLoc(LsimulatorTownLocation.TOWNID_SILVER_KNIGHT_TOWN);
		}
		return loc;
	}
	...
	public static int getCastleIdByNpcid(int npcid) {
		...
		switch (town_id) {
			case LsimulatorTownLocation.TOWNID_KENT:
			case LsimulatorTownLocation.TOWNID_GLUDIO:
				castle_id = KENT_CASTLE_ID; // ケント城
				break;

			case LsimulatorTownLocation.TOWNID_ORCISH_FOREST:
				castle_id = OT_CASTLE_ID; // オークの森
				break;

			case LsimulatorTownLocation.TOWNID_SILVER_KNIGHT_TOWN:
			case LsimulatorTownLocation.TOWNID_WINDAWOOD:
				castle_id = WW_CASTLE_ID; // ウィンダウッド城
				break;

			case LsimulatorTownLocation.TOWNID_TALKING_ISLAND:
			case LsimulatorTownLocation.TOWNID_GIRAN:
				castle_id = GIRAN_CASTLE_ID; // ギラン城
				break;

			case LsimulatorTownLocation.TOWNID_HEINE:
				castle_id = HEINE_CASTLE_ID; // ハイネ城
				break;

			case LsimulatorTownLocation.TOWNID_WERLDAN:
			case LsimulatorTownLocation.TOWNID_OREN:
				castle_id = DOWA_CASTLE_ID; // ドワーフ城
				break;

			case LsimulatorTownLocation.TOWNID_ADEN:
				castle_id = ADEN_CASTLE_ID; // アデン城
				break;

			case LsimulatorTownLocation.TOWNID_OUM_DUNGEON:
	...
	private static LsimulatorCastleTaxRateListener _listener;

	// GameServer#initialize,LsimulatorCastleTaxRateListener#onDayChangedだけに呼び出される予定。
	public static void setCastleTaxRate() {
		for (LsimulatorCastle castle : CastleTable.getInstance().getCastleTableList()) {
			_castleTaxRate.put(castle.getId(), castle.getTaxRate());
		}
		if (_listener == null) {
			_listener = new LsimulatorCastleTaxRateListener();
			LsimulatorGameTimeClock.getInstance().addListener(_listener);
		}
	}

	private static class LsimulatorCastleTaxRateListener extends LsimulatorGameTimeAdapter {
		@Override
		public void onDayChanged(LsimulatorGameTime time) {
			LsimulatorCastleLocation.setCastleTaxRate();
		}
	}
	...
	public static int[] getSubTowerLoc(int no) {
		int[] result = new int[3];
		LsimulatorLocation loc = _subTowers.get(no);
		if (loc != null) {
			result[0] = loc.getX();
			result[1] = loc.getY();
			result[2] = loc.getMapId();
