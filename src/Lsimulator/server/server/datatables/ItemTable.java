...
import Lsimulator.server.server.templates.LsimulatorWeapon;
...
public class ItemTable {
	...
	private LsimulatorItem _allTemplates[];

	private final Map<Integer, LsimulatorEtcItem> _etcitems;

	private final Map<Integer, LsimulatorArmor> _armors;

	private final Map<Integer, LsimulatorWeapon> _weapons;
	...
	private Map<Integer, LsimulatorEtcItem> allEtcItem() {
		Map<Integer, LsimulatorEtcItem> result = Maps.newMap();

		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		LsimulatorEtcItem item = null;
		try {
	...
	private Map<Integer, LsimulatorWeapon> allWeapon() {
		Map<Integer, LsimulatorWeapon> result = Maps.newMap();

		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		LsimulatorWeapon weapon = null;
		try {
			con = LsimulatorDatabaseFactory.getInstance().getConnection();
	...
	private Map<Integer, LsimulatorArmor> allArmor() {
		Map<Integer, LsimulatorArmor> result = Maps.newMap();
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		LsimulatorArmor armor = null;
		try {
			con = LsimulatorDatabaseFactory.getInstance().getConnection();
	...
	private void buildFastLookupTable() {
		...
		Collection<LsimulatorEtcItem> items = _etcitems.values();
		for (LsimulatorEtcItem item : items) {
			if (item.getItemId() > highestId) {
				highestId = item.getItemId();
			}
		}

		Collection<LsimulatorWeapon> weapons = _weapons.values();
		for (LsimulatorWeapon weapon : weapons) {
			if (weapon.getItemId() > highestId) {
				highestId = weapon.getItemId();
			}
		}

		Collection<LsimulatorArmor> armors = _armors.values();
		for (LsimulatorArmor armor : armors) {
			if (armor.getItemId() > highestId) {
				highestId = armor.getItemId();
			}
		}

		_allTemplates = new LsimulatorItem[highestId + 1];

		for (Integer id : _etcitems.keySet()) {
			LsimulatorEtcItem item = _etcitems.get(id);
			_allTemplates[id.intValue()] = item;
		}

		for (Integer id : _weapons.keySet()) {
			LsimulatorWeapon item = _weapons.get(id);
			_allTemplates[id.intValue()] = item;
		}

		for (Integer id : _armors.keySet()) {
			LsimulatorArmor item = _armors.get(id);
			_allTemplates[id.intValue()] = item;
		}
	}

	public LsimulatorItem getTemplate(int id) {
	...
	public int findItemIdByName(String name) {
		int itemid = 0;
		for (LsimulatorItem item : _allTemplates) {
