...
public class RaceTicketTable {
	...
	private final HashMap<Integer, LsimulatorRaceTicket> _tickets = new HashMap<Integer, LsimulatorRaceTicket>();
	...
	public void storeNewTiket(LsimulatorRaceTicket ticket) {
		// PCのインベントリーが増える場合に実行
		// XXX 呼ばれる前と処理の重複
	...
	public void updateTicket(int round, int num, double allotment_percentage) {
		for (LsimulatorRaceTicket ticket : getRaceTicketTableList()) {
	...
	public LsimulatorRaceTicket getTemplate(int itemobjid) {
		if (_tickets.containsKey(itemobjid)) {
	...
	public LsimulatorRaceTicket[] getRaceTicketTableList() {
		return _tickets.values().toArray(new LsimulatorRaceTicket[_tickets.size()]);
	}
