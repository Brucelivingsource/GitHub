...
public class HouseTable {
	...
	private final Map<Integer, LsimulatorHouse> _house = Maps.newConcurrentMap();
	...
	public HouseTable() {
		...
		try {
			...
			while (rs.next()) {
				LsimulatorHouse house = new LsimulatorHouse();
	...
	public LsimulatorHouse[] getHouseTableList() {
		return _house.values().toArray(new LsimulatorHouse[_house.size()]);
	}

	public LsimulatorHouse getHouseTable(int houseId) {
		return _house.get(houseId);
	}

	public void updateHouse(LsimulatorHouse house) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = LsimulatorDatabaseFactory.getInstance().getConnection();
