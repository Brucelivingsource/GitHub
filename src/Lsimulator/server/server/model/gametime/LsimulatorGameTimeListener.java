 ...
 * 関連するメソッドだけをオーバーライドしてabstractクラスLsimulatorGameTimeAdapterを拡張する。
 * </p>
 * <p>
 * そのようなクラスから作成されたリスナーオブジェクトは、LsimulatorGameTimeClockのaddListenerメソッドを使用してLsimulatorGameTimeClockに登録される。
 * アデン時間変化の通知は、月日時分がそれぞれ変わったときに行われる。
 * </p>
 * <p>
 * これらのメソッドは、LsimulatorGameTimeClockのスレッド上で動作する。
 * これらのメソッドの処理に時間がかかった場合、他のリスナーへの通知が遅れる可能性がある。
 * 完了までに時間を要する処理や、スレッドをブロックするメソッドの呼び出しが含まれる処理を行う場合は、内部で新たにスレッドを作成して処理を行うべきである。
 * </p>
 * 
 */
public interface LsimulatorGameTimeListener {
	/**
	 ...
	 *            最新のアデン時間
	 */
	public void onMonthChanged(LsimulatorGameTime time);
	 ...
	 *            最新のアデン時間
	 */
	public void onHourChanged(LsimulatorGameTime time);
