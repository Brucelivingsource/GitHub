...
public class Enchant {
	...
	public static void scrollOfEnchantWeapon(PcInstance pc, ItemInstance l1iteminstance
		...
		if ((weaponId >= 246) && (weaponId <= 249)) { // 試煉之劍
			if (itemId != LsimulatorItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON) { // 非試煉卷軸
				pc.sendPackets(new S_ServerMessage(79));
				return;
			}
		}
		if (itemId == LsimulatorItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON) { // 試煉卷軸
		...
		if (itemId == LsimulatorItemId.C_SCROLL_OF_ENCHANT_WEAPON) { // 受咀咒的 對武器施法的卷軸
	...
	public static void scrollOfEnchantArmor(PcInstance pc, ItemInstance l1iteminstance
		...
		int enchant_level = l1iteminstance1.getEnchantLevel();
		if (itemId == LsimulatorItemId.C_SCROLL_OF_ENCHANT_ARMOR) { // 受咀咒的 對盔甲施法的卷軸
			pc.getInventory().removeItem(l1iteminstance, 1);
			if (enchant_level < -6) {
				// -7以上失敗。
				FailureEnchant(pc, l1iteminstance1);
		...
		if (rnd < enchant_chance_accessory) { // 成功
			...
			switch (l1iteminstance1.getItem().getGrade()) {
				...
				case 1: // 中等
					...
					itemStatus += LsimulatorPcInventory.COL_ADDHP;
					// LV6 額外獎勵：魔防 +1
					if (award) {
						l1iteminstance1.setM_Def(l1iteminstance1.getM_Def() + 1);
						itemStatus += LsimulatorPcInventory.COL_M_DEF;
					}
					// 裝備中
					if (l1iteminstance1.isEquipped()) {
						pc.addMaxHp(2);
				...
				case 2: // 初等
					...
					itemStatus += LsimulatorPcInventory.COL_ADDMP;
					// LV6 額外獎勵：魔攻 +1
					if (award) {
						l1iteminstance1.setaddSp(l1iteminstance1.getaddSp() + 1);
						itemStatus += LsimulatorPcInventory.COL_ADDSP;
					}
					// 裝備中
					if (l1iteminstance1.isEquipped()) {
						pc.addMaxMp(1);
	...
	public static void scrollOfEnchantWeaponAttr(PcInstance pc, ItemInstance l1iteminstance
		...
		if (Config.ATTR_ENCHANT_CHANCE >= rnd) {
			...
			pc.getInventory().updateItem(l1iteminstance1, LsimulatorPcInventory.COL_ATTR_ENCHANT_KIND);
			pc.getInventory().saveItem(l1iteminstance1, LsimulatorPcInventory.COL_ATTR_ENCHANT_KIND);
			l1iteminstance1.setAttrEnchantLevel(newAttrEnchantLevel);
			pc.getInventory().updateItem(l1iteminstance1, LsimulatorPcInventory.COL_ATTR_ENCHANT_LEVEL);
			pc.getInventory().saveItem(l1iteminstance1, LsimulatorPcInventory.COL_ATTR_ENCHANT_LEVEL);
		} else {
	...
	private static void SuccessEnchant(PcInstance pc, ItemInstance item, ClientThread client, int i) {
		...
		client.getActiveChar().getInventory().updateItem(item, LsimulatorPcInventory.COL_ENCHANTLVL);

		if (newEnchantLvl > safe_enchant) {
			client.getActiveChar().getInventory().saveItem(item, LsimulatorPcInventory.COL_ENCHANTLVL);
		}
