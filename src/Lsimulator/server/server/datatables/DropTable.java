...
// Referenced classes of package Lsimulator.server.server.templates:
// LsimulatorNpc, LsimulatorItem, ItemTable
...
public class DropTable {
	...
	private final Map<Integer, List<LsimulatorDrop>> _droplists; // モンスター毎のドロップリスト
	...
	private Map<Integer, List<LsimulatorDrop>> allDropList() {
		Map<Integer, List<LsimulatorDrop>> droplistMap = Maps.newMap();
		...
		try {
			...
			while (rs.next()) {
				...
				LsimulatorDrop drop = new LsimulatorDrop(mobId, itemId, min, max, chance, enchantlvl);

				List<LsimulatorDrop> dropList = droplistMap.get(drop.getMobid());
				if (dropList == null) {
	...
	public void setDrop(NpcInstance npc, LsimulatorInventory inventory) {
		...
		for (LsimulatorDrop drop : dropList) {
			...
			if ((adenarate == 0) && (itemId == LsimulatorItemId.ADENA)) {
			...
			if (itemId == LsimulatorItemId.ADENA) { // ドロップがアデナの場合はアデナレートを掛ける
				itemCount *= adenarate;
			}
			if (itemCount < 0) {
				itemCount = 0;
	...
	public void dropShare(NpcInstance npc, List<LsimulatorCharacter> acquisitorList, List<Integer> hateList) {
		...
		for (int i = inventory.getSize(); i > 0; i--) {
			...
			if (((Config.AUTO_LOOT != 0) || (itemId == LsimulatorItemId.ADENA)) && (totalHate > 0)) { // オートルーティングかアデナで取得者がいる場合
				...
				targetInventory = LsimulatorWorld.getInstance().getInventory(npc.getX() + x, npc.getY() + y, npc.getMapId());
				isGround = true;
			}
			if ((itemId >= 40131) && (itemId <= 40135)) {
