...
public class C_Attack extends ClientBasePacket {
	...
	public C_Attack(byte[] decrypt, ClientThread client) {
		...
		if (target instanceof LsimulatorCharacter) {
		...
		if ((target != null)
				&& !((LsimulatorCharacter) target).isDead()) {
			target.onAction(pc);
		} else { // 目標為空或死亡
			LsimulatorCharacter cha = new LsimulatorCharacter();
			cha.setId(targetId);
			cha.setX(x);
			cha.setY(y);
			LsimulatorAttack atk = new LsimulatorAttack(pc, cha);
			atk.actionPc();
		}
	}
}
