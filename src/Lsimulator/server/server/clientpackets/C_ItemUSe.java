...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.COOKING_NOW;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.DECAY_POTION;
...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STATUS_FLOATING_EYE;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STATUS_HOLY_MITHRIL_POWDER;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STATUS_HOLY_WATER;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STATUS_HOLY_WATER_OF_EVA;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.SECRET_MEDICINE_OF_DESTRUCTION;
...
import Lsimulator.server.server.FishingTimeController;
...
import Lsimulator.server.server.model.LsimulatorItemDelay;
...
import Lsimulator.server.server.model.item.action.Enchant;
import Lsimulator.server.server.model.item.action.FurnitureItem;
import Lsimulator.server.server.model.item.action.MagicDoll;
import Lsimulator.server.server.model.item.action.Potion;
...
import Lsimulator.server.server.serverpackets.S_DragonGate;
import Lsimulator.server.server.serverpackets.S_Fishing;
import Lsimulator.server.server.serverpackets.S_IdentifyDesc;
import Lsimulator.server.server.serverpackets.S_ItemName;
import Lsimulator.server.server.serverpackets.S_Letter;
...
import Lsimulator.server.server.serverpackets.S_ShowPolyList;
import Lsimulator.server.server.serverpackets.S_SkillSound;
import Lsimulator.server.server.serverpackets.S_Sound;
import Lsimulator.server.server.serverpackets.S_UseAttackSkill;
import Lsimulator.server.server.serverpackets.S_UseMap;
...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		switch (itemId) {
			case 40088: case 40096: case 49308: case 140088: // 變形卷軸
				s = readS();
				break;
			case LsimulatorItemId.SCROLL_OF_ENCHANT_ARMOR: case LsimulatorItemId.SCROLL_OF_ENCHANT_WEAPON:
			case LsimulatorItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON:
			case 40077: case 40078: case 40126: case 40098: case 40129: case 40130: case 140129: case 140130: 
			case LsimulatorItemId.B_SCROLL_OF_ENCHANT_ARMOR: case LsimulatorItemId.B_SCROLL_OF_ENCHANT_WEAPON:
			case LsimulatorItemId.C_SCROLL_OF_ENCHANT_ARMOR: case LsimulatorItemId.C_SCROLL_OF_ENCHANT_WEAPON:
		...
		if (pc.getCurrentHp() > 0) {
			...
			if (l1iteminstance.getItem().getType2() == 0) { // etcitem
				int delayEffect = ((LsimulatorEtcItem) l1iteminstance.getItem()).get_delayEffect();
				if (delayEffect > 0) {
					isDelayEffect = true;
			...
			if ((itemId == 40077) || (itemId == LsimulatorItemId.SCROLL_OF_ENCHANT_WEAPON) || (itemId == LsimulatorItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON)
					...
					|| (itemId == LsimulatorItemId.C_SCROLL_OF_ENCHANT_WEAPON) || (itemId == 40128)) { // 對武器施法的卷軸
			...
			else if ((itemId == 40078) || (itemId == LsimulatorItemId.SCROLL_OF_ENCHANT_ARMOR) || (itemId == 40129) || (itemId == 140129)
					|| (itemId == LsimulatorItemId.B_SCROLL_OF_ENCHANT_ARMOR) || (itemId == LsimulatorItemId.C_SCROLL_OF_ENCHANT_ARMOR) || (itemId == 40127)) { // 對盔甲施法的卷軸
				...
				int item_minlvl = ((LsimulatorEtcItem) l1iteminstance.getItem()).getMinLevel();
				int item_maxlvl = ((LsimulatorEtcItem) l1iteminstance.getItem()).getMaxLevel();
				...
				if (l1iteminstance.getItem().getType() == 0) { // アロー
					...
					LsimulatorTreasureBox box = LsimulatorTreasureBox.get(itemId);

					if (box != null) {
						if (box.open(pc)) {
							LsimulatorEtcItem temp = (LsimulatorEtcItem) l1iteminstance.getItem();
				...
				// 治癒藥水、濃縮體力恢復劑、象牙塔治癒藥水
				else if ((itemId == LsimulatorItemId.POTION_OF_HEALING) || (itemId == LsimulatorItemId.CONDENSED_POTION_OF_HEALING) || (itemId == 40029)) {
				...
				else if ((itemId == LsimulatorItemId.POTION_OF_EXTRA_HEALING) // 強力治癒藥水、濃縮強力體力恢復劑
						|| (itemId == LsimulatorItemId.CONDENSED_POTION_OF_EXTRA_HEALING)) {
				...
				else if ((itemId == LsimulatorItemId.POTION_OF_GREATER_HEALING) // 終極治癒藥水
						// 濃縮終極體力恢復劑、凝聚的化合物、鮮奶油蛋糕、神秘的體力藥水
						|| (itemId == LsimulatorItemId.CONDENSED_POTION_OF_GREATER_HEALING)
				...
				else if (itemId == LsimulatorItemId.B_POTION_OF_HEALING) { // 受祝福的 治癒藥水
					Potion.UseHeallingPotion(pc, l1iteminstance, 25, 189);
				}
				else if (itemId == LsimulatorItemId.C_POTION_OF_HEALING) { // 受咀咒的 治癒藥水
					Potion.UseHeallingPotion(pc, l1iteminstance, 10, 189);
				}
				else if (itemId == LsimulatorItemId.B_POTION_OF_EXTRA_HEALING) { // 受祝福的 強力治癒藥水
					Potion.UseHeallingPotion(pc, l1iteminstance, 55, 194);
				}
				else if (itemId == LsimulatorItemId.B_POTION_OF_GREATER_HEALING) { // 受祝福的 終極治癒藥水
				...
				else if ((itemId == LsimulatorItemId.POTION_OF_CURE_POISON) || (itemId == 40507)) { // 翡翠藥水
					if (pc.hasSkillEffect(DECAY_POTION)) { // 藥水霜化術狀態
						...
						if (itemId == LsimulatorItemId.POTION_OF_CURE_POISON) {
							pc.getInventory().removeItem(l1iteminstance, 1);
				...
				else if ((itemId == LsimulatorItemId.POTION_OF_HASTE_SELF) || (itemId == LsimulatorItemId.B_POTION_OF_HASTE_SELF
						// 自我加速藥水、受祝福的 自我加速藥水
				...
				else if ((itemId == LsimulatorItemId.POTION_OF_EMOTION_BRAVERY) // 勇敢藥水
						|| (itemId == LsimulatorItemId.B_POTION_OF_EMOTION_BRAVERY) // 受祝福的 勇敢藥水
						|| (itemId == LsimulatorItemId.POTION_OF_REINFORCED_CASE) // 強化勇氣的藥水
						|| (itemId == LsimulatorItemId.W_POTION_OF_EMOTION_BRAVERY)) { // 福利勇敢藥水
					if (pc.isKnight()) { // 騎士
				...
				else if (itemId == LsimulatorItemId.FORBIDDEN_FRUIT) { // 生命之樹果實
				...
				else if ((itemId == LsimulatorItemId.ELVEN_WAFER) // 精靈餅乾
						|| (itemId == LsimulatorItemId.B_ELVEN_WAFER) // 受祝福的 精靈餅乾
						|| (itemId == LsimulatorItemId.W_POTION_OF_FOREST)) { // 福利森林藥水
					if (pc.isElf()) { // 妖精
						Potion.Brave(pc, l1iteminstance, itemId);
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
				...
				else if (itemId == LsimulatorItemId.DEVILS_BLOOD) { // 惡魔之血
					if (pc.isCrown()) { // 王族
						Potion.Brave(pc, l1iteminstance, itemId);
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
				...
				else if (itemId == LsimulatorItemId.COIN_OF_REPUTATION) { // 名譽貨幣
				...
				else if (itemId == LsimulatorItemId.CHOCOLATE_CAKE) { // 巧克力蛋糕
					Potion.ThirdSpeed(pc, l1iteminstance, 600);
				}
				else if ((itemId >= LsimulatorItemId.POTION_OF_EXP_150)
						&& (itemId <= LsimulatorItemId.SCROLL_FOR_ENCHANTING_BATTLE)) { // 150%神力藥水 ~ 強化戰鬥卷軸
				...
				else if ((itemId == LsimulatorItemId.POTION_OF_MANA) // 藍色藥水
						|| (itemId == LsimulatorItemId.B_POTION_OF_MANA // 受祝福的 藍色藥水
						|| (itemId == 40736) || (itemId == 49306))) { // 智慧貨幣、福利藍色藥水
					Potion.useBluePotion(pc, l1iteminstance, itemId);
				}
				else if ((itemId == LsimulatorItemId.POTION_OF_EMOTION_WISDOM) // 慎重藥水
						|| (itemId == LsimulatorItemId.B_POTION_OF_EMOTION_WISDOM) // 受祝福的 慎重藥水
						|| (itemId == 49307)) { // 福利慎重藥水
					if (pc.isWizard()) { // 法師
				...
				else if (itemId == LsimulatorItemId.POTION_OF_BLINDNESS) { // 黑色藥水
					...
					if (l1iteminstance1.getItemId() >= 47053
						...
						if (Config.MAGIC_STONE_TYPE < rnd) {
							...
							LsimulatorItem template = ItemTable.getInstance().getTemplate(newItem);
							if (template == null) {
								pc.sendPackets(new S_ServerMessage(79));
							}
					...
					if (Config.MAGIC_STONE_LEVEL < rnd
						...
						pc.getInventory().updateItem(l1iteminstance1, LsimulatorPcInventory.COL_ITEMID);
						pc.getInventory().saveItem(l1iteminstance1, LsimulatorPcInventory.COL_ITEMID);
					} else {
						pc.sendPackets(new S_ServerMessage(1411, l1iteminstance1.getName())); // 對\f1%0附加魔法失敗。
					...
					for (ItemInstance eachItem : pc.getInventory().getItems()) {
						...
						LsimulatorItem template = ItemTable.getInstance().getTemplate(id_normal);
						if (template == null) {
							continue;
						}
						...
						if (pc.getInventory().checkItem(id_normal) && template.isStackable()) {
							...
							pc.getInventory().updateItem(eachItem, LsimulatorPcInventory.COL_ITEMID);
							pc.getInventory().saveItem(eachItem, LsimulatorPcInventory.COL_ITEMID);
						}
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
					...
					if (!l1iteminstance1.isIdentified()) {
						...
						pc.getInventory().updateItem(l1iteminstance1, LsimulatorPcInventory.COL_IS_ID);
					}
					...
					if ((spellsc_objid == 0) && (l1iteminstance.getItem().getUseType() != 51) && (l1iteminstance.getItem().getUseType() != 26)
						...
						LsimulatorWorld.getInstance().broadcastServerMessage("RETURN " + itemId);
						return;
					...
					l1skilluse.handleCommands(client.getActiveChar(), skillid, spellsc_objid, spellsc_x, spellsc_y, null, 0, LsimulatorSkillUse.TYPE_SPELLSC);
					...
					pc.getInventory().updateItem(l1iteminstance, LsimulatorPcInventory.COL_ITEMID);
					pc.getInventory().saveItem(l1iteminstance, LsimulatorPcInventory.COL_ITEMID);
				}
					...
					if (pc.getMap().isEscapable() || pc.isGm()) {
						...
						if (castle_id != 0) { // 城主クラン員
							if (pc.getMap().isEscapable() || pc.isGm()) {
								int[] loc = new int[3];
								loc = LsimulatorCastleLocation.getCastleLoc(castle_id);
								int locx = loc[0];
								int locy = loc[1];
							...
							if (pc.getMap().isEscapable() || pc.isGm()) {
								int[] loc = new int[3];
								loc = LsimulatorHouseLocation.getHouseLoc(house_id);
								int locx = loc[0];
								int locy = loc[1];
								short mapid = (short) (loc[2]);
								LsimulatorTeleport.teleport(pc, locx, locy, mapid, 5, true);
							...
							if (pc.getHomeTownId() > 0) {
								int[] loc = LsimulatorTownLocation.getGetBackLoc(pc.getHomeTownId());
								int locx = loc[0];
								int locy = loc[1];
								short mapid = (short) (loc[2]);
								LsimulatorTeleport.teleport(pc, locx, locy, mapid, 5, true);
					...
					LsimulatorBookMark bookm = pc.getBookMark(btele);
					if (bookm != null) { // ブックマークを取得出来たらテレポート
						if (pc.getMap().isEscapable() || pc.isGm()) {
							...
							LsimulatorTeleport.teleport(pc, newX, newY, mapId, 5, true);
							// 卷軸傳送後 使用物品延遲完才解開停止狀態
							LsimulatorItemDelay.teleportUnlock(pc, l1iteminstance);
							pc.getInventory().removeItem(l1iteminstance, 1);
						}
						else {
							pc.sendPackets(new S_ServerMessage(79));
					...
					LsimulatorTeleport.teleport(pc, pc.getX(), pc.getY(), pc.getMapId(), pc.getHeading(), true);
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
					...
					if (pc.isKnight() && ((pc.getX() >= 32806) && // オリム部屋
						...
						short mapid = 13;
						LsimulatorTeleport.teleport(pc, 32815, 32810, mapid, 5, false);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if (((pc.getX() >= 32665) && // 海賊島
						...
						short mapid = 430;
						LsimulatorTeleport.teleport(pc, 32922, 32812, mapid, 5, true);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getMapId() != 2004) && (pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50) > 1 )) {
						short mapid = 2004;
						LsimulatorTeleport.teleport(pc, 32723, 32834, mapid, 5, true);
						pc.getInventory().removeItem(l1iteminstance, 1);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					...
					if ((pc.isIllusionist()) && (pc.getMapId() == 2004) && (pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50) > 1 )) {
						short mapid = 1000;
						LsimulatorTeleport.teleport(pc, 32772, 32812, mapid, 5, true);
						pc.getInventory().removeItem(l1iteminstance, 1);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					...
					if ((pc.isDragonKnight()) && (pc.getMapId() == 2004) && (pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50) > 1 )) {
						short mapid = 1001;
						LsimulatorTeleport.teleport(pc, 32817, 32832, mapid, 5, true);
						pc.getInventory().removeItem(l1iteminstance, 1);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					...
					if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50) > 0) {
						if (pc.isCrown() && (pc.getMapId() == 2000)) {
							short mapid = 2000;
							LsimulatorTeleport.teleport(pc, 32860, 32739, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
					...
					if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50) > 0) {
						if (pc.isCrown() && (pc.getMapId() == 2000)) {
							short mapid = 2000;
							LsimulatorTeleport.teleport(pc, 32735, 32748, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isKnight() && (pc.getMapId() == 2001)) {
							short mapid = 2001;
							LsimulatorTeleport.teleport(pc, 32735, 32748, mapid, 5, true);
					...
					if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50) > 0 ) {
						if (pc.isCrown() && (pc.getMapId() == 2000)) {
							short mapid = 2000;
							LsimulatorTeleport.teleport(pc, 32793, 32757, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isKnight() && (pc.getMapId() == 2001)) {
							short mapid = 2001;
							LsimulatorTeleport.teleport(pc, 32793, 32757, mapid, 5, true);
					...
					if (pc.isElf()
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 45300, 0, 0); // 古代人の亡霊
						}
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					...
					if ((pc.getX() == 32620) && (pc.getY() == 32641) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45883, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 32730) && (pc.getY() == 32426) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45884, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 33046) && (pc.getY() == 32806) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45885, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 32580) && (pc.getY() == 33260) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45886, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 33447) && (pc.getY() == 33476) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45887, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 34215) && (pc.getY() == 33195) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45888, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 33513) && (pc.getY() == 32890) && (pc.getMapId() == 4)) {
						...
						LsimulatorSpawnUtil.spawn(pc, 45889, 0, 300000);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					...
					if ((pc.getX() == 32778) && (pc.getY() == 32738) && (pc.getMapId() == 21)) {
						LsimulatorTeleport.teleport(pc, 32781, 32728, (short) 21, 5, true);
					}
					else if ((pc.getX() == 32781) && (pc.getY() == 32728) && (pc.getMapId() == 21)) {
						LsimulatorTeleport.teleport(pc, 32778, 32738, (short) 21, 5, true);
					}
					else {
						pc.sendPackets(new S_ServerMessage(79));
					}
					...
					if (pc.getMap().isUsePainwand()) {
						...
						LsimulatorSpawnUtil.spawn(pc, mobArray[rnd], 0, 300000);
						if ((itemId == 40006) || (itemId == 140006)) {
							l1iteminstance.setChargeCount(l1iteminstance.getChargeCount() - 1);
							pc.getInventory().updateItem(l1iteminstance, LsimulatorPcInventory.COL_CHARGE_COUNT);
					...
					int dmg = 0;
					int[] data = null;
					LsimulatorObject target = LsimulatorWorld.getInstance().findObject(spellsc_objid);
					if (target != null) {
					...
					if ((pc.getMapId() == 63) || (pc.getMapId() == 552) || (pc.getMapId() == 555) || (pc.getMapId() == 557) || (pc.getMapId() == 558)
						...
						LsimulatorObject target = LsimulatorWorld.getInstance().findObject(spellsc_objid);
						if (target != null) {
							LsimulatorCharacter cha = (LsimulatorCharacter) target;
							polyAction(pc, cha);
					...
					if (((pc.getX() >= 32701) && (pc.getX() <= 32705)) && ((pc.getY() >= 32894) && (pc.getY() <= 32898)) && (pc.getMapId() == 522)) { // 影の神殿1F
						LsimulatorTeleport.teleport(pc, ((LsimulatorEtcItem) l1iteminstance.getItem()).get_locx(), ((LsimulatorEtcItem) l1iteminstance.getItem()).get_locy(),
								((LsimulatorEtcItem) l1iteminstance.getItem()).get_mapid(), 5, true);
					}
					else {
						// \f1沒有任何事情發生。
						pc.sendPackets(new S_ServerMessage(79));
					...
					if ( Config.ALT_TALKINGSCROLLQUEST ) {
						...
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 1) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollb"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 2) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollc"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 3) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrolld"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 4) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrolle"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 5) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollf"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 6) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollg"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 7) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollh"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 8) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrolli"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 9) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollj"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 10) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollk"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 11) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrolll"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 12) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrollm"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 13) {
							pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tscrolln"));
						}
						else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_TOSCROLL) == 255) {
					...
					if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 1) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "firsttmap"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 2) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "secondtmapa"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 3) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "secondtmapb"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 4) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "secondtmapc"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 5) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "thirdtmapd"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 6) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "thirdtmape"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 7) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "thirdtmapf"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 8) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "thirdtmapg"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 9) {
						pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "thirdtmaph"));
					}
					else if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LUKEIN1) == 10) {
					...
					if (((pc.getX() >= 32619) && (pc.getX() <= 32623)) && ((pc.getY() >= 33120) && (pc.getY() <= 33124)) && (pc.getMapId() == 440)) { // 海賊島前半魔方陣座標
						...
						if (found) {}
							...
							LsimulatorSpawnUtil.spawn(pc, 45875, 0, 0); // ラバーボーンヘッド
						}
					}
				}
				else if (itemId == 41121) { // カヘルの契約書
					if ((pc.getQuest().get_step(LsimulatorQuest.QUEST_SHADOWS) == LsimulatorQuest.QUEST_END) || pc.getInventory().checkItem(41122, 1)) {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					else {
					...
					if ((pc.getQuest().get_step(LsimulatorQuest.QUEST_DESIRE) == LsimulatorQuest.QUEST_END) || pc.getInventory().checkItem(41131, 1)) {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
					else {
					...
					LsimulatorTeleport.teleport(pc, spellsc_x, spellsc_y, pc.getMapId(), pc.getHeading(), true, LsimulatorTeleport.CHANGE_POSITION);
				}
					...
					int[] loc = new int[2];
					loc = pc.getFrontLoc();
					LsimulatorEffectSpawn.getInstance().spawnEffect(81170, 600000, loc[0], loc[1], pc.getMapId());
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				else if (((itemId >= 41277) && (itemId <= 41292)) || ((itemId >= 49049) && (itemId <= 49064))
						|| ((itemId >= 49244) && (itemId <= 49259))
						|| itemId == LsimulatorItemId.POTION_OF_WONDER_DRUG) { // 魔法料理、象牙塔妙藥
					LsimulatorCooking.useCookingItem(pc, l1iteminstance);
				}
				else if (itemId == 41411) { // 銀のチョンズ
					Potion.UseHeallingPotion(pc, l1iteminstance, 10, 189);
				}
				else if (itemId == 41345) { // 酸性の乳液
					LsimulatorDamagePoison.doInfection(pc, pc, 3000, 5);
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
					...
					if (((lockItem != null) && (lockItem.getItem().getType2() == 1)) || (lockItem.getItem().getType2() == 2)
						...
						if ((lockItem.getBless() == 0) || (lockItem.getBless() == 1) || (lockItem.getBless() == 2) || (lockItem.getBless() == 3)) {
							...
							pc.getInventory().updateItem(lockItem, LsimulatorPcInventory.COL_BLESS);
							pc.getInventory().saveItem(lockItem, LsimulatorPcInventory.COL_BLESS);
							pc.getInventory().removeItem(l1iteminstance, 1);
						}
						else {
							pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					...
					if (pc.isDragonKnight() && (pc.getMapId() == 61)) { 
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 46161, 0, 0); // オーク
																// 密使リーダー
						}
					...
					if (pc.isIllusionist() && (pc.getMapId() == 4)) { // 古魯丁祭壇
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 46163, 0, 0);
						}
					...
					if (pc.isIllusionist() && (pc.getMapId() == 4)) { // 火龍窟
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81254, 0, 0);
						}
					...
					if (pc.isIllusionist() && (pc.getMapId() == 2004)) { // 異界 奎斯特
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81313, 0, 0);
						}
					...
					if (pc.isDragonKnight() && (pc.getMapId() == 2004)) { // 異界 奎斯特
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81312, 0, 0);
						}
					...
					if (pc.isCrown() && (pc.getMapId() == 2000) && (pc.getX() == 32807) && (pc.getY() == 32773)) {
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81323, 0, 0);
						}
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81324, 0, 0);
						}
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81325, 0, 0);
						}
						...
						if (found) {
							...
							LsimulatorSpawnUtil.spawn(pc, 81326, 0, 0);
						}
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					} 
					...
					if (!LsimulatorCastleLocation.checkInAllWarArea(pc.getLocation())) {// 檢查是否在城堡區域內
						pc.sendPackets(new S_DragonGate(pc ,LsimulatorDragonSlayer.getInstance().checkDragonPortal()));
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // 沒有任何事情發生
					}
				}
				else {
					int locX = ((LsimulatorEtcItem) l1iteminstance.getItem()).get_locx();
					int locY = ((LsimulatorEtcItem) l1iteminstance.getItem()).get_locy();
					short mapId = ((LsimulatorEtcItem) l1iteminstance.getItem()).get_mapid();
					if ((locX != 0) && (locY != 0)) { // 各種テレポートスクロール
						if (pc.getMap().isEscapable() || pc.isGm()) {
							LsimulatorTeleport.teleport(pc, locX, locY, mapId, pc.getHeading(), true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						}
						else {
							pc.sendPackets(new S_ServerMessage(647));
				...
				if ((pc.isCrown() && l1iteminstance.getItem().isUseRoyal()) || (pc.isKnight() && l1iteminstance.getItem().isUseKnight())
					...
					int min = ((LsimulatorArmor) l1iteminstance.getItem()).getMinLevel();
					int max = ((LsimulatorArmor) l1iteminstance.getItem()).getMaxLevel();
					if ((min != 0) && (min > pc.getLevel())) {
						// 等級 %0以上才可使用此道具。
						pc.sendPackets(new S_ServerMessage(318, String.valueOf(min)));
					}
			...
			if (isDelayEffect) {
				...
				pc.getInventory().updateItem(l1iteminstance, LsimulatorPcInventory.COL_DELAY_EFFECT);
				pc.getInventory().saveItem(l1iteminstance, LsimulatorPcInventory.COL_DELAY_EFFECT);
			}

			LsimulatorItemDelay.onItemUse(client, l1iteminstance); // アイテムディレイ開始
		}
	}
	...
	private boolean usePolyScroll(PcInstance pc, int item_id, String s) {
		...
		if ((poly != null) || s.equals("")) {
			if (s.equals("")) {
				...
				LsimulatorPolyMorph.doPoly(pc, poly.getPolyId(), time, LsimulatorPolyMorph.MORPH_BY_ITEMMAGIC);
				return true;
			}
		}
		return false;
	...
	private void UseArmor(PcInstance activeChar, ItemInstance armor) {
		...
		if (equipeSpace && !armor.isEquipped()) { // 使用した防具を装備していなくて、その装備箇所が空いている場合（装着を試みる）
			...
			if (!LsimulatorPolyMorph.isEquipableArmor(polyid, type)) { // その変身では装備不可
				return;
			}
	...
	private void UseWeapon(PcInstance activeChar, ItemInstance weapon) {
		...
		if ((activeChar.getWeapon() == null) || !activeChar.getWeapon().equals(weapon)) { // 指定された武器が装備している武器と違う場合、装備できるか確認
			...
			if (!LsimulatorPolyMorph.isEquipableWeapon(polyid, weapon_type)) { // その変身では装備不可
				return;
			}
	...
	private void SpellBook1(PcInstance pc, ItemInstance l1iteminstance, ClientThread clientthread) {
		...
		for (int j6 = 97; j6 < 112; j6++) {
			LsimulatorSkills l1skills = SkillsTable.getInstance().getTemplate(j6);
			String s1 = null;
			if (Config.CLIENT_LANGUAGE == 3) {
	...
	private void polyAction(PcInstance attacker, LsimulatorCharacter cha) {
		...
		if (cha instanceof PcInstance) {
			...
			if (pc.getInventory().checkEquipped(20281)) { // 裝備變形控制戒指
				...
				LsimulatorSkills skillTemp = SkillsTable.getInstance().getTemplate(SHAPE_CHANGE);
			...
			if (mob.getLevel() < 50) {
				...
				if ((npcId != 45338) && (npcId != 45370) && (npcId != 45456 // クロコダイル、バンディットボス、ネクロマンサー
					...
					LsimulatorSkills skillTemp = SkillsTable.getInstance().getTemplate(SHAPE_CHANGE);
					LsimulatorPolyMorph.doPoly(mob, polyId, skillTemp.getBuffDuration(), LsimulatorPolyMorph.MORPH_BY_ITEMMAGIC);
				}
			}
		}
	}
	...
	private void useToiTeleportAmulet(PcInstance pc, int itemId, ItemInstance item) {
		...
		if (isTeleport) {
			LsimulatorTeleport.teleport(pc, item.getItem().get_locx(), item.getItem().get_locy(), item.getItem().get_mapid(), 5, true);
		}
		else {
			pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
		}
	...
	private void useResolvent(PcInstance pc, ItemInstance item, ItemInstance resolvent) {
		...
		if (crystalCount != 0) {
			...
			if (pc.getInventory().checkAddItem(crystal, 1) == LsimulatorInventory.OK) {
				...
				LsimulatorWorld.getInstance().getInventory(pc.getX(), pc.getY(), pc.getMapId()).storeItem(crystal);
			}
		}
		pc.getInventory().removeItem(item, 1);
