...
public class NpcInstance extends LsimulatorCharacter {
	...
	public void serchLink(PcInstance targetPlayer, int family) {
		List<LsimulatorObject> targetKnownObjects = targetPlayer.getKnownObjects();
		for (Object knownObject : targetKnownObjects) {
			if (knownObject instanceof NpcInstance) {
				NpcInstance npc = (NpcInstance) knownObject;
	...
	public void attackTarget(LsimulatorCharacter target) {
		if (target instanceof PcInstance) {
			PcInstance player = (PcInstance) target;
		...
		if (this instanceof PetInstance) {
			PetInstance pet = (PetInstance) this;
			LsimulatorCharacter cha = pet.getMaster();
			if (cha instanceof PcInstance) {
				PcInstance player = (PcInstance) cha;
				if (player.isTeleport()) { // テレポート処理中
		...
		} else if (this instanceof SummonInstance) {
			SummonInstance summon = (SummonInstance) this;
			LsimulatorCharacter cha = summon.getMaster();
			if (cha instanceof PcInstance) {
				PcInstance player = (PcInstance) cha;
				if (player.isTeleport()) { // テレポート処理中
	...
	// アイテムを拾う
	public void pickupTargetItem(ItemInstance targetItem) {
		...
		ItemInstance item = groundInventory.tradeItem(targetItem,
		...
		if ((_master != null)
			...
			if ((_master == null) && (getPassispeed() > 0) && !isRest()) {
				...
				if ((mobGroupInfo == null)
					...
					NpcInstance leader = mobGroupInfo.getLeader();
	...
	public void onFinalAction(PcInstance pc, String s) {
	}
		...
		if (_npcTemplate.get_npcId() == 45032) { // ブロッブ
			...
			if ((getExp() != 0) && !_inventory.checkItem(89)) {
				...
				if (_inventory.checkItem(materials, counts)) {
					...
					for (int j = 0; j < createitem.length; j++) {
						ItemInstance item = _inventory.storeItem(
	...
	protected List<ItemInstance> _targetItemList = Lists.newList(); // ダーゲットアイテム一覧

	protected LsimulatorCharacter _target = null; // 現在のターゲット

	protected ItemInstance _targetItem = null; // 現在のターゲットアイテム
			...
			while (!_destroyed && (_digestItems.size() > 0)) {
				...
				for (Object key2 : keys) {
					...
					if (digestCounter <= 0) {
						...
						ItemInstance digestItem = getInventory().getItem(key);
						if (digestItem != null) {
	...
	public NpcInstance(LsimulatorNpc template) {
		setStatus(0);
		setMoveSpeed(0);
		setDead(false);
		setreSpawn(false);
	...
	public void deleteMe() {
		...
		List<PcInstance> players = LsimulatorWorld.getInstance().getRecognizePlayer(
				this);
		if (players.size() > 0) {
	...
	public void setDigestItem(ItemInstance item) {
	...
	public void approachPlayer(PcInstance pc) {
	...
	public void appearOnGround(PcInstance pc) { // 怪物解除遁地、飛天、冰凍
		if (getHiddenStatus() == HIDDEN_STATUS_SINK) {
			setHiddenStatus(HIDDEN_STATUS_NONE);
			setStatus(LsimulatorNpcDefaultAction.getInstance().getStatus(getTempCharGfx()));
		...
		if (dir >= 0) {
			...
			if (getMovementDistance() > 0) {
				if ((this instanceof GuardInstance)
						|| (this instanceof MerchantInstance)
	...
	private boolean isExsistCharacterBetweenTarget(int dir) {
		if (!(this instanceof MonsterInstance)) { // モンスター以外は対象外
			return false;
		}
	...
	private int _serchCource(int x, int y) // 目標点Ｘ 目標点Ｙ
	{
		...
		boolean serchMap[][] = new boolean[ ( locCenter << 1) + 1][ ( locCenter << 1)  + 1];
		LinkedList<int[]> queueSerch = new LinkedList<int[]>();

		// 探索用マップの設定
		for (int j = ( courceRange << 1 ) + 1; j > 0; j--) {
	...
	private void useHealPotion(int healHp, int effectId) {
		...
		if (this instanceof PetInstance) {
			((PetInstance) this).setCurrentHp(getCurrentHp() + healHp);
		} else if (this instanceof SummonInstance) {
			((SummonInstance) this).setCurrentHp(getCurrentHp() + healHp);
		} else {
	...
	protected static class DeleteTimer extends TimerTask {
		...
		protected DeleteTimer(int oId) {
			_id = oId;
			if (!(LsimulatorWorld.getInstance().findObject(_id) instanceof NpcInstance)) {
		...
		@Override
		public void run() {
			NpcInstance npc = (NpcInstance) LsimulatorWorld.getInstance()
					.findObject(_id);
