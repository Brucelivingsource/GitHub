...
import Lsimulator.server.server.serverpackets.S_CharAmount;
import Lsimulator.server.server.serverpackets.S_CharPacks;
import Lsimulator.server.server.serverpackets.S_CharSynAck;
 ...
 * 處理自動登入後的角色列表封包
 */
public class LsimulatorCharList {
	private static Logger _log = Logger.getLogger(LsimulatorCharList.class.getName());

	public LsimulatorCharList(ClientThread client) {
	...
	private void deleteCharacter(ClientThread client) {
		...
		try {
			...
			conn = LsimulatorDatabaseFactory.getInstance().getConnection();
