...
public class LsimulatorSummonInstance extends LsimulatorNpcInstance {
	...
	// サモンモンスター用
	public LsimulatorSummonInstance(LsimulatorNpc template, LsimulatorCharacter master) {
		super(template);
	...
	// 造屍術處理
	public LsimulatorSummonInstance(LsimulatorNpcInstance target, LsimulatorCharacter master, boolean isCreateZombie) {
		super(null);
		setId(IdFactory.getInstance().nextId());

		if (isCreateZombie) { // クリエイトゾンビ
			int npcId = 45065;
			LsimulatorPcInstance pc = (LsimulatorPcInstance) master;
		...
		if ((target instanceof LsimulatorMonsterInstance) && !((LsimulatorMonsterInstance) target).is_storeDroped()) {
		...
		// ペットが攻撃中だった場合止めさせる
		for (LsimulatorNpcInstance each : master.getPetList().values()) {
		...
		if (!isDead()) {
			...
			for (LsimulatorItemInstance item : items) {
				if (_master.getInventory().checkAddItem( // 容量重量確認及びメッセージ送信
					...
					// \f1%0が%1をくれました。
					((LsimulatorPcInstance) _master).sendPackets(new S_ServerMessage(
	...
	public synchronized void deleteMe() {
		...
		//if (_master.getPetList().isEmpty()) {
			...
			if (pc instanceof LsimulatorPcInstance) {
	...
	public void onFinalAction(LsimulatorPcInstance player, String action) {
		...
		if (status == 6) {
			...
			for (Object petObject : petList) {
				...
				} else if (petObject instanceof LsimulatorPetInstance) { // ペット
					LsimulatorPetInstance pet = (LsimulatorPetInstance) petObject;
					if ((player != null)
	...
	public void setCurrentHp(int i) {
		...
		if (_master instanceof LsimulatorPcInstance) {
			...
			LsimulatorPcInstance Master = (LsimulatorPcInstance) _master;
