 ...
 * 
 */
package Lsimulator.server.telnet.command;
...
import static Lsimulator.server.telnet.command.TelnetCommandResult.*;
...
class CharStatusCommand implements TelnetCommand {
	...
	public TelnetCommandResult execute(String args) {
		...
		LsimulatorObject obj = LsimulatorWorld.getInstance().findObject(id);
		if (obj == null) {
...
class GlobalChatCommand implements TelnetCommand {
	...
	public TelnetCommandResult execute(String args) {
		...
		LsimulatorWorld.getInstance().broadcastPacketToAll(new S_ChatPacket(pc, text, Opcodes.S_OPCODE_GLOBALCHAT, 3));
