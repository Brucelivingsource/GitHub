...
// LsimulatorUltimateBattle

public class LsimulatorUltimateBattle {
	private int _locX;

	private int _locY;

	private LsimulatorLocation _location; // 中心点
	...
	private void spawnSupplies(int curRound) {
		if (curRound == 1) {
			spawnGroundItem(LsimulatorItemId.ADENA, 1000, 60);
			spawnGroundItem(LsimulatorItemId.POTION_OF_CURE_POISON, 3, 20);
			spawnGroundItem(LsimulatorItemId.POTION_OF_EXTRA_HEALING, 5, 20);
			spawnGroundItem(LsimulatorItemId.POTION_OF_GREATER_HEALING, 3, 20);
			spawnGroundItem(40317, 1, 5); // 砥石
			spawnGroundItem(40079, 1, 20); // 帰還スク
		}
		else if (curRound == 2) {
			spawnGroundItem(LsimulatorItemId.ADENA, 5000, 50);
			spawnGroundItem(LsimulatorItemId.POTION_OF_CURE_POISON, 5, 20);
			spawnGroundItem(LsimulatorItemId.POTION_OF_EXTRA_HEALING, 10, 20);
			spawnGroundItem(LsimulatorItemId.POTION_OF_GREATER_HEALING, 5, 20);
			spawnGroundItem(40317, 1, 7); // 砥石
			spawnGroundItem(40093, 1, 10); // ブランクスク(Lv4)
			spawnGroundItem(40079, 1, 5); // 帰還スク
		}
		else if (curRound == 3) {
			spawnGroundItem(LsimulatorItemId.ADENA, 10000, 30);
			spawnGroundItem(LsimulatorItemId.POTION_OF_CURE_POISON, 7, 20);
			spawnGroundItem(LsimulatorItemId.POTION_OF_EXTRA_HEALING, 20, 20);
			spawnGroundItem(LsimulatorItemId.POTION_OF_GREATER_HEALING, 10, 20);
	...
	private void spawnGroundItem(int itemId, int stackCount, int count) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (temp.isStackable()) {
				...
				if (ground.checkAddItem(item, stackCount) == LsimulatorInventory.OK) {
	...
	private void clearColosseum() {
		for (Object obj : LsimulatorWorld.getInstance().getVisibleObjects(_mapId).values()) {
			...
			else if (obj instanceof LsimulatorInventory) // アイテム削除
			{
				LsimulatorInventory inventory = (LsimulatorInventory) obj;
	 ...
	 * コンストラクタ。
	 */
	public LsimulatorUltimateBattle() {
	}
		...
		public void run() {
			try {
				...
				for (int round = 1; round <= 4; round++) {
					...
					LsimulatorUbPattern pattern = UBSpawnTable.getInstance().getPattern(_ubId, _pattern);

					List<LsimulatorUbSpawn> spawnList = pattern.getSpawnList(round);

					for (LsimulatorUbSpawn spawn : spawnList) {
