...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.SHADOW_FANG;
...
import Lsimulator.server.server.model.LsimulatorEquipmentTimer;
import Lsimulator.server.server.model.LsimulatorItemOwnerTimer;
import Lsimulator.server.server.model.LsimulatorObject;
import Lsimulator.server.server.model.LsimulatorPcInventory;
import Lsimulator.server.server.model.identity.LsimulatorArmorId;
...
import Lsimulator.server.server.utils.BinaryOutputStream;

// Referenced classes of package Lsimulator.server.server.model:
// LsimulatorObject, LsimulatorPcInstance

public class LsimulatorItemInstance extends LsimulatorObject {
	...
	public LsimulatorItemInstance() {
		_count = 1;
		_enchantLevel = 0;
	}

	public LsimulatorItemInstance(LsimulatorItem item, int count) {
		this();
		setItem(item);
		setCount(count);
	}
	...
	public LsimulatorItem getItem() {
		return _item;
	}

	public void setItem(LsimulatorItem item) {
		_item = item;
	...
	public int getMr() {
		...
		if ((getItemId() == LsimulatorArmorId.HELMET_OF_MAGIC_RESISTANCE) || (getItemId() == LsimulatorArmorId.CHAIN_MAIL_OF_MAGIC_RESISTANCE) // 抗魔法頭盔、抗魔法鏈甲
				|| (getItemId() >= LsimulatorArmorId.ELITE_PLATE_MAIL_OF_LINDVIOR && getItemId() <= LsimulatorArmorId.ELITE_SCALE_MAIL_OF_LINDVIOR) // 林德拜爾的力量、林德拜爾的魅惑、林德拜爾的泉源、林德拜爾的霸氣
				|| (getItemId() == LsimulatorArmorId.B_HELMET_OF_MAGIC_RESISTANCE)) { // 受祝福的 抗魔法頭盔
			mr += getEnchantLevel();
		}
		if ((getItemId() == LsimulatorArmorId.CLOAK_OF_MAGIC_RESISTANCE) || (getItemId() == LsimulatorArmorId.B_CLOAK_OF_MAGIC_RESISTANCE) // 抗魔法斗篷、受祝福的 抗魔法斗篷
				|| (getItemId() == LsimulatorArmorId.C_CLOAK_OF_MAGIC_RESISTANCE)) { // 受咀咒的 抗魔法斗篷
	...
	public int getRecordingColumns() {
		...
		if (getCount() != _lastStatus.count) {
			column += LsimulatorPcInventory.COL_COUNT;
		}
		if (getItemId() != _lastStatus.itemId) {
			column += LsimulatorPcInventory.COL_ITEMID;
		}
		if (isEquipped() != _lastStatus.isEquipped) {
			column += LsimulatorPcInventory.COL_EQUIPPED;
		}
		if (getEnchantLevel() != _lastStatus.enchantLevel) {
			column += LsimulatorPcInventory.COL_ENCHANTLVL;
		}
		if (get_durability() != _lastStatus.durability) {
			column += LsimulatorPcInventory.COL_DURABILITY;
		}
		if (getChargeCount() != _lastStatus.chargeCount) {
			column += LsimulatorPcInventory.COL_CHARGE_COUNT;
		}
		if (getLastUsed() != _lastStatus.lastUsed) {
			column += LsimulatorPcInventory.COL_DELAY_EFFECT;
		}
		if (isIdentified() != _lastStatus.isIdentified) {
			column += LsimulatorPcInventory.COL_IS_ID;
		}
		if (getRemainingTime() != _lastStatus.remainingTime) {
			column += LsimulatorPcInventory.COL_REMAINING_TIME;
		}
		if (getBless() != _lastStatus.bless) {
			column += LsimulatorPcInventory.COL_BLESS;
		}
		if (getAttrEnchantKind() != _lastStatus.attrEnchantKind) {
			column += LsimulatorPcInventory.COL_ATTR_ENCHANT_KIND;
		}
		if (getAttrEnchantLevel() != _lastStatus.attrEnchantLevel) {
			column += LsimulatorPcInventory.COL_ATTR_ENCHANT_LEVEL;
		}
	...
	public int getRecordingColumnsEnchantAccessory() {
		...
		if (getaddHp() != _lastStatus.addhp) {
			column += LsimulatorPcInventory.COL_ADDHP;
		}
		if (getaddMp() != _lastStatus.addmp) {
			column += LsimulatorPcInventory.COL_ADDMP;
		}
		if (getHpr() != _lastStatus.hpr) {
			column += LsimulatorPcInventory.COL_HPR;
		}
		if (getMpr() != _lastStatus.mpr) {
			column += LsimulatorPcInventory.COL_MPR;
		}
		if (getaddSp() != _lastStatus.addsp) {
			column += LsimulatorPcInventory.COL_ADDSP;
		}
		if (getM_Def() != _lastStatus.m_def) {
			column += LsimulatorPcInventory.COL_M_DEF;
		}
		if (getEarthMr() != _lastStatus.earthmr) {
			column += LsimulatorPcInventory.COL_EARTHMR;
		}
		if (getFireMr() != _lastStatus.firemr) {
			column += LsimulatorPcInventory.COL_FIREMR;
		}
		if (getWaterMr() != _lastStatus.watermr) {
			column += LsimulatorPcInventory.COL_WATERMR;
		}
		if (getWindMr() != _lastStatus.windmr) {
			column += LsimulatorPcInventory.COL_WINDMR;
		}
	...
	public String getNumberedViewName(int count) {
		...
		if ((itemId == 40314) || (itemId == 40316)) { // ペットのアミュレット
			LsimulatorPet pet = PetTable.getInstance().getTemplate(getId());
			if (pet != null) {
				LsimulatorNpc npc = NpcTable.getInstance().getTemplate(pet.get_npcid());
	...
	public byte[] getStatusBytes() {
		...
		if (petItem != null) { // 寵物裝備
			...
			if (itemType2 == 1) { // weapon
				...
				// AC
				os.writeC(19);
				int ac = ((LsimulatorArmor) getItem()).get_ac();
				if (ac < 0) {
					ac = ac - ac - ac;
				}
				os.writeC(ac);
	...
	public void setSkillArmorEnchant(LsimulatorPcInstance pc, int skillId, int skillTime) {
		int type = getItem().getType();
		int type2 = getItem().getType2();
		if (_isRunning) {
			_timer.cancel();
	...
	public void setSkillWeaponEnchant(LsimulatorPcInstance pc, int skillId,
			int skillTime) {
	...
	public void startItemOwnerTimer(LsimulatorPcInstance pc) {
		setItemOwnerId(pc.getId());
		LsimulatorItemOwnerTimer timer = new LsimulatorItemOwnerTimer(this, 10000);
		timer.begin();
	}

	private LsimulatorEquipmentTimer _equipmentTimer;

	public void startEquipmentTimer(LsimulatorPcInstance pc) {
		if (getRemainingTime() > 0) {
			_equipmentTimer = new LsimulatorEquipmentTimer(pc, this);
			Timer timer = new Timer(true);
			timer.scheduleAtFixedRate(_equipmentTimer, 1000, 1000);
		}
	}

	public void stopEquipmentTimer(LsimulatorPcInstance pc) {
		if (getRemainingTime() > 0) {
