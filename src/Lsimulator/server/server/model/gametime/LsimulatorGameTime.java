...
public class LsimulatorGameTime {
	...
	private LsimulatorGameTime(int time) {
		_time = time;
		_calendar = makeCalendar(time);
	}

	public static LsimulatorGameTime valueOf(long timeMillis) {
		...
		return new LsimulatorGameTime(t2 - t3);
	}

	public static LsimulatorGameTime fromSystemCurrentTime() {
		return LsimulatorGameTime.valueOf(System.currentTimeMillis());
	}

	public static LsimulatorGameTime valueOfGameTime(Time time) {
		long t = time.getTime() + TimeZone.getDefault().getRawOffset();
		return new LsimulatorGameTime((int) (t / 1000L));
	}
