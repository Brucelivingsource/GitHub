...
// LsimulatorNpcInstance, LsimulatorTeleport, LsimulatorNpcTalkData, LsimulatorPcInstance,
// LsimulatorTeleporterPrices, LsimulatorTeleportLocations

public class LsimulatorTeleporterInstance extends LsimulatorNpcInstance {
	...
	public LsimulatorTeleporterInstance(LsimulatorNpc template) {
		super(template);
	}
	...
	private void doFinalAction(LsimulatorPcInstance player, String action) {
		int objid = getId();
		...
		if (isTeleport) { // テレポート実行
			try {
				...
				if (action.equalsIgnoreCase("teleport mutant-dungen")) {
					// 3マス以内のPc
					for (LsimulatorPcInstance otherPc : LsimulatorWorld.getInstance()
				...
				// 試練のダンジョン（ウィザードLv30クエスト）
					...
					LsimulatorPcInstance kni = null;
					LsimulatorPcInstance elf = null;
					LsimulatorPcInstance wiz = null;
					// 3マス以内のPc
					for (LsimulatorPcInstance otherPc : LsimulatorWorld.getInstance()
							.getVisiblePlayer(player, 3)) {
	...
	private static Logger _log = Logger
			.getLogger(Lsimulator.server.server.model.Instance.LsimulatorTeleporterInstance.class
					.getName());
