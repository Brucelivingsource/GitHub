...
public class LsimulatorNpcMakeItemAction extends LsimulatorNpcXmlAction {
	private final List<LsimulatorObjectAmount<Integer>> _materials = Lists.newList();

	private final List<LsimulatorObjectAmount<Integer>> _items = Lists.newList();

	private final boolean _isAmountInputable;

	private final LsimulatorNpcAction _actionOnSucceed;

	private final LsimulatorNpcAction _actionOnFail;

	public LsimulatorNpcMakeItemAction(Element element) {
		super(element);

		_isAmountInputable = LsimulatorNpcXmlParser.getBoolAttribute(element, "AmountInputable", true);
		...
		for (Element elem : new IterableElementList(list)) {
			if (elem.getNodeName().equalsIgnoreCase("Material")) {
				...
				_materials.add(new LsimulatorObjectAmount<Integer>(id, amount));
				continue;
			}
			if (elem.getNodeName().equalsIgnoreCase("Item")) {
				int id = Integer.valueOf(elem.getAttribute("ItemId"));
				int amount = Integer.valueOf(elem.getAttribute("Amount"));
				_items.add(new LsimulatorObjectAmount<Integer>(id, amount));
				continue;
			}
		}
		...
		Element elem = LsimulatorNpcXmlParser.getFirstChildElementByTagName(element, "Succeed");
		_actionOnSucceed = elem == null ? null : new LsimulatorNpcListedAction(elem);
		elem = LsimulatorNpcXmlParser.getFirstChildElementByTagName(element, "Fail");
	...
	private boolean makeItems(LsimulatorPcInstance pc, String npcName, int amount) {
		...
		for (LsimulatorObjectAmount<Integer> material : _materials) {
			if (!pc.getInventory().checkItemNotEquipped(material.getObject(), material.getAmount() * amount)) {
				LsimulatorItem temp = ItemTable.getInstance().getTemplate(material.getObject());
		...
		for (LsimulatorObjectAmount<Integer> makingItem : _items) {
			LsimulatorItem temp = ItemTable.getInstance().getTemplate(makingItem.getObject());
			if (temp.isStackable()) {
	 ...
	 */
	private int countNumOfMaterials(LsimulatorPcInventory inv) {
	...
	public LsimulatorNpcHtml executeWithAmount(String actionName, LsimulatorPcInstance pc, LsimulatorObject obj, int amount) {
		...
		return result == null ? LsimulatorNpcHtml.HTML_CLOSE : result;
	}
