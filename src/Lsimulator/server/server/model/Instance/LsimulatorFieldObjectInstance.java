...
public class LsimulatorFieldObjectInstance extends LsimulatorNpcInstance {

	private static final long serialVersionUID = 1L;

	public LsimulatorFieldObjectInstance(LsimulatorNpc template) {
		super(template);
	}
		...
		if (getNpcTemplate().get_npcId() == 81171) { // おばけ屋敷のゴールの炎
			if (LsimulatorHauntedHouse.getInstance().getHauntedHouseStatus() == LsimulatorHauntedHouse.STATUS_PLAYING) {
				...
				if (winnersCount == goalCount + 1) {
					LsimulatorItemInstance item = ItemTable.getInstance().createItem(49280); // 勇者のパンプキン袋(銅)
					int count = 1;
					if (item != null) {
