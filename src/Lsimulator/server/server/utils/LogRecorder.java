...
public class LogRecorder {
	...
	public static void writeWeaponLog(String messenge, LsimulatorPcInstance pc) {
		try {
			...
			out.write("來自帳號: " + pc.getAccountName() 
	...
	public static void writeArmorLog(String messenge, LsimulatorPcInstance pc) {
		try {
			...
			out.write("\r\n");// 每次填寫資料都控一行
			out.write("來自帳號: " + pc.getAccountName() + ", 來自玩家: "
					+ pc.getName() + ", " + messenge + "。" + "\r\n");
			out.close();
		} catch (IOException e) {
			System.out.println("以下是錯誤訊息: " + e.getMessage());
	...
	public static void writeTradeLog(LsimulatorPcInstance sender,
		...
		try {
			...
			out.write("\r\n");// 每次填寫資料都控一行
			out.write("來自帳號: " + sender.getAccountName() 
					+ ", 來自玩家: "+ sender.getName() 
					+ ",將  +"+ l1iteminstance1.getEnchantLevel()+ l1iteminstance1.getName() 
					+ "("+ l1iteminstance1.getCount() + "個)," 
					+ "交易給 " + " 帳號:"+ receiver.getAccountName() 
	...
	public static void writeRobotsLog(LsimulatorPcInstance player) {
		try {
			...
			out.write("\r\n");// 每次填寫資料都控一行
			out.write("加速器紀錄 → 來自帳號: " + player.getAccountName() 
					+ ", 來自玩家: "+ player.getName() 
					+ ", 〈時間〉" + TimeInform.getNowTime(3, 0)
					+ "\r\n");
			out.close();
		} catch (IOException e) {
			System.out.println("以下是錯誤訊息: " + e.getMessage());
	...
	public static void writeDropLog(LsimulatorPcInstance player, LsimulatorItemInstance item) {
		try {
			...
			out.write("來自帳號: " + player.getAccountName() 
					...
					+ ",地點: " + item.getLocation()
					+ ",〈時間〉" + TimeInform.getNowTime(3, 0)
					+ ",丟棄了: " + item.getCount() + " 個 " + item.getName()
					+ ",是否受祝福: " + item.getBless() 
					+ ",加成值: "+ item.getEnchantLevel() 
					+ ",屬性強化類型: "+item.getAttrEnchantKind()
					+ ",屬性強化等級: "+item.getAttrEnchantLevel()
					+ "\r\n");
			out.close();
		} catch (IOException e) {
			System.out.println("以下是錯誤訊息: " + e.getMessage());
