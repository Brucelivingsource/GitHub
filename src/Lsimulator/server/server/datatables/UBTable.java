...
public class UBTable {
	...
	private Map<Integer, LsimulatorUltimateBattle> _ub = Maps.newMap();
	...
	private void loadTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				LsimulatorUltimateBattle ub = new LsimulatorUltimateBattle();
		...
		try {
			...
			while (rs.next()) {
				LsimulatorUltimateBattle ub = getUb(rs.getInt("ub_id"));
				if (ub != null) {
	...
	public LsimulatorUltimateBattle getUb(int ubId) {
		return _ub.get(ubId);
	}

	public Collection<LsimulatorUltimateBattle> getAllUb() {
		return Collections.unmodifiableCollection(_ub.values());
	}

	public LsimulatorUltimateBattle getUbForNpcId(int npcId) {
		for (LsimulatorUltimateBattle ub : _ub.values()) {
