...
public class C_War extends ClientBasePacket {
	...
	public C_War(byte abyte0[], ClientThread clientthread) throws Exception {
		...
		LsimulatorClan enemyClan = null;
		String enemyClanName = null;
		for (LsimulatorClan checkClan : LsimulatorWorld.getInstance().getAllClans()) { // 取得所有的血盟
		...
		boolean inWar = false;
		List<LsimulatorWar> warList = LsimulatorWorld.getInstance().getWarList(); // 取得所有的盟戰
		for (LsimulatorWar war : warList) {
