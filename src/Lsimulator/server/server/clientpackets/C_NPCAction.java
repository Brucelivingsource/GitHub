...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.BLESSED_ARMOR;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.CANCELLATION;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.EFFECT_BLESS_OF_CRAY;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.EFFECT_BLESS_OF_SAELL;
...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STATUS_CURSE_BARLOG;
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.STATUS_CURSE_YAHEE;
...
import Lsimulator.server.server.model.LsimulatorPetMatch;
...
import Lsimulator.server.server.serverpackets.S_ApplyAuction;
import Lsimulator.server.server.serverpackets.S_AuctionBoardRead;
import Lsimulator.server.server.serverpackets.S_CharReset;
import Lsimulator.server.server.serverpackets.S_DelSkill;
import Lsimulator.server.server.serverpackets.S_Deposit;
import Lsimulator.server.server.serverpackets.S_Drawal;
import Lsimulator.server.server.serverpackets.S_HPUpdate;
import Lsimulator.server.server.serverpackets.S_HouseMap;
import Lsimulator.server.server.serverpackets.S_HowManyKey;
...
import Lsimulator.server.server.serverpackets.S_PetList;
import Lsimulator.server.server.serverpackets.S_PledgeWarehouseHistory;
...
import Lsimulator.server.server.serverpackets.S_SelectTarget;
import Lsimulator.server.server.serverpackets.S_SellHouse;
import Lsimulator.server.server.serverpackets.S_ServerMessage;
import Lsimulator.server.server.serverpackets.S_ShopBuyList;
import Lsimulator.server.server.serverpackets.S_ShopSellList;
import Lsimulator.server.server.serverpackets.S_SkillHaste;
import Lsimulator.server.server.serverpackets.S_SkillIconAura;
import Lsimulator.server.server.serverpackets.S_SkillSound;
import Lsimulator.server.server.serverpackets.S_SystemMessage;
import Lsimulator.server.server.serverpackets.S_TaxRate;
...
public class C_NPCAction extends ClientBasePacket {
	...
	public C_NPCAction(byte abyte0[], ClientThread client) throws Exception {
		...
		if (obj != null) {
			if (obj instanceof LsimulatorNpcInstance) {
				...
				if (target.isShapeChange()) {
					LsimulatorPolyMorph.handleCommands(target, s);
		...
		// XML化されたアクション
		LsimulatorNpcAction action = NpcActionTable.getInstance().get(s, pc, obj);
		if (action != null) {
			LsimulatorNpcHtml result = action.execute(s, pc, obj, readByte());
			if (result != null) {
		...
		if (s.equalsIgnoreCase("buy")) {
			...
			if (pc.getLevel() >= 5) {
				...
				if ((rank != LsimulatorClan.CLAN_RANK_PROBATION)
						&& (rank != LsimulatorClan.CLAN_RANK_GUARDIAN)
						&& (rank != LsimulatorClan.CLAN_RANK_PRINCE)
						&& (rank != LsimulatorClan.CLAN_RANK_LEAGUE_PUBLIC)
						&& (rank != LsimulatorClan.CLAN_RANK_LEAGUE_PROBATION)
						&& (rank != LsimulatorClan.CLAN_RANK_LEAGUE_GUARDIAN)
						&& (rank != LsimulatorClan.CLAN_RANK_LEAGUE_VICEPRINCE)
			...
			if ((npcId == 70099) || (npcId == 70796)) {
				...
				pc.getQuest().set_end(LsimulatorQuest.QUEST_OILSKINMANT);
				htmlid = ""; // ウィンドウを消す
			}
			// HomeTown 村莊管理人 支付福利金
				...
				if (pay < 1) {
					...
					if ((item != null)
						...
						pc.getInventory().storeItem(LsimulatorItemId.ADENA, pay);
			...
			if (isBreak) {
				...
				htmlid = "inn20";
				pc.getInventory().storeItem(LsimulatorItemId.ADENA, price); // 取得金幣
			} else {
				htmlid = "";
			}
							...
							if (dueTime != null) { // 時間不為空值
								...
								if (((cal.getTimeInMillis() - dueTime.getTime()) / 1000) < 0) { // 鑰匙租用時間未到
									...
									if (!item.checkRoomOrHall()) { // 房間
										LsimulatorTeleport.teleport(pc, data[0],
												data[1], (short) data[2], 6,
												false);
									} else { // 會議室
										LsimulatorTeleport.teleport(pc, data[3],
			...
			if (npc.getNpcTemplate().get_npcId() == 60514) { // ケント城近衛兵
				...
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.OT_CASTLE_ID);
				htmlid = "orcguard7";
			} else if (npc.getNpcTemplate().get_npcId() == 60552) { // ウィンダウッド城近衛兵
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.WW_CASTLE_ID);
				htmlid = "wdguard7";
				...
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.GIRAN_CASTLE_ID);
				htmlid = "grguard7";
			} else if (npc.getNpcTemplate().get_npcId() == 70857) { // ハイネ城ハイネガード
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.HEINE_CASTLE_ID);
				htmlid = "heguard7";
			} else if ((npc.getNpcTemplate().get_npcId() == 60530) || // ドワーフ城ドワーフガード
					(npc.getNpcTemplate().get_npcId() == 60531)) {
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.DOWA_CASTLE_ID);
				htmlid = "dcguard7";
			} else if ((npc.getNpcTemplate().get_npcId() == 60533) || // アデン城
																		// ガード
					(npc.getNpcTemplate().get_npcId() == 60534)) {
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.ADEN_CASTLE_ID);
				htmlid = "adguard7";
			} else if (npc.getNpcTemplate().get_npcId() == 81156) { // アデン偵察兵（ディアド要塞）
				htmldata = makeWarTimeStrings(LsimulatorCastleLocation.DIAD_CASTLE_ID);
				htmlid = "dfguard3";
			}
			...
			if (clan != null) {
				...
				if (castle_id != 0) { // 城主クラン
					LsimulatorCastle l1castle = CastleTable.getInstance()
				...
				for (LsimulatorItemInstance item : pc.getInventory().getItems()) {
					if (pc.getWeapon().equals(item)) {
						...
						l1skilluse.handleCommands(pc, ENCHANT_WEAPON,
								...
								LsimulatorSkillUse.TYPE_SPELLSC);
						break;
					}
				}
			}
			...
			if (item != null) {
				...
				l1skilluse.handleCommands(pc, BLESSED_ARMOR, item.getId(), 0,
						0, null, 0, LsimulatorSkillUse.TYPE_SPELLSC);
			} else {
				pc.sendPackets(new S_ServerMessage(79));
			}
			htmlid = ""; // ウィンドウを消す
			...
			if (obj instanceof LsimulatorCharacter) {
				...
				if (houseId != 0) {
					...
					if (npc.getNpcTemplate().get_npcId() == keeperId) {
						...
						if (s.equalsIgnoreCase("tel0")) {
							loc = LsimulatorHouseLocation.getHouseTeleportLoc(houseId,
									0);
						...
						LsimulatorTeleport.teleport(pc, loc[0], loc[1], (short) loc[2],
								5, true);
					}
				}
			}
						...
						if (pc.isCrown() && (pc.getId() == clan.getLeaderId())) { // 君主、かつ、血盟主
							if (house.isPurchaseBasement()) {
								...
								if (pc.getInventory().consumeItem(
										LsimulatorItemId.ADENA, 5000000)) {
			...
			if (clan != null) {
				...
				if (houseId != 0) {
					...
					if (npc.getNpcTemplate().get_npcId() == keeperId) {
						if (house.isPurchaseBasement()) {
							int[] loc = new int[3];
							loc = LsimulatorHouseLocation.getBasementLoc(houseId);
							LsimulatorTeleport.teleport(pc, loc[0], loc[1],
									(short) (loc[2]), 5, true);
						} else {
		...
		else if (s.equalsIgnoreCase("fire")) // エルフの属性変更「火の系列を習う」
		{
			...
			if (pc.isElf()) {
				...
				{
					LsimulatorSkills l1skills1 = SkillsTable.getInstance().getTemplate(
							cnt);
			...
			if (pc.getLawful() < 30000) {
				...
				if (pc.getInventory().consumeItem(LsimulatorItemId.ADENA, 700000)) {
			...
			if (npcId == 80085) {
				...
				l1skilluse.handleCommands(pc, CANCELLATION, pc.getId(),
						pc.getX(), pc.getY(), null, 0, LsimulatorSkillUse.TYPE_LOGIN);
				pc.getInventory().takeoffEquip(945); // 牛のpolyIdで装備を全部外す。
				LsimulatorTeleport.teleport(pc, 32737, 32789, (short) 997, 4, false);
		...
		// 変身専門家
			...
			if (s.equalsIgnoreCase("teleport evil-dungeon")) { // 往邪念地監
				...
				if (!find) {
					LsimulatorQuest quest = pc.getQuest();
					int lv50_step = quest.get_step(LsimulatorQuest.QUEST_LEVEL50);
					if (lv50_step == LsimulatorQuest.QUEST_END) {
						htmlid = "csoulq3";
					} else if (lv50_step >= 3) {
						LsimulatorTeleport.teleport(pc, 32747, 32799, (short) 306, 6, true);
					} else {
						htmlid = "csoulq2";
					}
				}
			...
			if (s.equalsIgnoreCase("a")) {
				// 卡瑞的祝福已經環繞整個身軀
				LsimulatorBuffUtil.effectBlessOfDragonSlayer(pc, EFFECT_BLESS_OF_CRAY,
						2400, 7681);
				htmlid = "grayknight2";
			}
			...
			if (s.equalsIgnoreCase("a")) {
				// 巫女莎爾的祝福纏繞著整個身體。
				LsimulatorBuffUtil.effectBlessOfDragonSlayer(pc, EFFECT_BLESS_OF_SAELL,
						2400, 7680);
				htmlid = "";
			}
		}
		...
		// 次元の扉(バルログの部屋)
			...
			if (s.equalsIgnoreCase("a")) {
				if (pc.getInventory().checkItem(40921)) { // 元素の支配者
					LsimulatorTeleport.teleport(pc, 32674, 32832, (short) 603, 2, true);
				} else {
					htmlid = "gpass02";
				}
			}
		...
		// バルログの執政官
			...
			// 「バルログ様に会わせてください」
				...
				if (pc.getInventory().checkItem(40909) // 地の通行証
					...
					LsimulatorTeleport.teleport(pc, 32674, 32832, (short) 602, 2, true);
				}
			}
		}
		// 揺らめく者
		...
		// 一般クエスト / ライラ
			...
			pc.getQuest().set_step(LsimulatorQuest.QUEST_LYRA, 1);
			htmlid = "lyraev2";
			...
			if (s.equalsIgnoreCase("contract1yes")) {
				...
				pc.getQuest().set_step(LsimulatorQuest.QUEST_LYRA, 0);
				htmlid = "lyraev4";
			}
			int totem = 0;
			...
			if (totem != 0) {
				...
				if (pc.getInventory().checkItem(40131)) {
					...
					counts[totem] = i1;
					createitem[totem] = LsimulatorItemId.ADENA;
				...
				if ((town_id >= 1) && (town_id <= 10)) {
					if (pc.getHomeTownId() == -1) {
						...
						if (pc.getHomeTownId() != town_id) {
							LsimulatorTown town = TownTable.getInstance().getTownTable(
									pc.getHomeTownId());
							if (town != null) {
						...
						if (pc.getLevel() < 10) {
							...
							if (pc.getInventory().consumeItem(LsimulatorItemId.ADENA,
									cost)) {
		...
		// タウンマスター（村の村長が誰かを聞く）
			...
			if (obj instanceof LsimulatorNpcInstance) {
				...
				if ((town_id >= 1) && (town_id <= 10)) {
					LsimulatorTown town = TownTable.getInstance().getTownTable(town_id);
		...
		// ルケイン(海賊島)
			...
			if (s.equalsIgnoreCase("0")) {
				...
				htmlid = "lukein12";
				pc.getQuest().set_step(LsimulatorQuest.QUEST_RESTA, 3);
			}
		}
		// 小さな箱-1番目
			...
			if (s.equalsIgnoreCase("0")) {
				...
				createcount = new int[] { 1 };
				htmlid = "maptbox1";
				pc.getQuest().set_end(LsimulatorQuest.QUEST_TBOX1);
				int[] nextbox = { 1, 2, 3 };
				int pid = Random.nextInt(nextbox.length);
				int nb = nextbox[pid];
				if (nb == 1) { // b地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 2);
				} else if (nb == 2) { // c地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 3);
				} else if (nb == 3) { // d地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 4);
				}
			}
		}
		// 小さな箱-2番目
			...
			if (s.equalsIgnoreCase("0")) {
				...
				createcount = new int[] { 1 };
				htmlid = "maptbox1";
				pc.getQuest().set_end(LsimulatorQuest.QUEST_TBOX2);
				int[] nextbox2 = { 1, 2, 3, 4, 5, 6 };
				int pid = Random.nextInt(nextbox2.length);
				int nb2 = nextbox2[pid];
				if (nb2 == 1) { // e地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 5);
				} else if (nb2 == 2) { // f地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 6);
				} else if (nb2 == 3) { // g地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 7);
				} else if (nb2 == 4) { // h地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 8);
				} else if (nb2 == 5) { // i地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 9);
				} else if (nb2 == 6) { // j地点
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LUKEIN1, 10);
				}
			}
		}
		// シミズ(海賊島)
			...
			if (s.equalsIgnoreCase("a")) {
				...
				if (pc.getInventory().checkItem(40661)
					...
					htmlid = "SIMIZZ8";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_SIMIZZ, 2);
				...
				htmlid = "SIMIZZ12";
				pc.getQuest().set_step(LsimulatorQuest.QUEST_SIMIZZ, LsimulatorQuest.QUEST_END);
			}
		}
		// ドイル(海賊島)
			...
			if (s.equalsIgnoreCase("3")) {
				...
				if (pc.getInventory().checkItem(40714)) {
					...
					createcount = new int[] { 1 };
					pc.getQuest().set_step(LsimulatorQuest.QUEST_DOIL,
		...
		// ルディアン(海賊島)
			...
			if (s.equalsIgnoreCase("A")) {
				...
				if (pc.getInventory().checkItem(40710)) {
					...
					createitem = new int[] { 40647 };
					createcount = new int[] { 1 };
					pc.getQuest().set_step(LsimulatorQuest.QUEST_RUDIAN,
							LsimulatorQuest.QUEST_END);
				} else {
					htmlid = "rudian9";
				}
		...
		// レスタ(海賊島)
			...
			if (s.equalsIgnoreCase("A")) {
				...
				htmlid = "resta10";
				pc.getQuest().set_step(LsimulatorQuest.QUEST_RESTA, 2);
			}
		}
		// カドムス(海賊島)
			...
			if (s.equalsIgnoreCase("A")) {
				if (pc.getInventory().checkItem(40647, 3)) {
					...
					pc.getQuest().set_step(LsimulatorQuest.QUEST_CADMUS, 2);
				} else {
					htmlid = "cadmus5";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_CADMUS, 1);
				}
			}
		}
		// カミーラ(海賊島)
			...
			if (s.equalsIgnoreCase("a")) {
				...
				pc.getQuest().set_step(LsimulatorQuest.QUEST_KAMYLA, 3);
			} else if (s.equalsIgnoreCase("e")) {
				htmlid = "kamyla13";
				pc.getInventory().consumeItem(40630, 1);
				pc.getQuest().set_step(LsimulatorQuest.QUEST_KAMYLA, 4);
			} else if (s.equalsIgnoreCase("i")) {
				htmlid = "kamyla25";
			} else if (s.equalsIgnoreCase("b")) { // カーミラ（フランコの迷宮）
				if (pc.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 1) {
					LsimulatorTeleport.teleport(pc, 32679, 32742, (short) 482, 5, true);
				}
			} else if (s.equalsIgnoreCase("d")) { // カーミラ（ディエゴの閉ざされた牢）
				if (pc.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 3) {
					LsimulatorTeleport.teleport(pc, 32736, 32800, (short) 483, 5, true);
				}
			} else if (s.equalsIgnoreCase("f")) { // カーミラ（ホセ地下牢）
				if (pc.getQuest().get_step(LsimulatorQuest.QUEST_KAMYLA) == 4) {
					LsimulatorTeleport.teleport(pc, 32746, 32807, (short) 484, 5, true);
				}
			}
		}
		// フランコ(海賊島)
		...
		// 試練のクリスタル2(海賊島)
			...
			if (s.equalsIgnoreCase("a")) {
				...
				if (pc.getInventory().checkEquipped(246)
					...
					pc.getQuest().set_step(LsimulatorQuest.QUEST_CRYSTAL, 2);
					LsimulatorTeleport.teleport(pc, 32801, 32895, (short) 483, 4, true);
				}
			} else if (s.equalsIgnoreCase("c")) {
				if (pc.getInventory().checkEquipped(246)
						|| pc.getInventory().checkEquipped(247)
					...
					pc.getInventory().consumeItem(249, 1);
					pc.getInventory().consumeItem(40620, 1);
					pc.getQuest().set_step(LsimulatorQuest.QUEST_CRYSTAL, 0);
					LsimulatorTeleport.teleport(pc, 32736, 32800, (short) 483, 4, true);
				}
			}
		}
		// 試練のクリスタル2(海賊島)
			...
			if (s.equalsIgnoreCase("a")) {
				...
				pc.getQuest()
						.set_step(LsimulatorQuest.QUEST_CRYSTAL, LsimulatorQuest.QUEST_END);
				LsimulatorTeleport.teleport(pc, 32744, 32927, (short) 483, 4, true);
			}
		}
		// リザードマンの長老(海賊島)
			...
			if (s.equalsIgnoreCase("A")) {
				htmlid = "lelder5";
				pc.getQuest().set_step(LsimulatorQuest.QUEST_LIZARD, 1);
				// 宝を取り戻してきます
			} else if (s.equalsIgnoreCase("B")) {
				htmlid = "lelder10";
				pc.getInventory().consumeItem(40633, 1);
				pc.getQuest().set_step(LsimulatorQuest.QUEST_LIZARD, 3);
			} else if (s.equalsIgnoreCase("C")) {
				htmlid = "lelder13";
				if (pc.getQuest().get_step(LsimulatorQuest.QUEST_LIZARD) == LsimulatorQuest.QUEST_END) {
				}
				materials = new int[] { 40634 };
				counts = new int[] { 1 };
				createitem = new int[] { 20167 }; // リザードマングローブ
				createcount = new int[] { 1 };
				pc.getQuest().set_step(LsimulatorQuest.QUEST_LIZARD, LsimulatorQuest.QUEST_END);
			}
		}
		// 傭兵団長 ティオン
		...
		// 占星術師ケプリシャ
			...
			// 援助金を出して運勢を見る
				...
				if (!pc.getInventory().checkItem(41314)) { // 占星術師のお守り
					if (pc.getInventory().checkItem(LsimulatorItemId.ADENA, 1000)) {
						materials = new int[] { LsimulatorItemId.ADENA, 41313 }; // アデナ、占星術師の玉
		...
		// 釣魚小童 波爾 (進入釣魚池)
			...
			if (s.equalsIgnoreCase("a")) {
				...
				} else if (pc.getInventory().consumeItem(LsimulatorItemId.ADENA, 1000)) {
					...
					if (pc.getInventory().checkItem(20383, 1)) {
						if (pc.getInventory().checkItem(LsimulatorItemId.ADENA, 100000)) {
							...
							if ((item != null) && (item.getChargeCount() != 50)) {
								...
								pc.getInventory().consumeItem(LsimulatorItemId.ADENA,
										100000);
								htmlid = "";
							}
						} else {
		...
		// 真冥王 ダンテス
			...
			if (s.equalsIgnoreCase("a")) {
				if (pc.getInventory().checkItem(41028, 1)) {
					LsimulatorTeleport.teleport(pc, 32648, 32921, (short) 535, 6, true);
		...
		// 諜報員(欲望の洞窟側)
			...
			if (s.equalsIgnoreCase("n")) {
				...
				pc.getQuest().set_step(LsimulatorQuest.QUEST_DESIRE, 0);
				// 精髄を渡す
			} else if (s.equalsIgnoreCase("e")) {
				if ((pc.getQuest().get_step(LsimulatorQuest.QUEST_DESIRE) == LsimulatorQuest.QUEST_END)
						|| (pc.getKarmaLevel() >= 1)) {
					htmlid = "";
				} else {
					if (pc.getInventory().checkItem(41138)) {
						...
						pc.getInventory().consumeItem(41138, 1); // カヘルの精髄
						pc.getQuest().set_step(LsimulatorQuest.QUEST_DESIRE,
								LsimulatorQuest.QUEST_END);
					} else {
						htmlid = "minicod04";
					}
		...
		// 諜報員(影の神殿側)
			...
			if (s.equalsIgnoreCase("n")) {
				...
				pc.getQuest().set_step(LsimulatorQuest.QUEST_SHADOWS, 0);
				// 精髄を渡す
			} else if (s.equalsIgnoreCase("e")) {
				if ((pc.getQuest().get_step(LsimulatorQuest.QUEST_SHADOWS) == LsimulatorQuest.QUEST_END)
						|| (pc.getKarmaLevel() >= 1)) {
					htmlid = "";
				} else {
					if (pc.getInventory().checkItem(41129)) {
						...
						pc.getInventory().consumeItem(41129, 1); // 血痕の精髄
						pc.getQuest().set_step(LsimulatorQuest.QUEST_SHADOWS,
								LsimulatorQuest.QUEST_END);
					} else {
						htmlid = "minitos04";
					}
		...
		// ゾウのストーンゴーレム テーベ砂漠
			...
			if (s.equalsIgnoreCase("A")) {
				...
				if (pc.getInventory().checkItem(49101, 1)) {
					...
					LsimulatorTeleport.teleport(pc, 33966, 33253, (short) 4, 5, true);
					htmlid = "";
				} else {
					htmlid = "joegolem20";
				}
		...
		// テーベ オシリス祭壇のキーパー
			...
			if (s.equalsIgnoreCase("e")) {
				if (pc.getInventory().checkItem(49242, 1)) { // 鍵のチェック(20人限定/時の歪みが現れてから2h30は未実装)
					...
					LsimulatorTeleport.teleport(pc, 32735, 32831, (short) 782, 2, true);
					htmlid = "";
				} else {
					htmlid = "tebegate3";
					// 「上限人数に達している場合は」
		...
		// ロビンフッド
			...
			if (s.equalsIgnoreCase("E")) {
				if ((pc.getQuest().get_step(LsimulatorQuest.QUEST_MOONOFLONGBOW) == 8)
					...
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW,
							LsimulatorQuest.QUEST_END);
				}
			} else if (s.equalsIgnoreCase("C")) {
				if (pc.getQuest().get_step(LsimulatorQuest.QUEST_MOONOFLONGBOW) == 7) {
					if (pc.getInventory().checkItem(41352, 4)
						...
						htmlid = "robinhood10";
						pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 8);
					}
				}
			} else if (s.equalsIgnoreCase("B")) {
				if (pc.getInventory().checkItem(41348)
					...
					htmlid = "robinhood13";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 2);
				}
			} else if (s.equalsIgnoreCase("A")) {
				if (pc.getInventory().checkItem(40028)) {
					pc.getInventory().consumeItem(40028, 1);
					htmlid = "robinhood4";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 1);
				} else {
					htmlid = "robinhood19";
				}
			}
		...
		// ジブリル
			...
			if (s.equalsIgnoreCase("D")) {
				if (pc.getInventory().checkItem(41349)) {
					...
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 7);
				} else {
					htmlid = "zybril14";
				}
			} else if (s.equalsIgnoreCase("C")) {
				if (pc.getInventory().checkItem(40514, 10)
					...
					htmlid = "zybril9";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 6);
				}
				...
				if (pc.getInventory().checkItem(40048, 10)
					...
					htmlid = "zybril15";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 5);
				} else {
					htmlid = "zybril12";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 4);
				}
			} else if (s.equalsIgnoreCase("A")) {
				if (pc.getInventory().checkItem(41348)
						&& pc.getInventory().checkItem(41346)) {
					htmlid = "zybril3";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_MOONOFLONGBOW, 3);
				} else {
					htmlid = "zybril11";
				}
			}
			...
			if (s.equalsIgnoreCase("A")) {
				if (pc.getInventory().checkItem(LsimulatorItemId.ADENA, 300)) {
					...
					pc.getQuest().set_step(
							LsimulatorQuest.QUEST_GENERALHAMELOFRESENTMENT, 1);
					htmlid = "rarson16";
				} else if (!pc.getInventory().checkItem(LsimulatorItemId.ADENA, 300)) {
					htmlid = "rarson7";
				}
			} else if (s.equalsIgnoreCase("B")) {
				if ((pc.getQuest().get_step(
					...
					pc.getInventory().storeItem(LsimulatorItemId.ADENA, 2000);
				...
				if ((pc.getQuest().get_step(
						LsimulatorQuest.QUEST_GENERALHAMELOFRESENTMENT) == 4)
						&& (pc.getInventory().checkItem(41326, 1))) {
					pc.getInventory().storeItem(LsimulatorItemId.ADENA, 30000);
					pc.getInventory().consumeItem(41326, 1);
					htmlid = "rarson12";
					pc.getQuest().set_step(
							LsimulatorQuest.QUEST_GENERALHAMELOFRESENTMENT, 5);
				} else {
					htmlid = "rarson17";
				}
			} else if (s.equalsIgnoreCase("D")) {
		...
		// クエン
			...
			if (s.equalsIgnoreCase("request letter of kuen")) {
				if ((pc.getQuest().get_step(
					...
					pc.getQuest().set_step(
							LsimulatorQuest.QUEST_GENERALHAMELOFRESENTMENT, 3);
					htmlid = "";
				} else {
					htmlid = "";
				}
		...
		// 長老 普洛凱爾
			...
			int lv50_step = pc.getQuest().get_step(LsimulatorQuest.QUEST_LEVEL50);
		 ...
		 * pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL45, 2); htmlid =
		 * "silrein38"; } } else if (s.equalsIgnoreCase("m") && (lv45_step ==
		 * 2)) { pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL45, 3); htmlid =
		 * "silrein39"; } } }
		 */
			...
			if (s.equalsIgnoreCase("status")) {// status
				...
				for (int i = 0; i < 5; i++) {
					htmldata[i * 3] = (NpcTable.getInstance().getTemplate(
							Lsimulator.server.server.model.game.LsimulatorBugBearRace.getInstance()
									.getRunner(i).getNpcId()).get_nameid());
					String condition;// 610 普通
					if (Lsimulator.server.server.model.game.LsimulatorBugBearRace.getInstance()
							.getCondition(i) == 0) {
						condition = "$610";
					} else {
						if (Lsimulator.server.server.model.game.LsimulatorBugBearRace.getInstance()
					...
					htmldata[i * 3 + 2] = String
							.valueOf(Lsimulator.server.server.model.game.LsimulatorBugBearRace
		...
		// 然柳寵物商
			...
			if (s.equalsIgnoreCase("buy 1")) {
				petNpcId = 45042;// 杜賓狗
				consumeItem = LsimulatorItemId.ADENA;
		...
		// 幻術士 試練任務
			...
			if (pc.isDragonKnight()) {
				if (s.equalsIgnoreCase("l") && (lv45_step == 1)) {
					if (pc.getInventory().checkItem(49209, 1)) { // check
						pc.getInventory().consumeItem(49209, 1); // del
						pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL45, 2);
						htmlid = "silrein38";
					}
				} else if (s.equalsIgnoreCase("m") && (lv45_step == 2)) {
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL45, 3);
					htmlid = "silrein39";
				}
			}
			if (pc.isIllusionist()) {
			...
			if (pc.isCrown()) {
				if (s.equalsIgnoreCase("e")) {
					if (pc.getInventory().checkItem(49159, 1)) {
						...
						pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL50, 2);
					} else {
						htmlid = "dicardingp4a";
					}
				} else if (s.equalsIgnoreCase("d")) {
					htmlid = "dicardingp7";
					LsimulatorPolyMorph.doPoly(pc, 6035, 900, 1, true);
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL50, 3);
				} else if (s.equalsIgnoreCase("c")) {
					htmlid = "dicardingp9";
					LsimulatorPolyMorph.undoPoly(pc);
					LsimulatorPolyMorph.doPoly(pc, 6035, 900, 1, true);
					...
					htmlid = "dicardingp12";
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL50, 4);
			...
			if (pc.isElf()) {
				if (s.equalsIgnoreCase("n")) {
					...
					pc.getQuest().set_step(LsimulatorQuest.QUEST_LEVEL50, 5);
			...
			if (s.equalsIgnoreCase("a") || s.equalsIgnoreCase("b")){
				if(pc.getInventory().consumeItem(LsimulatorItemId.ADENA,3000)){
					LsimulatorSkillUse l1skilluse = new LsimulatorSkillUse();
					for (int i = 0; i < skills.length; i++) {
						l1skilluse.handleCommands(pc, 
								skills[i], pc.getId(), pc.getX(), pc.getY(), null, 0, LsimulatorSkillUse.TYPE_GMBUFF);
					}
					htmlid = "bs_done";           
				} else {
					htmlid = "bs_adena";
			...
			if (level < 13) {
				...
				case 'H':
				case 'h':
					LsimulatorTeleport.teleport(pc, 32575, 32945, (short) 0, 5, true); // 說話之島倉庫管理員
					htmlid = "";
					break;
				case 'I':
				case 'i':
					LsimulatorTeleport.teleport(pc, 32579, 32923, (short) 0, 5, true); // 血盟執行人
					htmlid = "";
					break;
				case 'J':
				case 'j':
				...
				case 'K':
				case 'k':
					LsimulatorTeleport.teleport(pc, 32562, 33082, (short) 0, 5, true); // 魔法師吉倫小屋
					htmlid = "";
					break;
				case 'L':
				case 'l':
					LsimulatorTeleport.teleport(pc, 32792, 32820, (short) 75, 5, true); // 象牙塔
					htmlid = "";
					break;
				case 'M':
				case 'm':
					LsimulatorTeleport.teleport(pc, 32877, 32904, (short) 304, 5, true); // 黑暗魔法師賽帝亞
					htmlid = "";
					break;
				case 'N':
				case 'n':
			...
			if (s.equalsIgnoreCase("A")) {
				if ((level > 4) && (quest_step == 2)) {
					...
					questid = LsimulatorQuest.QUEST_TUTOR2;
					questvalue = 3;
				}
			}
			htmlid = "";
			...
			if (level < 46) {
				...
				case 'B':
				case 'b':
					LsimulatorTeleport.teleport(pc, 33119, 32933, (short) 4, 5, true); // 正義神殿
					htmlid = "";
					break;
				case 'C':
				case 'c':
					LsimulatorTeleport.teleport(pc, 32887, 32652, (short) 4, 5, true); // 邪惡神殿
					htmlid = "";
					break;
				case 'D':
				case 'd':
					LsimulatorTeleport.teleport(pc, 32792, 32820, (short) 75, 5, true); // 販售妖精精靈魔法的琳達
					htmlid = "";
					break;
				case 'E':
				case 'e':
					LsimulatorTeleport.teleport(pc, 32789, 32851, (short) 76, 5, true); // 象牙塔的精靈魔法修煉室
					htmlid = "";
					break;
				case 'F':
				case 'f':
					LsimulatorTeleport.teleport(pc, 32750, 32847, (short) 76, 5, true); // 象牙塔的艾利溫
					htmlid = "";
					break;
				case 'G':
				case 'g':
					if (pc.isDarkelf()) {
						LsimulatorTeleport.teleport(pc, 32877, 32904, (short) 304, 5,
								true); // 黑暗魔法師賽帝亞
						htmlid = "";
					} else {
						htmlid = "lowlv40";
				...
				case 'h':
					if (pc.isDragonKnight()) {
						LsimulatorTeleport.teleport(pc, 32811, 32873, (short) 1001, 5,
								true); // 販售龍騎士技能的森帕爾處
						htmlid = "";
					} else {
						htmlid = "lowlv41";
				...
				case 'i':
					if (pc.isIllusionist()) {
						LsimulatorTeleport.teleport(pc, 32759, 32884, (short) 1000, 5,
								true); // 販售幻術士魔法的史菲爾處
						htmlid = "";
					} else {
						htmlid = "lowlv42";
				...
				case 'J':
				case 'j':
					LsimulatorTeleport.teleport(pc, 32509, 32867, (short) 0, 5, true); // 說話之島的甘特處
					htmlid = "";
					break;
				case 'K':
				case 'k':
			...
			if ((pc.getLevel() > 9) && (townid > 0) && (townid < 11)) {
				...
				case '0':
					createitem = new int[] { 49305 }; // 製作 福利勇敢藥水
														// addContribution + 2
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40014 };
				...
				case '1':
					createitem = new int[] { 49304 }; // 製作 福利森林藥水
														// addContribution + 4
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40068 };
					counts = new int[] { 1000, 3 };
					contribution = 4;
					htmlid = "";
					break;
				case '2':
					createitem = new int[] { 49307 }; // 製作 福利慎重藥水
														// addContribution + 2
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40016 };
				...
				case '3':
					createitem = new int[] { 49306 }; // 製作 福利藍色藥水
														// addContribution + 2
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40015 };
					counts = new int[] { 1000, 3 };
					contribution = 2;
					htmlid = "";
					break;
				case '4':
					createitem = new int[] { 49302 }; // 製作 福利加速藥水
														// addContribution + 1
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40013 };
					counts = new int[] { 500, 3 };
					contribution = 1;
					htmlid = "";
					break;
				case '5':
					createitem = new int[] { 49303 }; // 製作 福利呼吸藥水
														// addContribution + 1
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40032 };
					counts = new int[] { 500, 3 };
					contribution = 1;
					htmlid = "";
					break;
				case '6':
					createitem = new int[] { 49308 }; // 製作 福利變形藥水
														// addContribution + 3
					createcount = new int[] { 1 };
					materials = new int[] { LsimulatorItemId.ADENA, 40088 };
					counts = new int[] { 1000, 3 };
					contribution = 3;
					htmlid = "";
					break;
		...
		// 多魯嘉貝爾
			...
			if (s.equalsIgnoreCase("0")) {
				if (pc.getInventory().checkItem(46000, 1)) { // 檢查身上是否有多魯嘉之袋
					htmlid = "veil3"; // 已經有袋子了
				} else if (pc.getInventory().checkItem(LsimulatorItemId.ADENA, 1000000)) { // 檢查身上金幣是否足夠
					pc.getInventory().consumeItem(LsimulatorItemId.ADENA, 1000000);
			...
			if (materials != null) {
				for (int j = 0; j < materials.length; j++) {
					if (!pc.getInventory().checkItemNotEquipped(materials[j],
							counts[j])) {
						LsimulatorItem temp = ItemTable.getInstance().getTemplate(
								materials[j]);
	...
	private String enterUb(LsimulatorPcInstance pc, int npcId) {
		...
		LsimulatorLocation loc = ub.getLocation().randomLocation(10, false);
		LsimulatorTeleport.teleport(pc, loc.getX(), loc.getY(), ub.getMapId(), 5, true);
		return "";
	}
	...
	private String enterHauntedHouse(LsimulatorPcInstance pc) {
		...
		if (LsimulatorHauntedHouse.getInstance().getMembersCount() >= 10) { // 定員オーバー
			pc.sendPackets(new S_ServerMessage(1184)); // お化け屋敷は人でいっぱいだよ。
			return "";
		}

		LsimulatorHauntedHouse.getInstance().addMember(pc); // メンバーに追加
		LsimulatorTeleport.teleport(pc, 32722, 32830, (short) 5140, 2, true);
		return "";
	}
	...
	private String enterPetMatch(LsimulatorPcInstance pc, int objid2) {
		...
		if (!LsimulatorPetMatch.getInstance().enterPetMatch(pc, objid2)) {
			pc.sendPackets(new S_ServerMessage(1182)); // もうゲームは始まってるよ。
		}
		return "";
	}
	...
	private void poly(ClientThread clientthread, int polyId) {
		...
		if (pc.getInventory().checkItem(LsimulatorItemId.ADENA, 100)) { // check
			pc.getInventory().consumeItem(LsimulatorItemId.ADENA, 100); // del
	...
	private void polyByKeplisha(ClientThread clientthread, int polyId) {
		...
		if (pc.getInventory().checkItem(LsimulatorItemId.ADENA, 100)) { // check
			...
			LsimulatorPolyMorph.doPoly(pc, polyId, 1800, LsimulatorPolyMorph.MORPH_BY_KEPLISHA);
		} else {
			pc.sendPackets(new S_ServerMessage(337, "$4")); // アデナが不足しています。
		}
	}
	...
	private void openCloseGate(LsimulatorPcInstance pc, int keeperId, boolean isOpen) {
		...
		if ((keeperId == 70656) || (keeperId == 70549) || (keeperId == 70985)) { // ケント城
			if (isExistDefenseClan(LsimulatorCastleLocation.KENT_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.KENT_CASTLE_ID) {
					return;
				}
			}
			...
			if (isExistDefenseClan(LsimulatorCastleLocation.OT_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.OT_CASTLE_ID) {
					return;
				}
			}
			isNowWar = WarTimeController.getInstance().isNowWar(
				...
				|| (keeperId == 70687)) { // WW城
			if (isExistDefenseClan(LsimulatorCastleLocation.WW_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.WW_CASTLE_ID) {
					return;
				}
			}
			isNowWar = WarTimeController.getInstance().isNowWar(
			...
			if (isExistDefenseClan(LsimulatorCastleLocation.GIRAN_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.GIRAN_CASTLE_ID) {
					return;
				}
			}
			isNowWar = WarTimeController.getInstance().isNowWar(
				...
				|| (keeperId == 70862)) { // ハイネ城
			if (isExistDefenseClan(LsimulatorCastleLocation.HEINE_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.HEINE_CASTLE_ID) {
					return;
				}
			}
			isNowWar = WarTimeController.getInstance().isNowWar(
			...
			if (isExistDefenseClan(LsimulatorCastleLocation.DOWA_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.DOWA_CASTLE_ID) {
					return;
				}
			}
			isNowWar = WarTimeController.getInstance().isNowWar(
			...
			if (isExistDefenseClan(LsimulatorCastleLocation.ADEN_CASTLE_ID)) {
				if (pcCastleId != LsimulatorCastleLocation.ADEN_CASTLE_ID) {
					return;
				}
			}
			isNowWar = WarTimeController.getInstance().isNowWar(
		...
		for (LsimulatorObject object : LsimulatorWorld.getInstance().getObject()) {
			if (object instanceof LsimulatorPcInstance) {
				...
				if (LsimulatorHouseLocation.isInHouseLoc(houseId, pc.getX(), pc.getY(),
					...
					loc = LsimulatorHouseLocation.getHouseTeleportLoc(houseId, 0);
					if (pc != null) {
						LsimulatorTeleport.teleport(pc, loc[0], loc[1], (short) loc[2],
								5, true);
					}
	...
	private String[] makeWarTimeStrings(int castleId) {
		LsimulatorCastle castle = CastleTable.getInstance().getCastleTable(castleId);
		if (castle == null) {
			return null;
		}
		...
		String[] result;
		if (castleId == LsimulatorCastleLocation.OT_CASTLE_ID) {
	...
	private boolean usePolyScroll(LsimulatorPcInstance pc, int itemId, String s) {
		...
		if ((poly != null) || s.equals("none")) {
			if (s.equals("none")) {
				...
				LsimulatorPolyMorph.doPoly(pc, poly.getPolyId(), time,
						LsimulatorPolyMorph.MORPH_BY_ITEMMAGIC);
				isUseItem = true;
			}
		}
		if (isUseItem) {
