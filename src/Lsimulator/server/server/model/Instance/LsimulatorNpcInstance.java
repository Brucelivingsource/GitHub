...
import static Lsimulator.server.server.model.identity.LsimulatorItemId.B_POTION_OF_GREATER_HASTE_SELF;
import static Lsimulator.server.server.model.identity.LsimulatorItemId.B_POTION_OF_HASTE_SELF;
import static Lsimulator.server.server.model.identity.LsimulatorItemId.POTION_OF_EXTRA_HEALING;
import static Lsimulator.server.server.model.identity.LsimulatorItemId.POTION_OF_GREATER_HASTE_SELF;
import static Lsimulator.server.server.model.identity.LsimulatorItemId.POTION_OF_GREATER_HEALING;
import static Lsimulator.server.server.model.identity.LsimulatorItemId.POTION_OF_HASTE_SELF;
import static Lsimulator.server.server.model.identity.LsimulatorItemId.POTION_OF_HEALING;
...
import static Lsimulator.server.server.model.skill.LsimulatorSkillId.POLLUTE_WATER;
...
import Lsimulator.server.server.model.LsimulatorGroundInventory;
import Lsimulator.server.server.model.LsimulatorHateList;
...
import Lsimulator.server.server.model.LsimulatorMobSkillUse;
import Lsimulator.server.server.model.LsimulatorNpcChatTimer;
...
import Lsimulator.server.server.utils.TimerPool;
import Lsimulator.server.server.utils.collections.Lists;
import Lsimulator.server.server.utils.collections.Maps;

public class LsimulatorNpcInstance extends LsimulatorCharacter {
	...
	private static Logger _log = Logger.getLogger(LsimulatorNpcInstance.class.getName());

	private LsimulatorNpc _npcTemplate;

	private LsimulatorSpawn _spawn;

	private int _spawnNumber; // LsimulatorSpawnで管理されているナンバー

	private int _petcost; // ペットになったときのコスト

	public LsimulatorInventory _inventory = new LsimulatorInventory();

	private LsimulatorMobSkillUse mobSkill;
	...
	private boolean AIProcess() {
		...
		if (_target == null) {
			...
			if (_targetItem == null) {
				...
				// onTargetItem();
				LsimulatorInventory groundInventory = LsimulatorWorld.getInstance()
	...
	// ヘイトの設定
	public void setHate(LsimulatorCharacter cha, int hate) {
	...
	public void serchLink(LsimulatorPcInstance targetPlayer, int family) {
		List<LsimulatorObject> targetKnownObjects = targetPlayer.getKnownObjects();
		for (Object knownObject : targetKnownObjects) {
			if (knownObject instanceof LsimulatorNpcInstance) {
				LsimulatorNpcInstance npc = (LsimulatorNpcInstance) knownObject;
				...
				LsimulatorMobGroupInfo mobGroupInfo = getMobGroupInfo();
	...
	public void onTarget() {
		...
		_targetItem = null;
		LsimulatorCharacter target = _target; // ここから先は_targetが変わると影響出るので別領域に参照確保
	...
	// 目標を指定のスキルで攻撃
	public void attackTarget(LsimulatorCharacter target) {
		if (target instanceof LsimulatorPcInstance) {
			LsimulatorPcInstance player = (LsimulatorPcInstance) target;
			...
			LsimulatorSummonInstance summon = (LsimulatorSummonInstance) target;
		...
		if (this instanceof LsimulatorPetInstance) {
			LsimulatorPetInstance pet = (LsimulatorPetInstance) this;
			LsimulatorCharacter cha = pet.getMaster();
			if (cha instanceof LsimulatorPcInstance) {
				LsimulatorPcInstance player = (LsimulatorPcInstance) cha;
				if (player.isTeleport()) { // テレポート処理中
		...
		} else if (this instanceof LsimulatorSummonInstance) {
			LsimulatorSummonInstance summon = (LsimulatorSummonInstance) this;
			LsimulatorCharacter cha = summon.getMaster();
			if (cha instanceof LsimulatorPcInstance) {
				LsimulatorPcInstance player = (LsimulatorPcInstance) cha;
				if (player.isTeleport()) { // テレポート処理中
		...
		if (target instanceof LsimulatorNpcInstance) {
			LsimulatorNpcInstance npc = (LsimulatorNpcInstance) target;
		...
		LsimulatorAttack attack = new LsimulatorAttack(this, target);
		if (attack.calcHit()) {
			if (target.hasSkillEffect(COUNTER_BARRIER)) {
				LsimulatorMagic magic = new LsimulatorMagic(target, this);
	...
	public void searchTargetItem() {
		List<LsimulatorGroundInventory> gInventorys = Lists.newList();

		for (LsimulatorObject obj : LsimulatorWorld.getInstance().getVisibleObjects(this)) {
			if ((obj != null) && (obj instanceof LsimulatorGroundInventory)) {
				gInventorys.add((LsimulatorGroundInventory) obj);
			}
		}
		...
		LsimulatorGroundInventory inventory = gInventorys.get(pickupIndex);
		for (LsimulatorItemInstance item : inventory.getItems()) {
			if (getInventory().checkAddItem(item, item.getCount()) == LsimulatorInventory.OK) { // 持てるならターゲットアイテムに加える
				_targetItem = item;
	...
	public void searchItemFromAir() { // 怪物飛天中，發現特定道具時解除飛天撿拾道具
		...
		for (LsimulatorItemInstance item : inventory.getItems()) {
			if ((item.getItem().getType() == 6) // potion
				...
				if (getInventory().checkAddItem(item, item.getCount()) == LsimulatorInventory.OK) {
					if (getHiddenStatus() == HIDDEN_STATUS_FLY) {
						setHiddenStatus(HIDDEN_STATUS_NONE);
						setStatus(LsimulatorNpcDefaultAction.getInstance().getStatus(getTempCharGfx()));
	...
	public static void shuffle(LsimulatorObject[] arr) {
		for (int i = arr.length - 1; i > 0; i--) {
			int t = Random.nextInt(i);

			// 選ばれた値と交換する
			LsimulatorObject tmp = arr[i];
			arr[i] = arr[t];
			arr[t] = tmp;
		}
	}
	...
	// アイテムを拾う
	public void pickupTargetItem(LsimulatorItemInstance targetItem) {
		LsimulatorInventory groundInventory = LsimulatorWorld.getInstance().getInventory(
				targetItem.getX(), targetItem.getY(), targetItem.getMapId());
		LsimulatorItemInstance item = groundInventory.tradeItem(targetItem,
	...
	public boolean noTarget() {
		if ((_master != null)
			...
			if (LsimulatorWorld.getInstance().getRecognizePlayer(this).isEmpty()) {
			...
			if ((_master == null) && (getPassispeed() > 0) && !isRest()) {
				...
				if ((mobGroupInfo == null)
					...
					LsimulatorNpcInstance leader = mobGroupInfo.getLeader();
	...
	public void onFinalAction(LsimulatorPcInstance pc, String s) {
	}
	...
	// 指定されたターゲットを削除
	public void targetRemove(LsimulatorCharacter target) {
	...
	// マスターの設定
	public void setMaster(LsimulatorCharacter cha) {
		_master = cha;
	}

	// マスターの取得
	public LsimulatorCharacter getMaster() {
		return _master;
	}
		...
		if (_npcTemplate.get_npcId() == 45032) { // ブロッブ
			...
			if ((getExp() != 0) && !_inventory.checkItem(89)) {
				...
				if (_inventory.checkItem(materials, counts)) {
					...
					for (int j = 0; j < createitem.length; j++) {
						LsimulatorItemInstance item = _inventory.storeItem(
	...
	protected LsimulatorHateList _hateList = new LsimulatorHateList();

	protected LsimulatorHateList _dropHateList = new LsimulatorHateList();

	// ※攻撃するターゲットの判定とＰＴ時のドロップ判定で使用
	protected List<LsimulatorItemInstance> _targetItemList = Lists.newList(); // ダーゲットアイテム一覧

	protected LsimulatorCharacter _target = null; // 現在のターゲット

	protected LsimulatorItemInstance _targetItem = null; // 現在のターゲットアイテム

	protected LsimulatorCharacter _master = null; // 主人orグループリーダー
	...
	public LsimulatorHateList getHateList() {
		return _hateList;
	}
	...
	public final void startHpRegeneration() {
		...
		if (!_hprRunning && (hprInterval > 0) && (hpr > 0)) {
			...
			LsimulatorNpcRegenerationTimer.getInstance().scheduleAtFixedRate(_hprTimer,
	...
	public final void startMpRegeneration() {
		...
		if (!_mprRunning && (mprInterval > 0) && (mpr > 0)) {
			...
			LsimulatorNpcRegenerationTimer.getInstance().scheduleAtFixedRate(_mprTimer,
			...
			while (!_destroyed && (_digestItems.size() > 0)) {
				...
				for (Object key2 : keys) {
					...
					if (digestCounter <= 0) {
						...
						LsimulatorItemInstance digestItem = getInventory().getItem(key);
						if (digestItem != null) {
	...
	public LsimulatorNpcInstance(LsimulatorNpc template) {
		setStatus(0);
		setMoveSpeed(0);
		setDead(false);
		setreSpawn(false);
	...
	// 指定のテンプレートで各種値を初期化
	public void setting_template(LsimulatorNpc template) {
		...
		if (template.get_atkspeed() != 0) {
			...
			if (LsimulatorNpcDefaultAction.getInstance().getDefaultAttack(getTempCharGfx()) != actid) {
				actid = LsimulatorNpcDefaultAction.getInstance().getDefaultAttack(getTempCharGfx());
			}
		...
		mobSkill = new LsimulatorMobSkillUse(this);
	}
	...
	@Override
	public LsimulatorInventory getInventory() {
		return _inventory;
	}

	public void setInventory(LsimulatorInventory inventory) {
		_inventory = inventory;
	}

	public LsimulatorNpc getNpcTemplate() {
		return _npcTemplate;
	}
	...
	public void deleteMe() {
		...
		List<LsimulatorPcInstance> players = LsimulatorWorld.getInstance().getRecognizePlayer(
				this);
		if (players.size() > 0) {
			...
			for (LsimulatorPcInstance pc : players) {
				if (pc != null) {
					...
					// if(!LsimulatorCharacter.distancepc(user, this))
	...
	public void ReceiveManaDamage(LsimulatorCharacter attacker, int damageMp) {
	}

	public void receiveDamage(LsimulatorCharacter attacker, int damage) {
	}

	public void setDigestItem(LsimulatorItemInstance item) {
	...
	public void approachPlayer(LsimulatorPcInstance pc) {
	...
	public void appearOnGround(LsimulatorPcInstance pc) { // 怪物解除遁地、飛天、冰凍
		if (getHiddenStatus() == HIDDEN_STATUS_SINK) {
			setHiddenStatus(HIDDEN_STATUS_NONE);
			setStatus(LsimulatorNpcDefaultAction.getInstance().getStatus(getTempCharGfx()));
	...
	public void setDirectionMove(int dir) {
		if (dir >= 0) {
			...
			if (getMovementDistance() > 0) {
				if ((this instanceof LsimulatorGuardInstance)
						|| (this instanceof LsimulatorMerchantInstance)
						|| (this instanceof LsimulatorMonsterInstance)) {
	...
	private boolean isExsistCharacterBetweenTarget(int dir) {
		if (!(this instanceof LsimulatorMonsterInstance)) { // モンスター以外は対象外
			return false;
		}
		...
		for (LsimulatorObject object : LsimulatorWorld.getInstance().getVisibleObjects(this, 1)) {
			// PC, Summon, Petがいる場合
	...
	public static int checkObject(int x, int y, short m, int d) { // 起点Ｘ 起点Ｙ
																	// マップＩＤ
		// 進行方向
		LsimulatorMap map = LsimulatorWorldMap.getInstance().getMap(m);
		if (d == 1) {
	...
	private void useHealPotion(int healHp, int effectId) {
		...
		if (this instanceof LsimulatorPetInstance) {
			((LsimulatorPetInstance) this).setCurrentHp(getCurrentHp() + healHp);
		} else if (this instanceof LsimulatorSummonInstance) {
			((LsimulatorSummonInstance) this).setCurrentHp(getCurrentHp() + healHp);
		} else {
	...
	protected void transform(int transformId) {
		...
		LsimulatorNpc npcTemplate = NpcTable.getInstance().getTemplate(transformId);
	...
	public synchronized void resurrect(int hp) {
		...
		skill.handleCommands(null, CANCELLATION, getId(), getX(), getY(), null,
				0, LsimulatorSkillUse.TYPE_LOGIN, this);
	}
	...
	protected static class DeleteTimer extends TimerTask {
		...
		protected DeleteTimer(int oId) {
			_id = oId;
			if (!(LsimulatorWorld.getInstance().findObject(_id) instanceof LsimulatorNpcInstance)) {
				throw new IllegalArgumentException("allowed only LsimulatorNpcInstance");
			}
		}

		@Override
		public void run() {
			LsimulatorNpcInstance npc = (LsimulatorNpcInstance) LsimulatorWorld.getInstance()
					.findObject(_id);
	...
	private LsimulatorMobGroupInfo _mobGroupInfo = null;

	public boolean isInMobGroup() {
		return getMobGroupInfo() != null;
	}

	public LsimulatorMobGroupInfo getMobGroupInfo() {
		return _mobGroupInfo;
	}

	public void setMobGroupInfo(LsimulatorMobGroupInfo m) {
		_mobGroupInfo = m;
	}
	...
	public void startChat(int chatTiming) {
		...
		LsimulatorNpcChat npcChat = null;
		...
		LsimulatorNpcChatTimer npcChatTimer = new LsimulatorNpcChatTimer(this, npcChat);
		if (!npcChat.isRepeat()) {
